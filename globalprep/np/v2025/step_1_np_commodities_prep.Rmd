...
# raw_quant <- read_csv(here(dir_M, "git-annex", "globalprep", "_raw_data", "FAO_commodities", data_dir_year, "FAO_raw_commodities_quant_1976_2023.csv"))
# view(raw_quant)
...
# ===== loop ================
# for (f in files) {
  f <- files_old[2] # un-comment and update to test individual files # 1 = quant, 2 = usd
  cat(sprintf('\n\n\n====\nfile: %s\n', basename(f)))
...
tonnes_comb <- new_tonnes %>% 
  rename(new_tonnes = tonnes) %>% 
  left_join(old_tonnes, by = join_by(rgn_id, rgn_name, commodity, product, year)) %>% 
  mutate(diff_tonnes = new_tonnes - tonnes) %>% 
  view()

write_csv(tonnes_comb, here("globalprep", "np", "v2024", "test", "tonnes_comb_step1.csv"))
read_csv(tonnes_comb, here("globalprep", "np", "v2024", "test", "tonnes_comb_step1.csv"))

usd_comb <- new_usd %>% 
  rename(new_usd = usd) %>% 
  left_join(old_usd, by = join_by(rgn_id, rgn_name, commodity, product, year)) %>% 
  mutate(diff_usd = new_usd - usd) %>% 
  view()

write_csv(usd_comb, here("globalprep", "np", "v2024", "test", "usd_comb_step1.csv"))


# Plot sustainable tonnes
comp_plot_1 <- ggplot() + 
  geom_jitter(data = tonnes_comb, aes(x = tonnes, y = new_tonnes, text = paste("product:", product, "<br>region:", rgn_name)), alpha = 0.7) +
...
comp_plot_2 <- ggplot() + 
  geom_jitter(data = usd_comb, aes(x = usd, y = new_usd, text = paste("product:", product, "<br>region:", rgn_name)), alpha = 0.7) +
  # labs(x = paste("Previous Value", previous_data_year),
  #      y = paste("Current Value", latest_data_year),
  #      title = paste0("Sustainable tonnes: ", scen_year_number, " Assessment (",latest_data_year, " data) vs. ", prev_scen_year, " Assessment (",previous_data_year, " data)", " All Countries")) +
  geom_abline(slope = 1, intercept = 0, col = "red") +
...
# v2025: I think this file name should have '_gf' at the end of it. I ran into an error in step 2 where the file 'np_harvest_usd_tonnes_gf.csv' was missing, and it looks like it should be this one
# readr::write_csv(h_prod, here::here(current_np_dir, "int", "np_harvest_usd_tonnes.csv"))

# Rewriting to adding the gapfill notation
readr::write_csv(h_prod, here::here(current_np_dir, "int", "np_harvest_usd_tonnes_gf.csv"))
...
comp_plot_2 <- ggplot() + 
  geom_jitter(data = usd_comb, aes(x = tonnes, y = new_tonnes, text = paste("product:", product, "<br>region:", rgn_name)), alpha = 0.7) +
  # labs(x = paste("Previous Value", previous_data_year),
  #      y = paste("Current Value", latest_data_year),
  #      title = paste0("Sustainable tonnes: ", scen_year_number, " Assessment (",latest_data_year, " data) vs. ", prev_scen_year, " Assessment (",previous_data_year, " data)", " All Countries")) +
  geom_abline(slope = 1, intercept = 0, col = "red") +
