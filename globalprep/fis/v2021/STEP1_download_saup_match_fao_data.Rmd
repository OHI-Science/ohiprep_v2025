...
writeBin(bin, con) # write each zip file of 10 regions from SAUP
close(con)
...
sum(test$tonnes) # 104583995 # seems reasonable... i think Watson data was something like 111 million (including high seas)
...
test <- high_seas_prod %>% 
  distinct(scientific_name, common_name, functional_group)

write.csv(test, "tmp/saup_high_seas_species.csv", row.names = FALSE)
