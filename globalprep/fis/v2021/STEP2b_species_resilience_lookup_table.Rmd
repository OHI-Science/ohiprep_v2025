---
title: "OHI 2021: FP, SAUP Species Resilience Lookup Table"
...
# res <- read.csv("int/res_local.csv") ## v2021: I had to run this chunk on my local version of R, because for some reason it wasn't working on mazu... if that happens in v2022, just write sp to your local and run there (use archive/fishbase_test.Rmd script).
...
resil <- mclapply(sp_w_nas, stocks, fields = c("Resilience","Species"), mc.cores=5, server = "sealifebase")
...
# res <- read.csv("int/res_nas_local.csv") ## v2021: I had to run this chunk on my local version of R, because for some reason it wasn't working on mazu... if that happens in v2022, just write sp_w_nas to your local and run there (use archive/fishbase_test.Rmd script).
...
taxon_res3 <- taxon_res2 %>%
  ...
  mutate(Resilience = ifelse(is.na(Resilience.x), Resilience.y, Resilience.x), 
         SpecCode = ifelse(is.na(SpecCode.x), SpecCode.y, SpecCode.x)) %>%
  dplyr::select(-Resilience.x, -Resilience.y, -SpecCode.x, -SpecCode.y)
```
