...
load(here::here(RAM_dir, "RAMLDB v4.66/R Data/DBdata[asmt][v4.66].RData")) # update if downloading a new database
...
test <- ram_bmsy_new %>%
  # Should we update max year here? 
...
plot(test) # 207 stocks have data until 2019
```
Gapfill 5 years into the future for the stocks that drop off. 
...
**Don't limit data years to 2019** = more gapfilling. Go to 2023
...
ram_bmsy_gf <- ram_bmsy_new %>%
  ...
  group_by(stockid) %>%
  mutate(max_year = max(year),
         # v2025: added 2010 and 2015 because we increased the max year 
         years_data_2015_now = length(ram_bmsy[year >= 2015]),
         years_data_2010_now = length(ram_bmsy[year >= 2010]),
...
ram_bmsy_gf_3 <- ram_bmsy_gf %>%
  spread(year, ram_bmsy) %>% 
  gather("year", "ram_bmsy", -stockid, -years_data_2015_now, -years_data_2010_now, -years_data_2005_now, -years_data_2001_now, -stocklong, -max_year) %>%
  mutate(year = as.numeric(year)) %>%
  dplyr::select(-years_data_2015_now, -years_data_2010_now, -years_data_2005_now, -years_data_2001_now)
...
unique(is.na(ram_bmsy_gf_final$ram_bmsy_gf)) # v2025: FALSE
...
**Issue 2025**: I cannot find this intermediate file. It looks like it gets made in the next step, so jumping there for now. 
...
# v2025:
 ...
 # 1st Qu.:2006   1st Qu.:6.125e+09  
 # Median :2012   Median :1.227e+11  
 # Mean   :2012   Mean   :7.151e+11  
 # 3rd Qu.:2018   3rd Qu.:4.412e+11  
 # Max.   :2023   Max.   :3.044e+13  
 #                NA's   :13340      
 #    ram_bmsy           gapfilled     
 # Min.   : 0.002532   Min.   :0.0000  
 # 1st Qu.: 0.646000   1st Qu.:0.0000  
 # Median : 1.055343   Median :0.0000  
 # Mean   : 1.180505   Mean   :0.2598  
 # 3rd Qu.: 1.600000   3rd Qu.:1.0000  
