...
# usd_tonnes_raw <- read_csv(here(current_np_dir, "int", "harvest_tonnes_usd_weighting_gf.csv"))
...
usd_tonnes <- usd_tonnes_raw %>% 
  select(rgn_id, product, year, usd_per_tonne, mean_usd_per_tonne, georgn_usd_per_tonne, global_usd_per_tonne)
...
np_value_test <- np_value %>% 
  ...
  dplyr::select(rgn_id, year, product, tonnes, mean_usd_per_tonne, georgn_usd_per_tonne, global_usd_per_tonne, usd_product, overall_usd_value, weight) %>%
  filter(rgn_id != 213) 
...
readr::write_csv(np_value_test, here(current_np_dir, "output", "np_product_weights.csv"))
...
old_prod_weights <- read_csv(here(previous_np_dir, "test", "harvest_weighting_values.csv")) %>% 
  select(!c(georgn_id, level))
prod_weights <- read_csv(here(current_np_dir, "int", "harvest_weighting_values.csv"))  %>% 
  select(!c(georgn_id, level))
...
# Read in current, previous, and 2 years previous data
old_prod_weights <- read_csv(here(previous_np_dir, "test", "np_product_weights.csv"))
prod_weights <- read_csv(here(current_np_dir, "output", "np_product_weights.csv"))
view(prod_weights)
