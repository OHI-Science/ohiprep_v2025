...
gf_mod <- lm(tourism_arrivals_ct ~ year, data = lm_coef_data_tourism_int)
summary(gf_mod)

mod_plot2 <- ggplot(lm_coef_data_tourism_int, aes(y = tourism_arrivals_ct, x = year)) +
  geom_point(alpha = 0.5) + 
  geom_abline(intercept = gf_mod$coefficients[1], 
              slope = gf_mod$coefficients[2], 
              color = "red", linetype = "dashed") +
  theme_minimal() + 
  labs(title = "Tourism arrivals vs Year, International data",
       x = "Year",
       y = "Tourism Arrivals");ggplotly(mod_plot2)
