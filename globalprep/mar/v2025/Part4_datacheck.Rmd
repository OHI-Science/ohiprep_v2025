...
diff_plot <- ggplot() +
  geom_abline(slope = 1, intercept = 0, col = "red") +
  geom_point(data = sust_total, aes(x = sust_new, y = sust_old, text = paste("Region ID:", rgn_id, "<br>", "Species:", species))) +
...
total_taiwan <- sust_total %>% 
  filter(rgn_id == 14)

nrow(total_taiwan) # 100

taiwan_plot <- ggplot() +
  geom_abline(slope = 1, intercept = 0, col = "red") +
  # Added jitter and alpha to help with the overlap
  geom_jitter(data = total_taiwan, aes(x = sust_new, y = sust_old, text = paste("Region ID:", rgn_id, "<br>", "Species:", species)), alpha = 0.6) +
  labs(title = "Sustainability Taiwan 2025 vs. 2024",
       x = "Sust Coeff 2025",
       y = "Sust Coeff 2024") +
  theme_bw();plotly::ggplotly(taiwan_plot)
