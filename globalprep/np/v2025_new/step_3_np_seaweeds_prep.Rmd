...
# mar_raw <- readr::read_csv(here(
#   dir_M, "git-annex", "globalprep", "_raw_data", "FAO_mariculture", prev_data_dir_year, "FAO_GlobalAquacultureProduction_Quantity_1950_2022.csv"))
...
```{r}
ton_new <- read_csv(here(current_np_dir, "int", "np_seaweeds_tonnes_weighting.csv"))

ton_old <- read_csv(here(previous_np_dir, "int", "np_seaweeds_tonnes_weighting.csv"))
colnames(ton_new)
view(ton_new)
ton_old <- ton_old %>% 
  # filter(rgn_id == 182) %>% 
  dplyr::select(rgn_id, species, environment, old_tonnes = tonnes, old_include = include, year, old_gap_0_fill = gap_0_fill, old_value = value)

new_ton <- ton_new %>% 
  # filter(rgn_id == 182) %>% 
  dplyr::select(rgn_id, species, tonnes, environment, include, year, gap_0_fill, value)

tonnes <- ton_old %>% 
  full_join(new_ton, by = c("rgn_id", "species", "year", "environment")) %>% 
...
unique(diff_tonnes$rgn_name) 
length(unique(diff_tonnes$rgn_name)) # 11
length(unique(tonnes$rgn_name)) # 52

setdiff(tonnes$tonnes, tonnes$old_tonnes) # 104
setdiff(tonnes$old_tonnes, tonnes$tonnes) # 40
nrow(tonnes) # 3351
...
# 
# sw_leb <- old_leb %>% 
#   full_join(new_leb, by = c("rgn_id", "product", "year")) %>% 
#   relocate(year, .after = rgn_id) %>% 
#   relocate(tonnes, .after = year) %>% 
#   relocate(old_tonnes, .after = tonnes)
# 
...
ggplot(diff_sw, aes(x = old_tonnes, y = tonnes)) +
  geom_point()
...
unique(diff$rgn_name) 
length(unique(diff$rgn_name)) # 14
length(unique(sw_leb$rgn_name)) # 51

setdiff(sw_leb$tonnes, sw_leb$old_tonnes) # 115
setdiff(sw_leb$old_tonnes, sw_leb$tonnes) # 202
nrow(sw_leb) # 821
