...
if(!file.exists(cell_summary_file) | reload == TRUE) {
  ...
  ptm <- system.time({
    cell_values_all <- parallel::mclapply(comp_files, mc.cores = 12,
                                          FUN = function(x) {
                                            read_csv(x, col_types = 'dddiidi')
                                            }) %>%
      bind_rows()
  }) ### end of system.time
  message('... processing time ', ptm[3], ' sec') # ... processing time 11.7670000000071  sec
  ...
  # system.time({ 
    ### mclapply: 59.2379999999976 secs
  ptm <- system.time({
  ...
  ptm <- system.time({ #processing time 19.4599999999919 sec
