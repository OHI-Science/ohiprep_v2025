...
length(spp_2) # 58
# v2025 went from 62 species to 58 species (4 completely human food species removed)
...
setdiff(spp_2, spp_3) # 19 species lost 
# v2024:
...
#  [5] "BANGIACEAE"                 "Bright green nori"         
#  [7] "CHORDARIACEAE"              "DUNALIELLACEAE"            
#  [9] "Giant kelps nei"            "GIGARTINACEAE"             
# [11] "Green sea feather"          "Kelp nei"                  
# [13] "LESSONIACEAE"               "Mozuku"                    
# [15] "OSCILLATORIACEAE"           "SARGASSACEAE"              
# [17] "SOLIERIACEAE"               "ULOTRICHACEAE"             
# [19] "Wakame nei"

setdiff(rgn_1, rgn_2)
# v2025: NA/60 unique regions?
...
# v2025 - went from 3596 to 3562 decrease of 34 indicates expected trend from when Zanzibar / Tanzania were combined
nrow(mar_rgn_all)
nrow(mar_rgn)
...
## 561 out of 3454 jad NA converted to 0 - v2025 ~ 16.2%
```
...
# 113 unique identifiers - v2025

# Join identifier and maric
...
maric <- mar_rgn_gf
setdiff(mar_rgn_gf$species, sw_sus_rgn$species) # 31, all rows in mar that aren't in sfw
setdiff(sw_sus_rgn$species, mar_rgn_gf$species) # 71, all rows in sfw that aren't in mar
...
seaweed_st_rolling <- mar_sw_sust_tonnes %>%
  ...
  dplyr::mutate(
      sust_tonnes_rollmean = zoo::rollapply(sust_tonnes, width = 5,
...
> NOTE! Because relative sustainability is calculated using a rolling mean and the maximum sustainable tonnes from the previous ten years, the data check for this value will be a little strange. The temperoal reference point means that a change in the `peak_sust_tonnes` will change relative sustainability for everything. 
...
# In case you want to change to other years easily...
...
# Join previous assessment and current assessment data
sw_total1 <- old_sw1 %>% 
...
###### Relative Sustainable tonnes ######
# REMINDER: this is the data check graph that may look strange. Check the relative sustainable tonnes section for more context about this. 
comp_plot2 <- ggplot() + 
...
OLD: Data check for a specific country (mainly for troubleshooting purposes)

```{r eval=FALSE}
old_county <- old_seaweed %>%
  filter(rgn_id == 224) %>%
  dplyr::select(rgn_id, product, old_rel_sust_tonnes = rel_sust_tonnes, old_sust = sustainability, old_tonnes = tonnes, old_sust_tonnes = sust_tonnes, old_peak = peak_sust_tonnes, year)

new_country <- new_seaweed %>%
  filter(rgn_id == 224) %>%
  dplyr::select(rgn_id, product, rel_sust_tonnes, sustainability, tonnes, sust_tonnes, peak_sust_tonnes, year)

sw_country <- old_country %>%
  full_join(new_country, by = c("rgn_id", "product", "year")) %>%
  relocate(year, .after = rgn_id) %>%
  relocate(tonnes, .after = year) %>%
  relocate(old_tonnes, .after = tonnes)

sw_country <- sw_country %>%
  left_join(rgn_labels, by = "rgn_id") %>%
  select(-type, rgn_name = label) %>%
  relocate(rgn_name, .after = rgn_id) %>% 
  relocate(rel_sust_tonnes, .before = old_rel_sust_tonnes)


comp_plot1 <- ggplot() +
  geom_jitter(data = sw_country, aes(x = old_rel_sust_tonnes, y = rel_sust_tonnes, text = paste("Year:", year, "<br>Region:", rgn_name)), alpha = 0.7) +
  labs(x = "Old relative sustainable tonnes",
       y = "New relative sustainable tones",
       title = "Relative sustainable tonnes: last year vs. this year") +
  # geom_abline(slope = 1, intercept = 0, col = "red") +
  theme_bw();ggplotly(comp_plot1)

# Check if there's any actual differences in the tonnes between this year and last year. If it's an empty dataframe, that's good. 
diff_sw <- sw_country %>%
  filter(tonnes != old_tonnes) %>%
  mutate(diff = tonnes - old_tonnes) %>%
  relocate(diff, .after = old_tonnes) %>%
  dplyr::arrange(diff)

comp_plot1 <- ggplot() +
  geom_jitter(data = diff_sw, aes(x = old_tonnes, y = tonnes, text = paste("Product:", product, "<br>Region:", rgn_name)), alpha = 0.7) +
  labs(x = paste("Previous Value", previous_data_year),
       y = paste("Current Value", previous_data_year),
       title = paste("Sustainable tonnes of seaweed", previous_data_year, "Data", prev_scen_year, "Assessment vs.", previous_data_year, "Data", scen_year_number, "assessment")) +
  geom_abline(slope = 1, intercept = 0, col = "red") +
...
unique(diff_sw$rgn_name)
length(unique(diff_sw$rgn_name)) # 14
length(unique(sw_country$rgn_name)) # 51

setdiff(sw_country$tonnes, sw_country$old_tonnes) # 115
setdiff(sw_country$old_tonnes, sw_country$tonnes) # 202
nrow(sw_country) # 821
