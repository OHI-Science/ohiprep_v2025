...
write_csv(tbl_seven, file.path(ex_vess_dir, "price-db-results/exvessel_price_database_1976_2023.csv"))
...
### Clean up and gapfill ex-vessel prices
...
#roughly 90% of these have a significant relationship, appears to be a reasonable gapfilling method
...
fao_capture_clean_isscaap <- fao_capture_clean %>%
  ...
  select(-c(identifier, scientific_name, major_group, isscaap)) %>%
  # Fill in missing ISSCAAP groups by scientific name
  left_join(species_key, by = c("asfis_species" = "scientific_name"), relationship = 'many-to-many') %>%
  select(-c(identifier, major_group, isscaap)) %>%
  mutate(isscaap_group = ifelse(is.na(isscaap_group.x), isscaap_group.y, isscaap_group.x)) %>%
  # Drop the extra column
  select(-c(isscaap_group.x, isscaap_group.y))
  
#combine the price data with the capture data 
fao_capture_price <- fao_capture_clean_isscaap %>% 
  # Match exvessel case for "nei"
