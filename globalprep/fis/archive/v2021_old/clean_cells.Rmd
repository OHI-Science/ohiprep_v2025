...
write.csv(cells_raw, file.path(dir_M, "git-annex/globalprep/fis/v2021/raw/watson_rasters_to_ohi_rgns.csv"), row.names=FALSE)
...
```{r}
## Check raw file
cells_raw <- read.csv(file.path(dir_M, "git-annex/globalprep/fis/v2021/raw/watson_rasters_to_ohi_rgns.csv")) %>%
  arrange(rgn_id)

old_cells_raw <- read.csv(file.path(dir_M, "git-annex/globalprep/fis/v2020/raw/watson_rasters_to_ohi_rgns.csv")) %>% 
  arrange(rgn_id)

dim(cells_raw) # 280886 rows
...
ohi_cells <- read.csv(file.path(dir_M, "git-annex/globalprep/fis/v2021/raw/watson_rasters_to_ohi_rgns.csv")) %>%
...
write.csv(ohi_cells, file.path(dir_M, "git-annex/globalprep/fis/v2021/raw/watson_rasters_to_ohi_rgns_v2.csv"), 
          row.names=FALSE)
...
ohi_cells <- read.csv(file.path(dir_M, "git-annex/globalprep/fis/v2021/raw/watson_rasters_to_ohi_rgns_v2.csv")) 
...
summary(ohi_fao_cells) # 84,218 NAs in fao_id - v2018
                       ...
                       # 758 NAs in fao_id - v2021; makes sense, since the data didn't change this year
...
old_cells <- read.csv('../v2020/int/cells.csv') %>%
  arrange(CellID) %>% 
  mutate(cell_fao_ohi = paste(CellID, fao_id, rgn_id, sep="_"))

old_2019_cells <- read.csv('../v2019/int/cells.csv') %>%
  arrange(CellID) %>%
  mutate(cell_fao_ohi = paste(CellID, fao_id, rgn_id, sep = "_"))
...
new_old <- setdiff(cells$cell_fao_ohi, old_2019_cells$cell_fao_ohi)
