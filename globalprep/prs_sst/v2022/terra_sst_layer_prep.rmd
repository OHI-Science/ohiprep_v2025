---
title: 'OHI 2021: Sea Surface Temperature Pressure Layer'
...
**NOTE: This is an incomplete workflow using `terra` in place of `raster`. In the interest of time, this effort was sidelined and the `raster` package was again used for this data prep. At some point this doc can be used as a starting point when the transition to `terra` seems more worthwhile for this particular application.**
...
dir_data <- file.path(dir_M, "git-annex/globalprep/_raw_data/CoRTAD_sst/d2022")
dir_int  <- file.path(dir_M, "git-annex/globalprep/prs_sst/v2022/int")
dir_output  <- file.path(dir_M, "git-annex/globalprep/prs_sst/v2022/output")
...
ssta_filename <- file.path(dir_M, "git-annex/globalprep/_raw_data/CoRTAD_sst/d2022/cortadv6_SSTA.nc")
ssta_res <- httr::GET(ssta, write_disk(ssta_filename))

weekly_sst <- sprintf("%s/cortadv6_WeeklySST.nc", url)
weekly_sst_filename <- file.path(dir_M, "git-annex/globalprep/_raw_data/CoRTAD_sst/d2022/cortadv6_WeeklySST.nc")
...
ssta         <- rast(ssta_filename, "SSTA")
weekly_sst   <- rast(weekly_sst_filename, "WeeklySST")

# ----
ssta_orig         <- raster::stack(list.files(dir_data, pattern = "SSTA.nc",
                                 full.names = TRUE), varname = "SSTA")
weekly_sst_orig   <- raster::stack(list.files(dir_data, pattern = "WeeklySST.nc",
                                 full.names = TRUE), varname = "WeeklySST")
names_ssta_orig <- names(ssta_orig)
names_weekly_orig <- names(weekly_sst_orig)

# ----
names_ssta <- names(ssta)
dates_ssta <- time(ssta)
names_weekly <- names(weekly_sst)
dates_weekly <- time(weekly_sst)

ssta_df <- tibble(name = names_ssta,
                  date = dates_ssta) %>% 
...
# ----
ssta_df_orig <- names_ssta_orig %>% # View(ssta_df)
  data.frame() %>% 
  rename(name = ".") %>% 
...
sup_info <- "~/github/ohiprep_v2022/globalprep/supplementary_information/v2022" # should reflect assessment year
...
df_tmp <- data.frame(v2021 = getValues(sst_v2022_test), 
                     v2018 = getValues(sst_v2018_test))
df_samp <- df_tmp %>% mutate(rowcol = rownames(.)) %>% 
  filter(rowcol %in% sample(1:length(rowcol), 70000))
plot(df_samp$v2018, df_samp$v2022, "n")
points(cbind(df_samp$v2018, df_samp$v2022), 
...
p <- ggplot(combine, aes(x = old_prs_score, y = prs_score, label = rgn_id)) +
  ...
  theme_bw() + 
  geom_abline(intercept = 0, slope = 1) + 
  labs(title = "SST comparison v2022 and v2018 \n (data year 2017)")
ggplotly(p)
