...
The new v4 API has been introduced and the necessary revisions have been made.
...
for (i in countries$code[174:250]) {
...
country_assessments <- bind_rows(country_assessments))  %>%
    filter(latest == TRUE) %>%
    unnest(scopes, name_sep = "_") %>%
    filter(description$en == "Global") %>%
    left_join(countries, by = "code") 

write_csv(country_assessments, file.path(dir_data, 'v2025', 'iucn_countries_api.csv'))

setdiff(countries$code, unique(country_assessments_filtered$code))

country_assessments_filtered <- country_assessments %>%
   filter(latest == TRUE) %>%
    unnest(scopes, names_sep = "_") %>%
    filter(scopes_description$en == "Global") %>%
    left_join(countries, by = "code") %>%
  select(iucn_sid = sis_taxon_id, year = year_published, possibly_extinct, possibly_extinct_in_the_wild, sciname = taxon_scientific_name, cat = red_list_category_code, code, country = description)
...
assessment <- result$assessments %>%
  ...
  left_join(cat_lookup, by = "code") %>%
  group_by(iucn_sid) %>%
  mutate(assess_year = year[latest == TRUE][1]) %>%
  ungroup() %>%
  select(iucn_sid, year, assess_year, code = code_current, category, sciname) %>%
  distinct()
