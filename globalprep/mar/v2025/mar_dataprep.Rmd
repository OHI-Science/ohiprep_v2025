...
# Before changing region splitting
## 2025 assessment: 8928 out of 44598 (~20%) cases had NA values converted to 0

# After changing mar_fxs region splitting...
## 2025 assessment: 11101 out of 47619 (~23%) cases had NA values converted to 0
```
...
int_sfw_24 <- read_csv(here(previous_mar_dir, "int", "int_sfw.csv")) %>% 
  select(escape24 = escapes_score, score24 = score, species, rgn_id, rgn_name, scientific_name, sw_species, fao_species, fda_species, country, water_body, method)

sw_sus_comb <- full_join(sw_sus_rgn, int_sfw_24, by = join_by(rgn_id, species)) %>% 
  view()
```
...
maric_old <- readr::read_csv(here::here(previous_mar_dir, "output", "MAR_FP_data.csv"))
...
# compare species between v2024 and v2025
sort(setdiff(maric$species, maric_old$species)) # 2025 not in 2024
sort(setdiff(maric_old$species, maric$species)) # 2024 not in 2025

# Join previous and current year data
maric_comb <- full_join(maric, maric_old, by = join_by(rgn_id, species, year, environment, family))

# Clean names
maric_comb <- maric_comb %>% 
  select(rgn_id, species, year, environment, family, fao25 = fao.x, 
         fao24 = fao.y, value_25 = value.x, taxon_code25 = Taxon_code.x, 
         taxon_code24 = Taxon_code.y, gap_0_fill25 = gap_0_fill.x, 
         gap_0_fill24 = gap_0_fill.y, vyear_25 = vyear.x, 
         vyear_24 = vyear.y, value_24 = value.y)

# Check NAs for gap filling
colSums(is.na(maric_comb))

# Gap fill with other year data
maric_comb_clean <- maric_comb %>%
  mutate(value_24 = ifelse(is.na(value_24), value_25, value_24),
         value_25 = ifelse(is.na(value_25), value_24, value_25),
         vyear_25 = ifelse(is.na(vyear_25), 2025, vyear_25),
         vyear_24 = ifelse(is.na(vyear_24), 2024, vyear_24),
         taxon_code25 = ifelse(is.na(taxon_code25), taxon_code24, taxon_code25),
         taxon_code24 = ifelse(is.na(taxon_code24), taxon_code25, taxon_code24),
         fao25 = ifelse(is.na(fao25), fao24, fao25),
         fao24 = ifelse(is.na(fao24), fao25, fao24),
         gap_0_fill25 = ifelse(is.na(gap_0_fill25), gap_0_fill24, gap_0_fill25),
         gap_0_fill24 = ifelse(is.na(gap_0_fill24), gap_0_fill25, gap_0_fill24))

# Confirm NAs are gone for columns of interest
colSums(is.na(maric_comb_clean))

maric_clean <- maric_comb_clean %>% 
  select(rgn_id, species, year, fao = fao25, environment, value_24, value_25, taxon_code = taxon_code25, family, gap_0_fill = gap_0_fill25, vyear_24, vyear_25)

maric_long <- maric_clean %>%
  pivot_longer(
    cols = c(value_24, value_25, vyear_24, vyear_25),
...
maric_long <- maric_clean %>% 
  pivot_longer(c(value_24, value_25), 
               names_to = "type", 
               values_to = "value") 
