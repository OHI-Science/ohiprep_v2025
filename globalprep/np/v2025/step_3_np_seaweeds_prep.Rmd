...
mar_rgn2 <- mar_rgn %>%
  ...
  summarize(tonnes_mar = sum(tonnes_nonfood))  %>%
...
fao <- read_csv(here(sprintf("globalprep/np/v%s/int/np_usd_per_tonne.csv", current_year))) %>%
  ...
  rename(trade_tonnes = tonnes)

full_data <- full_join(mar_rgn2, fao, by=c("rgn_id", "year")) %>%
  mutate(
    tonnes = pmax(tonnes_mar, trade_tonnes, na.rm = TRUE),
    tonnes = replace(tonnes, is.infinite(tonnes), NA_real_)) 

plot(log(full_data$trade_tonnes+1), log(full_data$tonnes_mar+1))
abline(0,1, col="red")
