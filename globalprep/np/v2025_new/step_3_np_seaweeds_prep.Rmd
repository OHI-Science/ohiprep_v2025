...
old_rel <- read_csv(here("globalprep", "np", paste0(prev_v_scen_year, "_new"), "output", "np_relative_sust_tonnes_seaweeds.csv"))
old_harvest <- read_csv(here("globalprep", "np", paste0(prev_v_scen_year, "_new"), "output", "np_seaweed_harvest_tonnes_gf.csv"))
old_sust <- read_csv(here("globalprep", "np", paste0(prev_v_scen_year, "_new"), "output", "np_seaweed_sust_gf.csv"))

# Read in current assessment data
new_rel <- read_csv(here("globalprep", "np", paste0(v_scen_year, "_new"), "output", "np_relative_sust_tonnes_seaweeds.csv"))
new_harvest <- read_csv(here("globalprep", "np", paste0(v_scen_year, "_new"), "output", "np_seaweed_harvest_tonnes_gf.csv"))
new_sust <- read_csv(here("globalprep", "np", paste0(v_scen_year, "_new"), "output", "np_seaweed_sust_gf.csv"))
...
old_rel1 <- old_rel %>% 
  ...
  dplyr::select(rgn_id, product, old_rel_sust_tonnes = rel_sust_tonnes)

new_rel1 <- new_rel %>% 
  filter(year == previous_data_year) %>% 
  dplyr::select(rgn_id, product, rel_sust_tonnes)


rel_total <- old_rel1 %>% 
  full_join(new_rel1, by = c("rgn_id", "product"))

comp_plot1 <- ggplot() + 
  geom_jitter(data = rel_total, aes(x = old_rel_sust_tonnes, y = rel_sust_tonnes, text = paste("Product:", product, "<br>Region:", rgn_id)), alpha = 0.7) +
