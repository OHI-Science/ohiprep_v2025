...
sp2grp <- read_csv(here('globalprep', 'prs_targetedharvest', paste0('v', version_year, "_a"), 'raw', 'species2group.csv')) %>%
...
fao_clean <- fis_fao_raw %>% 
  ...
  dplyr::select(- c("Unit (Name)",
                "Unit")) %>% #added removal of Unit column
...
target_spp <- fao_clean %>%
  dplyr::filter(species %in% sp2grp$species) # this goes from 3415 spp in FAO list to just 72

unique(target_spp$area) # confirm these are all marine regions
unique(fao_clean$species) # 3415 species
...
Data is rescaled by dividing by the 95th quantile of values across all regions from 2011 to 2021 (most recent year of FAO data).
...
# Add year; for v2022, min year is 1950, and max year is 2021 
...
write_csv(
  target_harvest, 
  here('globalprep', 'prs_targetedharvest', paste0('v', version_year, "_a"), 'output', 'fao_targeted.csv')
)
...
write_csv(
  target_harvest_gf,
  here('globalprep', 'prs_targetedharvest', paste0('v', version_year, "_a"), 'output', 'fao_targeted_gf.csv')
)
```
...
# pull just 20120 data from target_harvest df, since its the most recent year in common
common_year <- 2020

new <- readr::read_csv(here("globalprep", "prs_targetedharvest", paste0('v', version_year, "_a"), "output", "fao_targeted.csv")) %>% 
