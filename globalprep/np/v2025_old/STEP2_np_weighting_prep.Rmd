...
readr::write_csv(harvest_tonnes_usd_values_gf,
                 here(current_np_dir, "int", "old_harvest_tonnes_usd_weighting_gf.csv"))
...
readr::write_csv(weighting_usd_values_final,
                 here(current_np_dir, "int", "old_harvest_weighting_values.csv"))
...
```{r}
harvest_weighting_values <- read_csv(here(current_np_dir, "int", "old_harvest_weighting_values.csv")) %>%
...
readr::write_csv(prod_weights, here(current_np_dir, "output", "old_np_product_weights.csv"))
...
old_prod_weights <- read_csv(here(previous_np_dir, "output", "np_product_weights.csv"))
prod_weights <- read_csv(here(current_np_dir, "output", "old_np_product_weights.csv"))
...
g_new1 <- ggplot(check, aes(x = weight, y = new_weight, color = product,
  ...
  scale_color_manual(values = c("fish_oil" = "#edae49",
                                "seaweeds" = "#386641",
                                "ornamentals" = "#83c5be")) +
  labs(title = paste("OLD", v_scen_year, "vs", v_prev_year, "Natural Product Weights by Product"),
...
# ---- Convert to plotly ------------------------------ #
p_new1 <- ggplotly(g_new1, tooltip = "text") %>%
  layout(
    hovermode = "closest",
...
p_new1$x$data <- lapply(p_new1$x$data, function(trace) {
