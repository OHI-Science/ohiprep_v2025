---
title: "OHI 2020: Food Provision, Investigate Differences"
...
RAM4.491_meta = as.data.frame(metadata)
RAM4.491_data = as.data.frame(timeseries)

## Check length in metadata
length(unique(RAM4.491_meta$assessid)) #1330 unique entries
length(unique(RAM4.491_meta$stockid)) # same number of stockids as assessids
length(unique(RAM4.491_meta$stocklong)) # same number of stocklong as the above two


## Check length in timeseries data
length(unique(RAM4.491_data$assessid)) # 2215 unique entries, more than in metadata
length(unique(RAM4.491_data$stockid)) # 1371 unique, more than in metadata..
length(unique(RAM4.491_data$stocklong)) #1373 unique

setdiff(RAM4.491_data$assessid, RAM4.491_meta$assessid) #886 diff in data not in meta
setdiff(RAM4.491_data$stockid, RAM4.491_meta$stockid) #44 stockids in data not in meta
...
## Check length
length(unique(RAM4.44_meta$assessid)) # 1330
length(unique(RAM4.44_meta$stockid)) # 1330
length(unique(RAM4.44_meta$assessid)) # 1330

length(unique(RAM4.44_data$assessid)) # 2197
length(unique(RAM4.44_data$stockid)) # 1371
length(unique(RAM4.44_data$stocklong)) # 1373

setdiff(RAM4.44_data$assessid, RAM4.44_meta$assessid) #868
setdiff(RAM4.44_data$stockid, RAM4.44_meta$stockid) #42
...
length(unique(ram_bmsy$assessid)) # 499 unique entries
length(unique(ram_bmsy$stockid)) # 360
length(unique(ram_bmsy$stocklong)) # 360

setdiff(ram_bmsy$assessid, RAM4.491_meta$assessid) # 177 diffs
setdiff(ram_bmsy$stockid, RAM4.491_meta$stockid) # 9 diffs
...
ram_bmsy_old <- data.frame(RAM4.44_data) %>%
  dplyr::filter(tsid == "BdivBmsytouse-dimensionless") %>%
  dplyr::filter(!is.na(tsvalue)) %>%
  dplyr::mutate(tsyear = as.numeric(as.character(tsyear))) %>%
  dplyr::filter(tsyear > 1979) %>%
...
## Compare stockid in raw_meta for v4.491 RAM data versus stockid in timeseries.. should match
setdiff(ram_bmsy$stockid,RAM4.491_meta$stockid)
...
# Investigate Multiple `assessid` in RAM v4.491 Database
...
ANGLVIIIc-IXa has 7 assessments, take a look
```{r}
test2 <- ram_bmsy %>% 
