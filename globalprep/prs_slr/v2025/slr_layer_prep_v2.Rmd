...
- For v2025, the full data year for 2023 was not included at the time of assessment, and therefore the data remains the same from v2024. Just to be sure, the data was downloaded again and the prs_slr recalculated.

# Update from 2025 assessment:
While there was new data added by v2025, there was not a full data year present, and thus slr for 2023 could not be calculated. To ensure previous data years had no been edited, this layer was recalculated using the following script.
***
...
scen_year <- 2025 # change to reflect assessment year!
...
year <- 2022 # data year to download; lags by 2 years. 2023 goes to May as of now, and we need a full year's worth of data for v2025
...
```{r, eval= FALSE}
# Define the new username and password. NOTE: The username and password of the last year will not be saved here, but replace it with your own each version year
h <- new_handle()
handle_setopt(h, username = "", password = "")

for (mo in months) {
  u <- sprintf("%s/dt_global_allsat_msla_h_y2022_m%s.nc", ub, mo)
  u_filename <- file.path(dir_anx_aviso, paste0("d", scen_year), substr(u, 121, 157))
  curl_download(u, destfile = u_filename, handle = h)
}
```
...
nc_files <- c(list.files(file.path(dir_anx_aviso, "d2025"),
                       full.names = TRUE, pattern = ".nc"),
              list.files(file.path(dir_anx_aviso, "d2024"),
                       full.names = TRUE, pattern = ".nc"),
...
```{r msla_monthly_to_annual, eval= FALSE}
...
write.csv(ref_tab,
          here(dir_refpt, "v2025", "reference_points_pressures.csv"),
          row.names = FALSE)

## grab reference value from the supp_info csv
ref <- read_csv(here(dir_refpt, "v2025", "reference_points_pressures.csv")) %>%
...
for (i in seq_along(coastal_rasts)) {
  yr <- stringr::str_sub(coastal_rasts[i], -8, -5)
  out_file <- sprintf("%s/output/slr_%s.tif", dir_prs_slr, yr)
  ...
  r <- try(terra::rast(coastal_rasts[i]), silent = TRUE)
  
  if (inherits(r, "try-error")) {
    message("Failed to read raster: ", coastal_rasts[i])
    next
  }

  r2 <- terra::app(r, fun = neg_vals)
  terra::app(r2, fun = greater_vals, filename = out_file, overwrite = TRUE)
}

toc()
# v2025 - 4259.81 sec elapsed
```
...
terra::hist(r_new_terra, main = sprintf("Sea Level Pressure %s current version (v%s) data", scen_year - 2, scen_year), breaks = 70) # v2025 - plots v2025 2022 data
terra::hist(r_new_old_terra, main = sprintf("Sea Level Pressure %s current version (v%s) data", scen_year - 3, scen_year), breaks = 70) # v2025 - plots v2024 2022 data
terra::hist(r_old_terra, main = sprintf("Sea Level Pressure %s previous version (v%s) data", scen_year - 3, scen_year - 1), breaks = 70) # v2025 - plots v2023 2021 data
```
...
write.csv(regions_stats_clean, here("globalprep", "prs_slr", "v2025", "output", "slr.csv"), row.names = FALSE)

regions_stats_clean <- read_csv(here("globalprep", "prs_slr", "v2025", "output", "slr.csv"))
...
new_data <- read_csv(here("globalprep", "prs_slr", "v2025", "output", "slr.csv")) %>%
...
write.csv(mjr_score_change, here("globalprep", "prs_slr", "v2025", "output", "major_changes_2025.csv"))
```

```{r score_change_plots, eval=FALSE}
# NOTE: This does not need to change to 2025, as we have seen that the data is the same. In subsequent years, when full 2023 data is added, this should be changed
...
bar_plot_mjr_change <- ggplot(mjr_score_change, aes(x = factor(rgn_id), y = diff)) +
  ...
  labs(x = 'Region ID',
       ...
       title = 'Major Score changes between New (v2025) and Old (v2024) Pressure Scores') +
...
```{r, eval= FALSE}
# refer back to line 503 for the rasterized output for current year and last year. v2025 update
...
```{r, eval=FALSE}
slr_gf <- read.csv(here("globalprep", "prs_slr", "v2025", "output", "slr.csv"))%>%
  mutate(pressure_score = 0) %>% 
  rename(gapfilled = pressure_score)

write.csv(slr_gf, here("globalprep", "prs_slr", "v2025", "output", "slr_gf.csv"), row.names=FALSE)
```
