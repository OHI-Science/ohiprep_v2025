---
title: "OHI 2021: Food Provision, Preparing catch data for BBmsy calculations and mean catch (weights in FIS model) and FP weights"
...
This script takes the SAUP 2021 catch data, provided at a resolution of per SAUP region/year/species totals, and creates 3 data layers:
...
v2021: Added new SAUP fisheries data to include year 2018 and updated the RAM data.

NOTE: for next year (v2022), consider using the FOFM identifier in the SAUP data, instead of what we currently do to quantify the FOFM use. In v2021, the FOFM identifier in the SAUP did not seem correct. 
...
## Paths for data
fis_path = file.path(dir_M,"git-annex/globalprep/fis/v2021/int") 
SAUP_d2021 <- file.path(dir_M, "git-annex/globalprep/fis/v2021")
...
```{r, eval = FALSE}
fao_eez_prod <- read.csv(file.path(SAUP_d2021, "eez_fao_ids_prod.csv"))
fao_high_seas <- read.csv(file.path(SAUP_d2021, "high_seas_fao_ids_prod.csv"))
...
setdiff(high_seas_regions$area_name_ohi, high_seas_ohi$rgn_name) # "Atlantic, Antarctic" "Atlantic, Western Central" "Indian Ocean, Antarctic"   "Pacific, Antarctic"    
...
sum(high_seas_ohi$tonnes) # 121796975
sum(fao_high_seas$tonnes) # 121796975 ; good they are the same 


#save file 
write.csv(high_seas_ohi, file.path(fis_path, "ohi_rgn_high_seas_prod.csv"), row.names = FALSE)

 # test <- read.csv(file.path(fis_path, "ohi_rgn_high_seas_prod.csv"))

#### Now match the EEZ production data to OHI regions using the lookup table 

fao_eez_prod <- read_csv(file.path(SAUP_d2021, "eez_fao_ids_prod.csv")) %>%
...
# so we should have 6947*2 + 106098*2 + 5689*3 = 243157 observations after splitting; We should also retain the same amount of tonnage 12856.2 + 901012.5 + 336143.2 = 1250012 tonnes after splitting
...
tonnes, fao_id) %>%
  ...
  ungroup()
sum(split_saba_sint$tonnes) # 12856.2 ; perfect
...
tonnes, fao_id) %>%
  ...
  ungroup()
sum(split_channel$tonnes) # 901012.5 ; perfect 
...
tonnes, fao_id) %>%
  ...
  ungroup()
sum(split_channel_moz$tonnes) # 336143.2 ; perfect 
...
write.csv(output_df, file = file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn.csv'), row.names=FALSE)
...
# watson_output_df <- read.csv(file.path(dir_M,'git-annex/globalprep/fis/v2021_old/int/stock_catch_by_rgn.csv')) %>%
#   filter(rgn_id < 255) 
# 
# test_watson <- watson_output_df %>%
#   group_by(year) %>%
#   summarise(sum = sum(tons))
# 
# unique(watson_output_df$year)
# unique(watson_output_df$rgn_id)
# unique(watson_output_df$fao_rgn)
# sort(unique(watson_output_df$stock_id))

# test_watson_2017 <- watson_output_df %>%
#   filter(year == 2017) 
# sum(test_watson_2017$tons) # 78378415
# 
# test_saup_2017 <- output_df %>%
#   filter(year == 2017) %>%
#   filter(rgn_id < 255)
# sum(test_saup_2017$tons) # 104662997 - getting a lot more tons than watson... There just seems to be more tonnes in the SAUP data I guess. 
# 
# test <- all_saup_ohi_prod %>%
#   filter(year == 2017) %>%
#   filter(rgn_id < 255)
# sum(test$tonnes) # 104662997
# 
# setdiff(test_saup_2017$stock_id, test_watson_2017$stock_id)
# setdiff(test_watson_2017$stock_id, test_saup_2017$stock_id) # lots of stock_id differences. Expected given the new data source.
...
stock_rgn <- read.csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn.csv'))

## First lets grab the taxon keys from the 2016 SAUP data that was shared with OHI way back when, these will probably best matched the taxa included in their newer data
...
dt_taxon           <- fread(file.path(path_data,file_taxon), sep=";", header = FALSE) %>%
  dplyr::rename(TaxonKey = V1, TaxonName = V2, CommonName = V3) %>%
  dplyr::select(-V4)


## read in watson TaxonKey data and merge with the SAUP TaxonKey data
...
taxon_key_SAUP_watson <- taxonkey %>%
  dplyr::select(TaxonKey = Taxonkey, TaxonName, CommonName) %>%
  rbind(dt_taxon) %>%
  distinct(TaxonKey, TaxonName, CommonName)

duplicated_taxa <- data.frame(cbind(taxon_key_SAUP_watson, dups = duplicated(taxon_key_SAUP_watson$TaxonKey))) %>%
  filter(dups == TRUE) # still some TaxonKey duplicates. We need to keep the SAUP taxon keys that are duplicated, not the watson ones...

taxonkey_fix <- taxonkey %>% 
  filter(!(Taxonkey %in% c(duplicated_taxa$TaxonKey)))

taxon_key_SAUP_watson <- taxonkey_fix %>%
  dplyr::select(TaxonKey = Taxonkey, TaxonName, CommonName) %>%
  rbind(dt_taxon) %>%
  distinct(TaxonKey, TaxonName, CommonName)
...
new_taxa <- stock_rgn %>% 
  ...
  dplyr::select(TaxonName, CommonName, TaxonKey) %>%
  unique() 

taxonkey <- rbind(taxon_key_SAUP_watson, new_taxa) 
...
stock_rgn <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn.csv'))
...
write.csv(taxonkey_no_dups, "int/taxon_key_v2021.csv", row.names=FALSE)
```
...
taxonkey <- read.csv("int/taxon_key_v2021.csv", stringsAsFactors = FALSE)
stock_rgn <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn.csv'))
...
setdiff(paste(taxonkey$TaxonName, taxonkey_no_dups$CommonName), 
        paste(stock_rgn$TaxonName, stock_rgn$CommonName)) # lots of diffs... that is ok. These are probably left-over stocks and taxonkeys from former versions of OHI that are no longer represented in the fisheries data.
...
write.csv(stock_rgn_taxa, file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn_taxa.csv'), row.names=FALSE)
...
sum(catch$tons) # 5308104214
5308104214/6124187447 # makes sense.
...
taxon_key_info <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn_taxa.csv'))
...
sum(catch_zeros$catch_human, na.rm = TRUE) # 3388041152
sum(catch_zeros$catch_fish_feed, na.rm = TRUE) # 1920063062

1920063062/(3388041152 + 1920063062) # 0.3617229; seems right. This states that forage fish account for more than a third of fish catch every year: https://www.pewtrusts.org/-/media/assets/2013/pffforagefishfaq.pdf
...
Compare v2021 with last year v2020 (should be a lot different since we are using a new datasource)
```{r, eval=FALSE}
...
old <- read.csv("../v2020/output/FP_fis_catch.csv")
old_filt <- old %>% 
  #filter(stock_id_taxonkey == "Carangidae-31_400314") %>% 
  rename(year = year) %>% 
  filter(year == 2014) %>% 
...
plot_check <- ggplot(check, aes(old_fis_catch, new_fis_catch, col = rgn_id)) +
  geom_point(alpha = 0.4) +
  geom_abline(col="red") +
  ggtitle("Food Provision Comparison for 2014 (v2020, v2021)")
