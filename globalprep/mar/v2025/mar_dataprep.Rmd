...
# Okay, so there's always at least 3 duplicates as of rn. I'm assuming they will be dropped later, but wanted to track before moving on 
sfw_int_old %>%
...
# Get rid of the duplicates?
  ...
  #   fao = first(fao),
  #   taxon_code = first(taxon_code),
  #   .groups = "drop"
  # )

# Filter down before combining
sfw_int_old <- sfw_int_old %>% 
  select(escape_score_old = escapes_score, score_old = score, species, rgn_id, rgn_name, scientific_name, sw_species, fao_species, water_body, method, rec, country)

# Trying to determine what I need to join on to avoid duplicates (i.e. what is the primary key)
sfw_int_old %>%
  count(rgn_id, species, method, water_body, rec) %>%
  filter(n > 1)
colnames(sfw_int_old)
...
colnames(sw_sus_rgn_comb)

# Clean names
sw_sus_rgn_comb <- sw_sus_rgn_comb %>% 
  select(rgn_id, species, score, method, rec, water_body, score_old, escapes_score, escape_score_old, scientific_name_new = scientific_name.x, scientific_name_old = scientific_name.y, rgn_name_new = rgn_name.x, rgn_name_old = rgn_name.y, vyear_new = vyear.x, vyear_old = vyear.y, country_new = country.x, country_old = country.y)
...
sw_sus_rgn_clean <- sw_sus_rgn_comb %>%
  mutate(
    ...
    score = ifelse(is.na(score), score_old, score),
    score_old = ifelse(is.na(score_old), score, score_old),
    # Escapes score
    escapes_score = ifelse(is.na(escapes_score), escape_score_old, escapes_score),
    escape_score_old = ifelse(is.na(escape_score_old), escapes_score, escape_score_old),
    # Version year
    vyear_new = ifelse(is.na(vyear_new), current_year, vyear_new),
    vyear_old = ifelse(is.na(vyear_old), previous_year, vyear_old),
    # Scientific name
...
# maric %>% 
#   filter(rgn_id == 164 & species == "Seaweed") %>% 
#   view()
...
length(unique(maric_join$species_code)) # 1367
nrow(identifier) # 1367
...
nrow(test3) # 1810
sum(is.na(test3$Sust_sw_sp_c)) # 1810
# All sust_sw_sp_c and esc_sw_sp_c scores are NA for this test...

# Checking for the entire data frame now
nrow(mar_sw_sus) # 58086
sum(is.na(mar_sw_sus$Sust_sw_sp_c)) # 49506
sum(is.na(mar_sw_sus$esc_sw_sp_c)) # 49506
