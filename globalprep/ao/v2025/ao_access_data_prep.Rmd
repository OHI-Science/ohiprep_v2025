...
## New method of comparing country names using countrycode() function 
...
setdiff(rgns_eez$rgn_name, region_data$geo_area_name)
```

```{r}
## Find the differences between api and downloaded UN regions
UN_not_in_api <- setdiff(region_info$country_name, clean_data$country_name)
```

```{r}
## Find the differences between OHI regions and UN regions (api)
OHI_not_in_api <- setdiff(rgns_eez$rgn_name, clean_data$country_name)
```

```{r}
OHI_not_in_UN <- setdiff(OHI_not_in_api, UN_not_in_api)
```
...
compare_diff <- compare %>% 
  ...
  dplyr::select(rgn_id, rgn_name, admin_country_name, year, value_v2023, value_v2025, difference)
