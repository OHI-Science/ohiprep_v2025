---
title: 'OHI 2024 - Targeted harvest pressure'
...
[REFERENCE RMD FILE](http://ohi-science.org/ohiprep_v2024/globalprep/prs_targetedharvest/v2024/targetharvest_dataprep.html)

# Summary

This analysis converts FAO capture production data into the OHI 2025 targeted harvest pressure data.

# Updates from previous assessment

2023 - One more year of data, 2021 Small fixes to how N data is handled in fao_fxn.R Delete unused/not relevant code

2024- Data until 2022 so one more year of data. Changes: removed more columns when making fao_clean in the 'Clean the FAO Data' chunk that were interfering with the merge.

2025- Data until 2023 so one more year of data. Column names have changed. Updated "nei" to "NEI" for species in raw/species2group.csv. 
...
FAO Global Capture Production Quantity 1950_2023\
Information:  <https://www.fao.org/fishery/en/collection/capture?lang=en>

**Reference**:

FAO. 2025. Global Capture Production. In: Fisheries and Aquaculture. [Cited Friday, August 8th 2025]. 
https://www.fao.org/fishery/en/collection/capture

**Downloaded**: August 7, 2025
...
Data must be downloaded from FishStatJ to obtain the correct formatting. 
...
fao_clean <- fis_fao_raw %>% 
  ...
  dplyr::select(-c("Unit (Name)", "Unit", contains("Flag"), contains("S..."))) %>% #  added removing Unit also
...
#not sure if this is necessary still but I ran it and kept it in just in case-v2024 (sMe v2025)
fao_clean <- fao_clean %>%
  dplyr::mutate(
...
#     Marine turtles NEI ### adjusted "nei" to NEI in .csv
#     River and lake turtles NEI  - not a marine turtle
# 
# MISSING in the lookup the following species in target='whale'.
#     Baleen whales NEI ### adjusted "nei" to NEI in .csv
#     Beaked whales NEI ### adjusted "nei" to NEI in .csv
#     Creek whaler - shark, not a whale
#     Pilot whales NEI ### adjusted "nei" to NEI in .csv
#     Toothed whales NEI ### adjusted "nei" to NEI in .csv
...
#     Spotted dolphins NEI ### adjusted "nei" to NEI in .csv
...
unique(fao_clean$species) #v2025: 3723 species, v2024: 3398 species
unique(target_spp$species) #v2025: 70 species, v2024: 72 species
```
...
Data is rescaled by dividing by the 95th quantile of values across all regions from 2011 to 2023 (most recent year of FAO data).
...
# Add year; for v2025, min year is 1950, and max year is 2023
...
# pull just 2022 data from target_harvest df, since its the most recent year in common
common_year <- 2022
...
compare_plot <- ggplot(data = old, aes(x = pressure_score_old, y = pressure_score, label = rgn_id)) +
  ...
  labs(y = paste0("Pressure Score ", version_year),
       x = paste0("Pressure score v", version_year-1)) +
  theme_minimal()
...
### v2024 outliers: Russia[73]
#out_country <- "Russian Federation"
...
old <- readr::read_csv(here("globalprep", "prs_targetedharvest", paste0('v', version_year - 1), "output", "fao_targeted.csv")) %>%
  dplyr::filter(year == common_year) %>%
  dplyr::select(-year) %>%
  dplyr::select(rgn_id, pressure_score_2022 = pressure_score) %>%
  dplyr::left_join(new, by = c("rgn_id")) %>%
  dplyr::rename(pressure_score_2023 = pressure_score) %>%
  left_join(rgns_eez)
  

# Compare pressure_score between last year and this year's assessments
compare_plot <- ggplot(data = old, aes(x = pressure_score_2022, y = pressure_score_2023, label = rgn_name)) +
  geom_point() +
  geom_abline(color = "red")
