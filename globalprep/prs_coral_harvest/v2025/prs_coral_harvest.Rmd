...
# Summary
This analysis converts FAO commodities data into data layers used to calculate OHI 2025 global coral harvest pressure.

## 2025 Updates from previous assessment
- New year of FAO data (1976-2023; new year 2023)
...
App release date: June 2025
FAO raw commodities quantity 1976_2023
FAO raw commodities value 1976_2023
...
## Calculate pressure score
 ...
 - Anything above the 95th quantile receives a pressure score of 1, otherwise what it was before. 
...
# Calculate reference point from years 2010-2014
coral_harvest_ref <- coral_harvest %>%
  filter(year %in% c(2010:2014))

# Find the 95th quantile for a reference point
ref <- quantile(coral_harvest_ref$intensity, probs = 0.95, na.rm = TRUE) 
...
```{r}
# Read in production harvest data 
roll_prod_24 <- readr::read_csv("/home/hoyt/OHI/ohiprep_v2025/globalprep/prs_coral_harvest/v2024/int/tonnes_coral_harvest.csv")

# check grouping status
#is.grouped_df(roll_prod)

# Read in coral extent data
coral_ext_24 <- readr::read_csv(here::here(
  "globalprep", "hab_coral",
# paste0("v", (as.numeric(scen_year) - 2)), # error in v2024: most recent update to this folder is 2021
  "v2021", # temporary hard-coded fix (UPDATE TO MOST RECENT VERSION!!)
  "data", "habitat_extent_coral_updated.csv")) %>% 
...
# coral_health <- readr::read_csv(paste0("globalprep/hab_coral/v", (as.numeric(scen_year) - 2), "/data/habitat_health_coral_updated.csv")) %>% # UPDATE TO MOST RECENT VERSION
#   dplyr::select(-habitat, -year)
coral_health_24 <- readr::read_csv(here::here(
  "globalprep", "hab_coral",
  # paste0("v", (as.numeric(scen_year) - 2)), # error in v2024: most recent update to this folder is 2021... 
  "v2021", # temporary hard-coded fix (UPDATE TO MOST RECENT VERSION!!)
  "data", "habitat_health_coral_updated.csv")) %>% 
...
# Join coral harvest data with the coral health and extent
coral_harvest_join_24 <- roll_prod_24 %>%
  left_join(coral_ext, by = "rgn_id") %>%
  left_join(coral_health, by = "rgn_id")

# Calculate harvest intensity 
coral_harvest_24 <- coral_harvest_join_24 %>%
  mutate(intensity = tonnes/km2)

# Find the 95th quantile for a reference point
ref_24 <- quantile(coral_harvest_24$intensity, probs = 0.95, na.rm = TRUE) 


ggplot(coral_harvest_24, aes(intensity)) +
  geom_histogram()

ggplot(coral_harvest, aes(intensity)) +
  geom_histogram()
```
...
ggplot(combined_coral, aes(old_prs, new_prs)) +
  geom_point()
...
# v2025: 49 (1995-2012), 220 (2000-2022), 245 (2000-2022), 248 (2000-2022), 249 (2000-2022), 250 (2000-2019)
...
# explore some deviations v2024 & v2025
previous_version_coral_latest_yr <- previous_version_coral %>%
  filter(year == common_year) # year 2022 in v2025
...
combined_latest_yr <- previous_version_coral_latest_yr %>%
  ...
  dplyr::select(-year.x, -year.y) # 2021 and 2024 in v2025
...
plot(combined_coral_common_yr$old_prs, combined_coral_common_yr$new_prs,
     xlab = paste0("Old Pressure Data Year ", (as.numeric(prev_scen_year) - 2), " v", prev_scen_year),
     ylab = paste0("New Pressure Data Year ", (as.numeric(scen_year) - 3), " v", scen_year)) +
abline(0,1, col = "red")
