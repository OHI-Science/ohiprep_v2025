...
[REFERENCE RMD FILE: http://ohi-science.org/ohiprep_v2022/globalprep/prs_coral_harvest/v2022/prs_coral_harvest.html]

# Summary
This analysis converts FAO commodities data into data layers used to calculate OHI 2022 global coral harvest pressure.

# Updates from previous assessment
New year of FAO data (1976-2019).  
...
h_tonnes <- read.csv(here(paste0('globalprep/np/v', scen_year, '/int/tonnes.csv')))

## Read in value dataset from intermediate folder
h_usd <- read.csv(here(paste0('globalprep/np/v', scen_year, '/int/usd.csv')))
...
coral_ext <- read_csv(paste0("globalprep/hab_coral/v", scen_year, "/data/habitat_extent_coral_updated.csv")) %>%
  dplyr::select(-habitat, -year) %>%
  filter(km2 != 0)

## read in coral health data 
coral_health <- read_csv(paste0("globalprep/hab_coral/v", scen_year, "/data/habitat_health_coral_updated.csv")) %>%
  dplyr::select(-habitat, -year)
...
```{r}
new_coral <- read_csv(paste0("globalprep/prs_coral_harvest/v", scen_year, "/output/prs_coral_harvest.csv"))

old_coral <- read_csv(paste0("globalprep/prs_coral_harvest/v", (as.numeric(scen_year) - 1), "/output/prs_coral_harvest.csv")) %>%
