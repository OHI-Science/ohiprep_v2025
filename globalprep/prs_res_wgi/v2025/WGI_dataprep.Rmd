...
# Extract data for the 6 broad indicators
ind_dict <- list(
  "key_va" = "Voice and Accountability: Estimate",
  "key_pv" = "Political Stability and Absence of Violence/Terrorism: Estimate",
  "key_ge" = "Government Effectiveness: Estimate",
  "key_rq" = "Regulatory Quality: Estimate",
  "key_rl" = "Rule of Law: Estimate",
  "key_cc" = "Control of Corruption: Estimate"
)
...
# Loop correctly over names
for (key in names(ind_dict)) {
  
  value <- ind_dict[[key]]
  
  # Get the indicator code by searching the name
  indicator_code <- WDIsearch(value, field = 'name')[1, 'indicator']
  
  # Download the data
  indicator_data[[key]] <- WDI(
...
d_api <- read_csv(here(current_wgi_dir, "raw", "worldbank_wgi_from_api_2025.csv"))
colnames(d)
```

```{r}
# Comparing the API data and the web download data
colSums(is.na(d)) # 117-231 NAs across all 6 inds
nrow(d) # 530
# ~ 38% NA
view(d)
...
diff_countries <- setdiff(d_api$country, d$country)
...
d_long <- d_api %>% 
  pivot_longer(cols = voice_acct:cont_corr, names_to = "indicator", values_to = "value") %>% 
...
## Special case: user-defined function deals with 
  ...
  d_split <- ohicore::split_regions(d_calcs, country_column = 'country', value_column = 
                                      'value')
