---
title: 'OHI 2022: Social Progress Index'
...
# Updates
Now 11 years of data included in SPI. Created updated spi_categories.csv in Mazu. See the 2022 Methodology Report cited below for a detailed description of the SPI changes from 2020 to 2022. This is saved in Mazu and can also be downloaded [here](https://www.socialprogress.org/global-index-2022-methodology).

***

**Citation**: http://www.socialprogress.org/

Stern, S., Harmacek, J., Htitich, M., & Krylova, P.: 2022 Social Progress Index
...
setwd(here::here("globalprep","prs_res_spi","v2022"))
...
```{r, eval = FALSE}
cats <- read.csv(file.path(dir_M, 'git-annex/globalprep/_raw_data/SocialProgressIndex/d2021/spi_categories.csv')) %>%
  mutate(subcategory = gsub(" ", "", subcategory)) # the categories changed a little bit for 2022... all of the "and"s were replaced with "&"... i will fix this in the SPI data

spi_raw <- read_excel(file.path(dir_M, "git-annex/globalprep/_raw_data/SocialProgressIndex/d2022/2022_social_progress_index_dataset-1663250377.xlsx"), sheet = 2, skip = 2)

names(spi_raw) <- gsub(" ", "", names(spi_raw))


all_spi <- spi_raw %>%
  dplyr::select(-`SPI\r\nRank`, -SPIcountrycode, -Status) %>%
  dplyr::filter(Country != "World") %>%
  gather('subcategory', 'score', -Country, -`SPI\r\nyear`) %>%
  mutate(subcategory = str_replace_all(subcategory, "&", "and")) %>%
...
bhn_tmp <- all_spi %>%
  ...
  dplyr::select(Country, year, bhn_score=score) %>%
...
bhn_tmp_long <- bhn_tmp %>%
  dplyr::select(Country, year, bhn_score, NA_tot, NutritionandBasicMedicalCare, 
...
op_tmp <- all_spi %>%
  ...
  dplyr::select(Country, year, op_score=score) %>%
...
op_tmp_long <- op_tmp %>%
  dplyr::select(Country, year, op_score, NA_tot, AccesstoAdvancedEducation, 
...
fw_tmp <- all_spi %>%
  ...
  dplyr::select(Country, year, fw_score=score) %>%
...
fw_tmp_long <- fw_tmp %>%
  dplyr::select(Country, year, fw_score, NA_tot, AccesstoBasicKnowledge, 
...
write.csv(all_together, "int/all_sp_vars.csv", row.names = FALSE)
```
...
spi_calc <- read.csv(here::here("globalprep/prs_res_spi/v2022/int/Country_spi.csv"), stringsAsFactors = FALSE)
...
sum(duplicated(tmp$rgn_id))
summary(spi_rgn)
unique(spi_rgn$rgn_name)

## This is currently not doing anything. These regions are not included.
...
WGI is a couple years behind the SPI, so we will use 2018 WGI for the 2019, 2020, 2021, and 2022 SPI. There is a strong correlation betwen the WGI and SPI indicators.
```{r, eval = FALSE}

wgi <- read.csv('../../prs_res_wgi/v2020/output/wgi_res.csv') %>%
  dplyr::select(rgn_id, year, wgi_score = resilience_score)
...
wgi <- rbind(wgi, wgi_2019, wgi_2020, wgi_2021, wgi_2022)
...
rgns_gf <- rgns_gf_un %>%
  ...
  dplyr::select(rgn_id, year, score, method, gapfill) %>%
  mutate(score = ifelse(score > 100, 100, score),
         score = ifelse(score < 0, 0, score))
...
res_data <- rgns_gf_uninhab %>%
  dplyr::select(rgn_id, year, resilience_score=score) %>%
...
```{r, eval = FALSE}
## Resilience comparison, filtered for 2021
new_spi_res <- read_csv(file.path("output/spi_res.csv")) %>%
  filter(year == 2021) %>%
  dplyr::select(rgn_id, new_resilience_score = resilience_score)

old_new <- read_csv("../v2021/output/spi_res.csv") %>%
  filter(year == 2021) %>% 
...
old_new <- read_csv("../v2021/output/spi_prs.csv") %>%
  filter(year == 2021) %>% 
