...
This file produces three layers: `mar_harvest_tonnes`, `mar_sustainability_score`, and `sp_genetic`. 
...
One thing to note, finding the best way to merge the 2024 assessment data (which is data year 2023) into this incredibly long workflow was tricky. Data is kept in a wide format for part of it to avoid having errors when combining with the FAO tonnes of production data. 
...
Since we have new SFW data, we've decided to combine data from v2024 and v2025 to have two years of data and to assist with gapfilling. 

```{r read-mar-data}
# Read in the mariculture data file we just wrote
...
# We don't pivot this data longer yet to avoid issues when combining with the mariculture data
...
# Check that the year conversion applied correctly for the SFW data.
view(mar_sw_sus_final)
...
# Another way to compare scores (older)
