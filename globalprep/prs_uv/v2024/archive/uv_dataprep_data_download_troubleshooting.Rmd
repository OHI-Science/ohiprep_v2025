...
**Time Range**: Daily data from 2005 - 2023 (10/1/2004 through 06/06/2024, but only full years of data are used)

\
**Format**: NetCDF HDF5 (.he5.nc)

**Downloaded**: June 10, 2024

# Data citation information

Jari Hovila, Antii Arola, and Johanna Tamminen (2013), OMI/Aura Surface UVB Irradiance and Erythemal Dose Daily L3 Global Gridded 1.0 degree x 1.0 degree V3, NASA Goddard Space Flight Center, Goddard Earth Sciences Data and Information Services Center (GES DISC), Accessed: June 10, 2024, 10.5067/Aura/OMI/DATA3009
...
3.  Download the links list *.txt* file
...
2024 note: I used the same login information as last year (under Erika Egg's name). See the GitHub issue [#289](https://github.com/OHI-Science/globalfellows-issues/issues/289)

NOTE: updated to OHIfellows.2024! in 2024 to troubleshoot authentication error.

Also made another account: ohifellows2024, with the same password

```{r download-test}
# ==== for testing local download with older version of R (for Dustin) ========
# usrname: ohifellows2024
# pass: OHIfellows.2024!

uv_local_dir <- file.path("/Users/dustinduncan/Desktop/uv_data_test")
uv_local_raw <- file.path(uv_local_dir, "data")

# download file list
file_list_raw <- read_delim(
  file.path(uv_local_dir, "file_list-07-16.txt"), 
  delim = "\\", col_names = FALSE)

file_list <- file_list_raw %>% 
  mutate(url_str = as.character(X1)) %>% 
  # check if file name is a NetCDF file
  mutate(check_netcdf = str_match(url_str, pattern = "http.*OMI-Aura_L3-OMUVBd.*nc4")) %>% 
  # only keep NetCDF files
  filter(!is.na(check_netcdf)) %>% 
  # keep character version of urls
  select(url_str)

# pull out 1st file url
test_url <- as.character(file_list[1, ])

# define file names
name_raw_file_test <- substr(test_url, 88, 146) 

test_get <- httr::GET(test_url, authenticate(usrname, pass, type = "basic"), verbose(info = TRUE, ssl = TRUE))
test_bin <- httr::content(test_get, as = "raw")
writeBin(test_bin, file.path(uv_local_raw, "data", name_raw_file_test))

# try to open 1st file
test <- nc_open(uv_local_raw,
                "OMI-Aura_L3-OMUVBd_2004m1005_v003-2016m0525t192148.he5.nc4")
```
...
# v2024: all files were 27..... definitely didn't download properly
...
## 2024 Download Issues 

### Testing out different solutions


Creating file structure following [this](https://urs.earthdata.nasa.gov/documentation/for_users/data_access/r) guide and [this](https://disc.gsfc.nasa.gov/information/howto?title=How%20to%20Generate%20Earthdata%20Prerequisite%20Files) guide (note that the second guide is for python applications)
```{r}
netrc_path <- "~/.netrc"
cookie_path <- "~/.urs_cookies"
downloaded_file_path <- "/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2024/wget_test"
...
httr::GET(url = file_list[1, ],
                  write_disk(downloaded_file_path, overwrite = TRUE))
```


Trying https://wiki.earthdata.nasa.gov/display/EL/How+To+Access+Data+With+cURL+And+Wget 

curl -O -b ~/.urs_cookies -c ~/.urs_cookies -L -n  sftp://ramji@mazu.nceas.ucsb.edu/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2024/wget_test


wget --load-cookies ~/.urs_cookies --save-cookies ~/.urs_cookies --keep-session-cookies sftp://ramji@mazu.nceas.ucsb.edu/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2024/wget_test




Creating an additional row index column to only download new files (after OMI-Aura_L3-OMUVBd_2021m0212_v003-2021m0216t093002.he5.nc4)

```{r}
# check how many files we should have
nrow(file_list)

# make a copy of file_list
file_list_index <- file_list 

# view head of list
head(file_list_index) # seems tobegin at the earliest date (important for indexing)

# Assign the new column "index" to the row index
file_list_index$index <- 1:nrow(file_list)
  
# check that updates were made (additional index column added)
head(file_list_index) # yep!

# Now we want to find the index position (row) at which the file
# "OMI-Aura_L3-OMUVBd_2021m0212_v003-2021m0216t093002.he5.nc4" can be found, 
# so that we can begin our download process again after that point

# Full url should be:
# https://acdisc.gesdisc.eosdis.nasa.gov/opendap/HDF-EOS5/Aura_OMI_Level3/OMUVBd.003/2021/OMI-Aura_L3-OMUVBd_2021m0212_v003-2021m0216t093002.he5.nc4?ErythemalDailyDose,lat,lon

file_list_index %>% filter(url_str %in% c("https://acdisc.gesdisc.eosdis.nasa.gov/opendap/HDF-EOS5/Aura_OMI_Level3/OMUVBd.003/2021/OMI-Aura_L3-OMUVBd_2021m0212_v003-2021m0216t093002.he5.nc4?ErythemalDailyDose,lat,lon"))

# appears to be index 5940 ! 

# so...

# we can update our data download function to begin at the 5940th position, rather than the 1st position in our list!

```

Re-running the previous chunk with the updated index position:
...
## download the data -------------- this is where I'm updating the index position
for(i in 5940:nrow(file_list)){   # starting from the 5940th item in the list (rather than the 1st)
  #i = 1
  url = as.character(file_list[i,])
  
  name_raw_file = substr(url, 88, 146) 
...
**2024:** Ran into timeout/crash issue when downloading the data. Process stopped after 
OMI-Aura_L3-OMUVBd_2021m0212_v003-2021m0216t093002.he5.nc4


https://acdisc.gesdisc.eosdis.nasa.gov/opendap/HDF-EOS5/Aura_OMI_Level3/OMUVBd.003/2004/OMI-Aura_L3-OMUVBd_2004m1001_v003-2016m0525t164359.he5.nc4?Er…
```{r}
# ============ TEST single file extraction ================

test_url <- as.character(file_list[1, ])

name_raw_file_test <- substr(test_url, 88, 146) # v2023: check that this is grabbing the full name (should end with .he5.nc4)
# v2024: checked & working!
# 
# if (file.exists(file.path(raw_data_dir, "data", name_raw_file_test)) != TRUE) {
#   
test_get <- httr::GET(test_url, authenticate(usrname, pass, type = "basic"), verbose(info = TRUE, ssl = TRUE))
test_bin <- httr::content(test_get, as = "raw")
#writeBin(test_bin, file.path(raw_data_dir, "data", name_raw_file_test))



test_files <- list.files(file.path(raw_data_dir, "data"), pattern = "OMI-Aura_L3-OMUVBd.*.he5.nc4$", full.names = TRUE) # netcdf not hdf
test_files <- files[substr(files, 96, 99) %in% yrs] # select only files for yrs we want

test_files %>% data.frame()

#   writeBin(bin, file.path(raw_data_dir, "data", name_raw_file_test)) # gnutls_handshake() failed: Handshake failed
# } # END test download


# ============= TEST httr2 package implementation ================

token <- "eyJ0eXAiOiJKV1QiLCJvcmlnaW4iOiJFYXJ0aGRhdGEgTG9naW4iLCJzaWciOiJlZGxqd3RwdWJrZXlfb3BzIiwiYWxnIjoiUlMyNTYifQ.eyJ0eXBlIjoiVXNlciIsInVpZCI6Im9oaWZlbGxvd3MiLCJleHAiOjE3MjQ0NTAxMzksImlhdCI6MTcxOTI2NjEzOSwiaXNzIjoiRWFydGhkYXRhIExvZ2luIn0.e8ixlxSh2QDjiLbw85puaGPTYTnjO26GgnnsoJwi93jopeM9dFxgGe-gosJrVn8-0ge-6d8BboU2Dz6w6PXKTlqTpDY_rjPTdkm2SqxJlNUvcp0GztRCekEu6zv5mPAzJyqapKLx7in9BQ1x80vspBpjHAMGCCxx5J0hOxZmCzcyVl2mFg1P2rhQin1UkgLT7xiZOlql8IzF0F-LgqWwmJmKyXP3ylTpOB-xDb_1Z_SXkKziZMDxAnvW5l6wHHodH60u-cd-XXRwJPs_KSOnHK9JSZ0ezhaZQB32059CByHg6Rsi4jtRyF4LtCh66sZVdanEICJnuiq9gi1r5ximPg"


old_user <- "ohifellows"
old_pass <- "OHIfellows.2024!"

req <- httr2::request(test_url) %>%
  httr2::req_auth_basic(usrname, pass) %>%
  httr2::req_perform()


req2 <- httr2::request(test_url) %>%
  httr2::req_auth_basic(old_user, token) %>%
  httr2::req_perform()


test2_get <- httr::GET(test_url, authenticate(old_user, token, type = "basic"),
                       verbose(info = TRUE, ssl = TRUE))
test2_bin <- httr::content(test2_get, as = "raw")
# using token failed too....
# keeps returning 27 ASCII code that translates to "HTTP Error: Access denied."
```



### June 24th: Using new file list 
...
## read in file list .text, downloaded from earthdata & saved in destination folder (same as raw_data_dir)
file_list_raw <- read_delim(
  file.path("/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV", data_yr, "file_list_06-24.txt"), 
  delim = "\\", col_names = FALSE)
...
Hardcoded downloading:
```{r, eval=FALSE}
doParallel::registerDoParallel(cores = 30)
# get list of files from previous year's download
file_list_23 <- list.files("/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2023/data", full.names = TRUE,)

# copy over files 
file.copy(from = file_list_23, to = "/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2024/data_copy")

```

Copied over data ending with /home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2024/data_copy/OMI-Aura_L3-OMUVBd_2023m0502_v003-2023m0506t093001.he5.nc4

aka 2023 May 2nd

Considering that the "date updated" listed on the data website (https://acdisc.gesdisc.eosdis.nasa.gov/data/Aura_OMI_Level3/OMUVBd.003/) has stayed consistent (hasn't been updated after the 2023 data was downloaded last year) I am assuming that the methodology hasn't changed (otherwise they would show that the "date updated" was a more recent date). Thus, I should be able to go ahead and continue downloading data starting after May 2nd, 2023.




### download.file method (not ideal, come back to this)

https://stat.ethz.ch/R-manual/R-devel/library/utils/html/download.file.html

```{r}
download.file(url = as.character(file_list[1, ]), destfile = "/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2024/test.he5.nc4",
              method = "curl",
              cacheOK = TRUE,
              extra = "-L")
...
test_file <- file.path("/home/shares/ohi/git-annex/globalprep/_raw_data/NASA_OMI_AURA_UV/d2024/test.he5.nc4")

test_df <- test_file[1] %>% data.frame()
```
