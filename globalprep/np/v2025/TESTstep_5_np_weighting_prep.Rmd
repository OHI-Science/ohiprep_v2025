...
write_csv(orn_tonnes, here(current_np_dir, "test", "orn_tonnes.csv"))
...
write_csv(sw_tonnes, here(current_np_dir, "test", "sw_tonnes.csv"))
...
new_fofm <- map_dfr(2020:2022, function(y) {
  fofm_tonnes %>%
    filter(year == 2019) %>% 
...
write_csv(fofm_tonnes, here(current_np_dir, "test", "fofm_tonnes.csv"))
...
readr::write_csv(harvest_tonnes_usd_values_gf,
                 here(current_np_dir, "test", "harvest_tonnes_usd_weighting_gf.csv"))
...
readr::write_csv(weighting_usd_values_raw,
                 here(current_np_dir, "test", "harvest_weighting_values_raw.csv"))
...
readr::write_csv(weight_usd_values_final,
                 here(current_np_dir, "test", "harvest_weighting_values.csv"))
...
orn_tonnes <- read_csv(here(current_np_dir, "test", 'orn_tonnes.csv'))
sw_tonnes <- read_csv(here(current_np_dir, "test", 'sw_tonnes.csv'))
fofm_tonnes <- read_csv(here(current_np_dir, "test", 'fofm_tonnes.csv')) %>% 
  select(!gapfilled)
...
usd_tonnes_raw <- read_csv(here(current_np_dir, "test", "harvest_weighting_values.csv"))
view(usd_tonnes_raw)
...
readr::write_csv(np_value_test, here(current_np_dir, "test", "np_product_weights.csv"))
...
old_prod_weights <- read_csv(here(previous_np_dir, "output", "np_product_weights.csv"))
prod_weights <- read_csv(here(current_np_dir, "test", "np_product_weights.csv"))
view(prod_weights)
check <- prod_weights %>%
  rename("new_weight" = "weight") %>%
  left_join(old_prod_weights, by = c("rgn_id", "product")) %>%
