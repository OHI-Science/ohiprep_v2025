...
if(!file.exists(spp_info_from_api_file) | reload) {
  ...
  spp_npage_url <- sprintf('http://api.iucnredlist.org/api/v4/speciescount?token=%s', api_key)
  n_spp <- fromJSON(spp_npage_url) %>%
    .$count %>% as.integer()
  n_pages <- ceiling(n_spp/10000)
  
  spp_page_url <- 'http://api.iucnredlist.org/api/v4/species/page/%s?token=%s'
