...
dir_git_old  <- file.path('~/github/ohiprep_v2025', "globalprep/hab_coral/v2012")
```
...
health <- read_csv(file.path(here("globalprep/hab_coral/v2025/data/habitat_health_coral_updated.csv")))

# Rename the dfs
health_2021 <- old_health %>%
...
compare_habitat_health <- health_2025 %>%
  filter(rgn_id <= 250) %>%
  left_join(health_2021, by = "rgn_id") %>%
  mutate(difference = health_2021 - health_2025) %>%
  left_join(rgns_eez)

test <- compare_habitat_health %>%
  filter(is.na(health_2021)) ## we gained 2 regions with health data
