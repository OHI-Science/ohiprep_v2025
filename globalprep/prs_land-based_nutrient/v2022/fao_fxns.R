mar_split <- function(m) {
  ...
  m_ant <- m %>%
    filter(area == 'Netherlands Antilles') %>%  # Conch was cultivated for restoration purposes in a joint programme across these 3 countries
    mutate(value            = country_application/3,  
      'Aruba'        = country_application,
      'Bonaire'           = country_application,
      'Curacao'   = country_application) %>%
    select(-country_application, -country) %>%
