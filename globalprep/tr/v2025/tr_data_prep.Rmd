...
mod_test <- lm(total_guests ~ 0 + hotel_guests, data = dom_acc_guests_agg)
summary(mod_test)
mod_test$coefficients[2]
...
# write_csv(dom_acc_guests_gf1, here(current_tr_dir, "int", "dom_acc_guest_gf1.csv"))
```
...
table(unwto_int_upfill$arrivals_method)
nrow(unwto_int_upfill)
colSums(is.na(unwto_int_upfill))
...
int_arrivals_gf <- pre_pandemic_int %>%
  ...
  mutate(arrivals_method = ifelse(is.na(arrivals_method) & !is.na(tourism_arrivals_ct), "2020:2022 gapfill", arrivals_method)) %>%
  mutate(arrivals_gapfilled = ifelse(arrivals_method == "2020:2022 gapfill", "gapfilled", arrivals_gapfilled)) %>%
...
length(unique(arrivals_join$rgn_name)) # v2024 and v2025: 156 regions that have data! This checks out!
```
...
length(!is.na(arrivals_aggregated$tourism_arrivals_ct_int)) # 2184 v2025: 2340
length(!is.na(arrivals_aggregated$tourism_arrivals_ct_dom)) # 2184 v2025: 2340
...
filter(total_population_final, is.na(year)) %>% view()
