...
fofm_tonnes <- read_csv(here(current_np_dir, "int", "raw_mean_catch_FOFM.csv")) %>%
  ...
  summarise(sum_tonnes = sum(catch_non_human) * 0.3) %>% 
  ungroup() # %>%
  # group_by(rgn_id, product) %>%
...
This code chunk above creates a data frame for each product with 5 year averages of tonnes of production. 
...
**ISSUE** the NP plan mentions that this should be the absolute tonnes of production (not mean catch)?
