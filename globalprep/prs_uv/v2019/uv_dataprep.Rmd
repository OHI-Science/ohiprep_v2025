...
**Time Range**: Daily data from 2005 - 2018 (10/1/2004 through 8/6/2019, but only full years of data are used)
...
int_sp_data <- file.path(dir_M, "git-annex/globalprep/prs_uv/v2019/int") # intermediate spatial data location
out_dir <- file.path(dir_M, sprintf("git-annex/globalprep/prs_uv/v2019/output"))
...
resc_num  <- stats::quantile(vals, prob = 0.9999, na.rm = TRUE) # 42 for v2018; 46 for v2019
...
foreach(i = 2005:(max(yrs) - 4)) %dopar% {
  ...
  mol1km_masked <- resc_files[str_sub(resc_files, -22, -19) == i] %>%
    ...
    mask(ocean, 
         filename = file.path("/home/shares/ohi/git-annex/globalprep/prs_uv/v2019/output", 
...
old <- uv_data %>% 
  filter(year == 2017) %>% # data years lag assessment yrs by 1
  select(-year, new_pressure = pressure_score) %>% 
  left_join(read.csv("../v2018/output/uv.csv") %>% filter(year == 2017),
             by = c("rgn_id")) %>% 
