...
This script takes the Watson 2020 (v5.0) catch data, provided at a resolution of half-degree cells globally, and creates 3 data layers:
...
v2021: Since we did not have new fisheries data for this year, we only updated the RAM data. However, just for reproducibility sake, I reran all of the fisheries scripts.
...
The raw Watson data is separated into industrial and non-industrial fishing. Combine both types for each year from 1950-2017.
...
catch_old <- readRDS(file.path("/home/shares/ohi/git-annex/globalprep/fis/v2020/int", "annual_catch","Catch_2015.rds"))
...
write.csv(total_catch, file = file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn.csv'), row.names=FALSE)
...
write.csv(taxonkey, "int/watson_taxon_key_v2021.csv", row.names=FALSE)
```
...
taxonkey <- read.csv("int/watson_taxon_key_v2021.csv", stringsAsFactors = FALSE)

stock_rgn <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2021/int/stock_catch_by_rgn.csv'))

# check
setdiff(paste(taxonkey$TaxonName, taxonkey$CommonName), 
        paste(stock_rgn$TaxonName, stock_rgn$CommonName)) # no differences
...
# 151050 catch data without fao regions assigned - v2021
df_na <- df %>% 
...
summary(output_df) # FAO ID 24,736 NAs - v2021
...
stks <- read_csv("globalprep/fis/v2021/output/stock_catch_no_res.csv")
...
Compare v2021 with last year v2020 (should be exactly the same)
```{r, eval=FALSE}
