...
watson <- read_csv(file.path(here(), "globalprep/np/v2020/explore/raw/Codes_taxa.csv"))
sort(setdiff(forage$Species, watson$TaxonName))
sort(setdiff(watson$TaxonName[watson$forage_fish %in% 1], forage$Species))

## Combined list:
foragefish_list <- data.frame(forage_fish = sort(unique(c(unique(forage$Species), watson$TaxonName[watson$forage_fish %in% 1]))))
...
write.csv(foragefish_list, here("globalprep/np/v2020/explore/output/master_taxa_list.csv"), row.names=FALSE)
