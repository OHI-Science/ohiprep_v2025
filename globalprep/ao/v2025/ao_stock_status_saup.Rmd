...
## NOTE v2025: There was no new data for mean_catch this year, so previous year is used
fis_mean_catch <- read_csv(here::here("globalprep","fis", "v2024", "int","mean_catch.csv")) %>% 
  dplyr::mutate(stock_id_taxonkey = as.character(stock_id_taxonkey)) %>%
...
# NOTE v2025: Since the FAO Fish bbmsy data was updated this year, there are a few species that have not been included in years past. Let's check and see what they are!
gapfill_NAs <- gapfill_missing %>%
  filter(is.na(mean_catch) == TRUE)

length(unique(gapfill_NAs$stock_id)) # There are 38 new species
sort(unique(gapfill_NAs$stock_id)) 

# # The new species that have been added are:

# Clupea_harengus-27 (Atlantic Herring) 
# Istiompax_indica-(51,57) (Sailfish)
# Istiophorus_albicans-(27,48) (Sailfish)
# Istiophorus_platypterus-51 (Sailfish) 
# Katsuwonus_pelamis-(27,48,51) (Skipjack Tuna) 
# Makaira_mazara-(51,57) (Blue Marlin)
# Makaira_nigricans-(27,48,61,71,77) (Blue Marlin)
# Pandalus_borealis-27 (Atlantic Shrimp)
# Pollachius_virens-27 (Atlantic Pollock), 
# Prionace_glauca-(61,71,77) (Greenland Halibut)
# Tetrapturus_albidus-(27,37,48) (White Marlin)
# Thunnus_alalunga-(27,48,51) (Albacore Tuna) 
# Thunnus_albacares-(27,48) (Yellowfin Tuna)
# Thunnus_maccoyii-(57,81) (Pacific Bluefin Tuna)
# Thunnus_obesus-(27,48) (Bigeye Tuna)
# Thunnus_thynnus-(27,48) (Atlantic Bluefin Tuna)
# Xiphias_gladius-(27,48) (Broadbill Swordfish)

# We will have to gapfill these values, or drop them

# First, let us test gap filling them with average of Genus
tuna_data <- c %>%
  filter(grepl("Thunnus|Katsuwonus", stock_id))

sailfish_data <- c %>%
  filter(grepl("Istiompax|Istiophorus", stock_id))

marlin_data <- c %>%
  filter(grepl("Makaira|Tetrapturus",stock_id))
...
```{r, eval= FALSE}
# Let's try finding our result without adding new species
gapfill_no_na <- gapfill_missing %>%
  filter(is.na(mean_catch) == FALSE)
```
...
data_fis_final <- rbind(data_fis, gapfill_no_na, fix_bouvet)
...
htmlwidgets::saveWidget(compare_plotly, here::here("globalprep","ao", version_year, "figs","v2024_v2025_compare_plot.html"))
...
diff_plot <- compare_diff %>%
  ...
  layout(title = "AO Sustainability Score Percent Change by Region",
         ...
         yaxis = list(title = "Percent change v2024 vs v2025"))
diff_plot
...
rgn_3_scores_plot <- compare_diff_rgn_3 %>%
  ...
  layout(title = "AO Sustainability Scores v2024 for Norfolk Island",
         xaxis = list(title = "Year"),
         yaxis = list(title = "Scores over time v2025"))
rgn_3_scores_plot
