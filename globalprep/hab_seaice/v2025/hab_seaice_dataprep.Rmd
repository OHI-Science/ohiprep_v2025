...
-   In your terminal on Mazu run:
    ...
    nsidc-download_NSIDC-0051.002_2025-08-05.py
...
## UPDATE THESE!
current_year <- 2025
last_year <- 2024 # final year of data (all months)
...
source(here("globalprep/hab_seaice/v2025/ObtainingData_Parallel.R")) 

toc()
## should take ~1.5 hrs to completely run with ObtainingData_Parallel.R 
# v2025: 5594.57 sec elapsed
```

## Function 2

Using the data from the .rdata files created from Function 1 with the
`ObtainingData_Parallel.R` script, this function calculates status and
...
tic()
ref.years <- 1979:2000
source(here("globalprep/hab_seaice/v2025/Status_Trend.R")) # calculates status and trend for shoreline ice and ice edge habitat
toc()
#v2025 116.813 sec elapsed
```

## Final calculations and organization

Read in ice edge habitat and ice shore protection csv-format datasets,
remove anomalous eez regions with minimal ice cover, remove disputed
...
n_edge <- read.csv("globalprep/hab_seaice/v2025/int/n_IceEdgeHabitat_ref1979to2000.csv")
s_edge <- read.csv("globalprep/hab_seaice/v2025/int/s_IceEdgeHabitat_ref1979to2000.csv")
...
n_shore <- read.csv("globalprep/hab_seaice/v2025/int/n_IceShoreProtection_ref1979to2000.csv")
s_shore <- read.csv("globalprep/hab_seaice/v2025/int/s_IceShoreProtection_ref1979to2000.csv")
...
write.csv(data, "globalprep/hab_seaice/v2025/int/sea_ice.csv", row.names = FALSE)
...
write.csv(health, "globalprep/hab_seaice/v2025/output/hab_ice_health_eez.csv", row.names = FALSE) # save sea ice health data
...
write.csv(trend, "globalprep/hab_seaice/v2025/output/hab_ice_trend_eez.csv", row.names = FALSE) # save sea ice trend data
...
write.csv(extent, "globalprep/hab_seaice/v2025/output/hab_ice_extent_eez.csv", row.names = FALSE) # save sea ice extent data
```
...
# 1. Compare between scenario years (v20__)

# Read in the health from current scenario year
health_file_path <- file.path("globalprep/hab_seaice", assess_year, "output/hab_ice_health_eez.csv")
health <- read.csv(health_file_path)

# Compare to last year's data                 
ice_health_eez <- read.csv(sprintf("globalprep/hab_seaice/%s/output/hab_ice_health_eez.csv", previous_year)) %>% 
  dplyr::rename(health_prev_assess = health) %>%
  dplyr::left_join(health, by = c("rgn_id", "habitat", "year")) %>% 
  na.omit("health") 

comp_plot <- ggplot() + geom_point(data = ice_health_eez, aes(x = health_prev_assess, y = health)) + labs(x = paste0("previous health (2012–", previous_year, ")"), y = paste0("health (2012–", assess_year, ")")) + geom_abline(slope = 1, intercept = 0, col = "red") + labs( title= "Compare Health Between Scenario Years")
plotly::ggplotly(comp_plot)


# 2. Compare between data years (most recent years of full data)
...
current_data_yr = 2024

## Last full year of data
previous_data_yr = 2023

# Create health filtered to full data years of relevance
health_current <- health %>% # Update year to current year
  dplyr::filter(year == current_data_yr)

health_previous <- ice_health_eez %>% # Update year to current year
  dplyr::filter(year == previous_data_yr) %>%
  dplyr::select(-year, -health)

health_old_new <-  dplyr::left_join(health_current, health_previous) %>% mutate(difference = (health - health_prev_assess)) %>% left_join(regions) # Update years to current years
...
year_change_plot <-ggplot() +
  geom_point(data = health_old_new,
             aes(x = health_prev_assess,
                 y = health, 
                 text = paste("Region ID:", rgn_id, "<br>Habitat:", habitat))) + labs(x = "previous assessment health", y = "current health") + geom_abline(slope = 1, intercept = 0, col = "red")
...
# Read in the trend from current scenario year
trend_file_path <- file.path("globalprep/hab_seaice", assess_year, "output/hab_ice_trend_eez.csv")
trend <- read.csv(trend_file_path)

# 1. Compare between scenario years (v20__)

 # compare to last year's data
ice_trend_eez <- read.csv(sprintf("globalprep/hab_seaice/%s/output/hab_ice_trend_eez.csv", previous_year)) %>%
  dplyr::rename(trend_prev_assess = trend) %>%
  dplyr::left_join(trend, by = c("rgn_id", "habitat", "year")) %>% 
  na.omit("trend")

# update label years 
trend_comp <- ggplot() + geom_point(data = ice_trend_eez, aes(x = trend_prev_assess, y = trend)) + labs(x = paste0("previous trend (2016–", last_year, ")"), y= paste0("current trend (2016–", current_year, ")")) + geom_abline(slope = 1, intercept = 0, col = "red")
...
trend_previous <- ice_trend_eez %>%
  dplyr::filter(year == previous_data_yr) %>%
  dplyr::select(-year, -trend)

trend_old_new <- dplyr::left_join(trend_current, trend_previous) %>% mutate(difference = (trend - trend_prev_assess)) %>% left_join(regions)

year_trend_change_plot <-ggplot() + geom_point(data = trend_old_new, aes(x = trend_prev_assess, y = trend, text = paste("Region ID:", rgn_id, "<br>Habitat:", habitat))) + labs(x = "previous data year trend", y = "current data year trend") + geom_abline(slope = 1, intercept = 0, col = "red")
...
# Read in the extent from current scenario year
extent_file_path <- file.path("globalprep/hab_seaice", assess_year, "output/hab_ice_extent_eez.csv")
extent <- read.csv(extent_file_path, stringsAsFactors = FALSE)
...
ice_extent_plot <- ggplot(data = ice_extent_eez) +geom_point(aes(x =km2_prev_assess, y = km2)) + geom_abline(slope = 1, intercept = 0, col = "red") +
  labs(title= "Change in Extent by Scenario Year")

plotly::ggplotly(ice_extent_plot)

# 2. Compare between data years (most recent years of full data)

extent_current <- extent 

extent_previous <- ice_extent_eez

extent_old_new <- dplyr::left_join(extent_current, extent_previous)

 # extent doesn't change at all, because we use the same year for extent every year
ice_extent_plot <- ggplot(data = extent_old_new) + geom_point(aes(x = km2_prev_assess, y = km2)) + geom_abline(slope = 1, intercept = 0, col = "red") + labs(title= "Change in Extent by Data Year")
...
file_path <- file.path("globalprep/hab_seaice", assess_year, "output/hab_ice_health_eez.csv")
data <- read.csv(file_path) %>%
  dplyr::left_join(regions, by = "rgn_id") %>%
  dplyr::arrange(habitat, health)

#### Check largest score changes for v2025 ####

# Nothing of note to record
...
```{r, eval=FALSE}
# NOTE: Change all of the version years (v20__) in the following lines!

## Health gapfill
hab_ice_health_gf <- read.csv("globalprep/hab_seaice/v2025/output/hab_ice_health_eez.csv")%>%
  dplyr::mutate(gapfilled = 0) %>% 
  dplyr::select(rgn_id, year, gapfilled)

write.csv(hab_ice_health_gf, "globalprep/hab_seaice/v2025/output/hab_ice_health_eez_gf.csv", row.names=FALSE) # save sea ice health gapfill file

## Extent gapfill
hab_ice_extent_gf <- read.csv("globalprep/hab_seaice/v2025/output/hab_ice_extent_eez.csv")%>%
  dplyr::mutate(gapfilled = 0) %>% 
  dplyr::select(rgn_id, year, gapfilled)

write.csv(hab_ice_health_gf, "globalprep/hab_seaice/v2025/output/hab_ice_extent_eez_gf.csv", row.names=FALSE) # save sea ice extent gapfill file

## Trend gapfill
hab_ice_trend_gf <- read.csv("globalprep/hab_seaice/v2025/output/hab_ice_trend_eez.csv")%>%
  dplyr::mutate(gapfilled = 0) %>% 
  dplyr::select(rgn_id, year, gapfilled)

write.csv(hab_ice_health_gf, "globalprep/hab_seaice/v2025/output/hab_ice_trend_eez_gf.csv", row.names=FALSE) # save sea ice trend gapfill file
...
```{r, eval=FALSE}
# Rename the 2025 data
hab_ice_health_eez_2025 <- hab_ice_health_eez
```

```{r, eval= FALSE}
hab_ice_health_eez_2024 <- hab_ice_health_eez
```
