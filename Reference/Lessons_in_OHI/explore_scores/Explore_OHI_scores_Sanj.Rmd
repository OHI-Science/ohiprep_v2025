---
title: "Learning about OHI scores"
output: html_document
---

```{r setup, include=FALSE}
## loading packages.. you might need to install them first
library(dplyr)
library(tidyr)
library(ggplot2)
install.packages("janitor")
library(janitor)
install.packages("stringr")
library(stringr)
```


## OH NO!
While you are working at your computer at NCEAS, you receive a frantic call from someone working on an OHI project:

THEY NEED HELP!

It is clearly going to be up to you to save the day!!

## You've got this!

Please help them answer the following questions.  You will work from this document, using the code chunks as a work space to work explore the data. Do NOT bother keeping the code chunks neat and organized.   

You can ask us any questions along the way (slack or issues)!

Good luck!

## Questions

### Getting the data

*Goal* Read the latest OHI global data into the R working space (call the data object "scores"). These are the scores that were calculated for the 2023 assessment. This is the final output after running the scripts in ohi-global. Poke around at the data to get a feel for what is there.

*Hint* Here is a link to the data: 
https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/eez/scores.csv


```{r, include=FALSE}
## Working space
scores<-read.csv("https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/eez/scores.csv")
scores
```

### Region explore

*Question* Why are there N=222 regions (see code below)...aren't there supposed to be 220? 

Can you make this data more human readable by including the full goal name (vs. the 2 letter abbreviation) and the country name.

*Hint 1* Here is a link to the official region list (the rgn_id column in this file matches the region_id column in the scores data): https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/eez/spatial/regions_list.csv

*Hint 2* Here is a table that includes goal names: https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/metadata_documentation/ohi_model/tables/ten_goals.csv

*Hint 3* Which region has all NA values for scores?

*Answer* Write your answer here!
The scores data set has a region 0 which isnt in the official region list and it has region 213 Antactica which isnt included in the global assessment

```{r, include=FALSE}
## Working space

#reading ohi regions in and renaming it to match scores column names for region id
unique(scores$region_id)
ohi_regions<-read.csv("https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/eez/spatial/regions_list.csv")
ohi_reg_countries<-ohi_regions %>% 
  select(rgn_id, admin_country_name) %>% 
  rename("region_id"="rgn_id")

#joining region names to scores with left join to keep scores as the main frame
scores_regionnames<-left_join(scores, ohi_reg_countries, by= "region_id")

#reading in goals names and renaming columns to match new scores dataset
goals<-read.csv("https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/metadata_documentation/ohi_model/tables/ten_goals.csv")
goals<-goals %>% 
  select(-Description) %>% 
  clean_names()
goals<-goals %>% 
  rename("fullgoal"="goal", "goal"="abbreviation")

#joining new scores and goals with a full join because there are more goal ids in new scores than in goals
scores_reg_goals<-full_join(scores_regionnames, goals, by= "goal")

#removing and renaming to keep it clean
rm(goals, ohi_reg_countries, ohi_regions, scores_regionnames)
scores_og<-scores
scores<-scores_reg_goals
rm(scores_reg_goals)
```

*Question* Which region id corresponds to the United States? 12, 13, 116, 149,150,151, 158,159,163


### Goal explore

*Question * When I look at the number of goals, N=19 show up....I thought there were supposed to be 10 goals!!! Can you explain what all these are? Also, why do some of the goal abbreviations have 2 letters and others 3 letters?  

*Hint 1* Review the methods doc here: https://raw.githack.com/OHI-Science/ohi-global/published/documents/methods/Supplement.html

*Answer* Write your answer here! There are 10 big goals but smaller subgoals within them, those have 3 letters in their ID while the big goals have 2 letters. The subgoals are ECO-Economies(LE) FIS-Fisheries(FP) HAB-Habitat(BD) ICO-Iconic Species(SP) LIV-Livelihoods(LE) LSP-Lasting Special Places(SP) MAR-Mariculture(FP) and SPP-Species Condition(BD)

```{r, include=FALSE}
## Working space
unique(scores$goal)

```


### Dimension explore

*Question* There are 6 dimensions reported in this dataset (future, pressures, resilience, score, status, and trend).  When I look at the number of records in each of these categories, I notice that there are more "score" values reported than "status" values (see working space below). Given that scores are calculated using status, it seems like these should have the same number of records. Can you figure out what is going on here. Is something going wrong?

*Answer* Write your answer here! 
Index doesnt use status

```{r, include=FALSE}
## Working space

unique(scores$dimension)
table(scores$dimension)

table(scores$goal)
table(scores$goal, scores$dimension)


```


### Missing data

*Question* Figure out which goals/subgoals have the most NA values for scores in 2023.  Which ones have the least? Can you discern any reason why some goals have lots of missing values and others do not?

*Hint* Include only dimension = score, and year = 2023, and cut region_id = 0.

*Answer* Write your answer here!

```{r, include=FALSE}
## Working space
class(scores$year)
scores2023<-scores %>% 
  filter(year==2023) %>% 
  filter(dimension=="score") %>% 
  filter(region_id==!0)

missing<-scores2023 %>% 
  filter(is.na(score))


###why is there only 1 :(


```


### Scores

*Question* If we have a goal with a future status of 80 and status of 90...what is the score?  

*Hint* Isolate the future, status, and score values for one region and one goal and see if you can identify a pattern.

*Answer* Write your answer here! Its the average of both so it would be 85

```{r, include=FALSE}
## Working space
scores_AUS<-scores %>% 
  filter(region_id==1)
scores_AUS

(70.87+72.67)/2

rm(scores_AUS)

```


### Metadata

*Project* Based on your data exploration and other resources, provide some metadata that describes each variable.  Write it so it would be useful to you in the future as you are looking through these data.

Write it in the following table.  NOTE: Knit the document to see if the table is formatting correctly (but don't bother if you do not know how to knit a document or if you are running into errors!).

Variable   | Description                  | Values
---------- | ---------------------------- | ------------------------------
goal       |  10 goals 8 subgoals         | AO, BD, CP, CS, CW, ECO, FIS, FP, HAB, ICO, LE, LIV, LSP, MAR, NP, SP, SPP, TR
dimension  |  Factors to combine for final score and final score                               | future, pressures, resilience, score, status, trend 
region_id  |  number associated w country | 0-250
score      |  Calculated value            | 0-100      
year       |  Scenario year               | 2012 to 2022


```{r, include=FALSE}
## Working space
unique(scores$goal)
```


### Plot

*Project* Create a scatterplot that compares 2012 and 2023 *scores* for each region for the artisanal opportunities goal. Based on this, do scores for the artisanal opportunities goal appear to have increased or decreased over time? 

Why might this be? How could you begin to figure this out?

*Answer* Write your answer here!

```{r, include=FALSE}
## Working space
artisanal_scores <- scores %>% 
  filter(goal == "AO") %>% 
  filter(year %in% c(2012,2023))
artisanal_scores

artisanal_scores<-artisanal_scores %>% 
  filter(dimension=="score")

unique(artisanal_scores$dimension)

artisanal_scores<-artisanal_scores %>% 
  pivot_wider(names_from=year, values_from= score)

artisanal_scores<-na.omit(artisanal_scores)
artisanal_scores$'2023'<-as.numeric(artisanal_scores$'2023')
artisanal_scores$'2012'<-as.numeric(artisanal_scores$'2012')


artisanal_scores<-artisanal_scores %>% 
  mutate(changes=`2023`-`2012`)

artisanal_scores %>% 
  ggplot(aes(x=region_id, y=changes))+
  geom_point(size=1, alpha=0.5)+
  labs(title="Artisanal Opportunities in 2012 vs 2023")+
  theme_minimal()


```


Create a plot that shows the distribution (e.g., histogram or barplot or ??) of *Index* scores of all countries.

```{r across year plot}

## Code to create a plot showing distribution of country Index scores
unique(scores$goal)
unique(scores$dimension)


index_scores<-scores %>% 
  filter(goal=="Index")
index_scores<-index_scores %>% 
  filter(dimension=="score")

ggplot(data=index_scores, aes(x=region_id, y=score))+
  geom_col()+
  facet_wrap(~year)+
  labs(title = "Index Scores for each Region by Year")


```
