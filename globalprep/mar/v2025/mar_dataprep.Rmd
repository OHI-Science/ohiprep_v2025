...
mar_sp <- readr::read_csv(here::here(current_mar_dir, "raw", "species_list.csv")) %>%
  ...
  mutate(FAO_name = case_when(
    # This name got cutoff when reading in the data
    FAO_name == "Smalleyed squillid mantis shri" ~ "Smalleyed squillid mantis shrimp",
    .default = FAO_name
  ))
...
# Name fixing 2025
## It seems like a lot of "different" species come from NEI being all caps instead of nei. I'm going to change the case and see if that shrinks the list down from 112 species. 

# NEIs in the 2025 data
sum(str_detect(mar$FAO_name, "NEI")) # 38480
sum(str_detect(mar$FAO_name, "nei")) # 0

# NEIs in the 2024 species list
sum(str_detect(mar_sp$FAO_name, "NEI")) # 0
sum(str_detect(mar_sp$FAO_name, "nei")) # 103
length(new_spp) # 112

# Looks like by the numbers, most of these are NEI errors
# Change to lowercase
mar_lower <- mar %>% 
  mutate(FAO_name = str_replace_all(mar$FAO_name, "NEI", "nei"))

sum(str_detect(mar_lower$FAO_name, "NEI")) # 0
sum(str_detect(mar_lower$FAO_name, "nei")) # 38480

new_spp2 <- setdiff(mar_lower$FAO_name, mar_sp$FAO_name)

mar_lower %>% 
  filter(FAO_name %in% new_spp2) %>%
  view()

new_spp2
# ! Now, there's only 16 new species

# Change to lowercase on the original data frame to continue working for now...
mar <- mar %>% 
  mutate(FAO_name = str_replace_all(mar$FAO_name, "NEI", "nei"))
...
mar <- mar %>% 
  mutate(FAO_name = case_when(
    FAO_name == "So-iny (redlip) mullet" ~ "So-iuy mullet",
    FAO_name == "Kelps nei" ~ "Kelp nei",
    FAO_name == "Ascidiaceans nei" ~ "Ascidiaceans",
    FAO_name == "Sea lettuce" ~ "Sea lettuces nei",
    .default = FAO_name
  ))
