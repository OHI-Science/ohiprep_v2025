...
if(nrow(maps_to_rasterize) == 0) { ### all maps accounted for as .csvs
  ...
  #bli_already_done <- read_csv(file.path(dir_setup, "int/bli_already_done.csv"))  
  ...
  for(i in seq_along(shps)) {
    ...
    polys_all <- st_read(shp, type = 6) %>%
      st_sf() %>%
      dplyr::rename("sciname" = 2)
...
## v2022: column name in IUCN shapes changed to "binomial", I renamed it to "sciname" when reading in. 
