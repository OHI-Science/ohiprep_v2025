...
-   New year of FAO mariculture yield data for 2021. Downloaded the data online through Fishstat online query instead of using the fishstatj software. Updated instructions for downlaoding the data. Updated the script to work with minor changes in formatting in the new data.

-   updated the species_list.csv so that any of the duplicate species that were relevant and had been excluded are now included. Also updated this file with any new species names in the fao data. Removed the alias column, as it was being inconsistently used, and instead grouped by family when removing species with less than four years of mariculture for a region. Reorganized code as needed to keep this from creating errors.

-   updated code to programmatically fill in analysis and data years throughout script

-   fixed an issue in the mar_split function (mar_fxs.R) that was duplicating Netherlands Antilles data (Aruba, Bonaire, 'Curacao') by binding the same rows multiple times.

-   Fixed a problem that was removing data due to duplicates in the fao column (FAO major fishing area (Name)) for some species/region/environment/year combinations in the FAO data. 

-   updated outdated function gather to pivot_longer
...
source('../../../workflow/R/fao_fxn.R') # old function for cleaning fao data
source('../../../workflow/R/fao_online_portal_clean.R') #new fao cleaning function
...
mar <- mar %>%
  ...
  rename(country = `Country Name En`,
    ...
    mutate(FAO_name = ifelse(!is.na(FAO_name), FAO_name, family_scientific)) #if species name is NA then use the family name
...
mar <- mar %>% fao_online_portal_clean(initial_data_year = 1950, last_data_year = latest_data_year)
```
