---
title: "OHI 2022: Food Provision/Fisheries, Preparing RAM B/Bmsy data"
...
* Time range: 1950 - 2024 (we only use the year which matches our fisheries catch data (2019 for v2022))
...
ram_bmsy_gf <- ram_bmsy_new %>%
  filter(year >= 2001 & year <= 2019) %>%   # 2017 corresponds to the final year of SAUP catch data
  group_by(stockid) %>%
    mutate(max_year = max(year)) %>%
...
tmp2 <- data.frame(scientificname = sort(setdiff(SAUP_sp$SAUP_scientificname, ram_sp$scientificname))) # 2346 species names.. unfortunately a lot.
...
length(unique(ram_spatial$stockid)) # 487 RAM stocks with B/Bmsy data - v2022
...
ram_2021 <- read_csv(file.path("../v2021/int/ram_bmsy.csv"))

ram_old_2021 <- read_csv("archive/ram_bmsy.csv")

ram_2022 <- read_csv("int/ram_bmsy.csv")
...
compare <- ram_2022 %>%
  left_join(ram_2021, by = c("rgn_id", "stockid", "stockid_ram", "stocklong", "year", "RAM_area_m2")) %>%
