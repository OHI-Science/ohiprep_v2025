...
colnames(international_arrivals)

test <- international_arrivals %>% 
  group_by(rgn_id, rgn_name) %>% 
  complete(year = 2008:max(data_years))  # ensures that there is a row for every year for each region, to ensure there are no gaps
  
setdiff(international_arrivals$year, test$year)
nrow(test)
nrow(international_arrivals)

view(international_arrivals)
...
pre_pandemic_int <-  international_arrivals %>%
  ...
  tidyr::fill(tourism_arrivals_ct, .direction = "down") %>% 
  mutate(
    arrivals_gapfilled = ifelse(is.na(arrivals_method) & !is.na(tourism_arrivals_ct), "gapfilled", arrivals_gapfilled),
    arrivals_method = ifelse(is.na(arrivals_method) & !is.na(tourism_arrivals_ct), "Downfilling", arrivals_method)
         ) 

pre_pandemic_int %>% view()
table(pre_pandemic_int$arrivals_method)
...
length(unique(int_arrivals_gf$rgn_name)) # 156 regions after gapfilling! good, same as `international_arrivals`

summary(int_arrivals_gf) 
table(int_arrivals_gf$arrivals_method) 
colnames(int_arrivals_gf)
...
mean_domestic_guests <- total_arrivals %>% 
  ...
  mutate(
    ...
    tourism_arrivals_ct_dom_r2 = mean(tourism_arrivals_ct_dom,
                                      na.rm = TRUE),
    ...
    tourism_arrivals_ct_int_r2 = mean(tourism_arrivals_ct_int,
                                      na.rm = TRUE),
    tourism_arrivals_ct_int_r2 = ifelse(is.nan(tourism_arrivals_ct_int_r2), NA, tourism_arrivals_ct_int_r2) # to make NaN = NAs
  ) %>%
  ungroup() %>%
  group_by(r1) %>%
  mutate(
    tourism_arrivals_ct_dom_r1 = mean(tourism_arrivals_ct_dom,
                                      na.rm = TRUE),
    tourism_arrivals_ct_dom_r1 = ifelse(is.nan(tourism_arrivals_ct_dom_r1), NA, tourism_arrivals_ct_dom_r1),
    tourism_arrivals_ct_int_r1 = mean(tourism_arrivals_ct_int,
                                      na.rm = TRUE),
    tourism_arrivals_ct_int_r1 = ifelse(is.nan(tourism_arrivals_ct_int_r1), NA, tourism_arrivals_ct_int_r1)
  ) %>%
  ungroup()
...
view(tr_arrivals_props_tourism)
...
new_gf <-read_csv( here::here(current_tr_dir, "int", "tourism_aggregated_gf.csv"))
old_gf <-read_csv( here::here(previous_tr_dir, "intermediate", "tourism_aggregated_gf.csv"))
colnames(old_gf)
view(old_gf)
