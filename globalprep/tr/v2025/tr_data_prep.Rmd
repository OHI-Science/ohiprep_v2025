...
data_years <- 2005:2022 # update with latest data year
...
length(unique(dom_acc_guests_noNA$country)) # 57 (83 now) unique countries, not that many. let's move on to raw gapfilling and see if we can increase those numbers using other columns.
...
summary(lm(total_guests ~ hotel_overnights, data = dom_acc_guests)) # p-value: < 0.00000000000000022, multiple R squared is 0.7702, not bad!! slope is significant, 0.692837.

mod <- lm(total_guests ~ hotel_overnights, data = dom_acc_guests)
summary(mod)

plot(dom_acc_guests$hotel_overnights, dom_acc_guests$total_guests)
abline(log_mod, col="red") # not a linear fit, would be better to do nonlinear, but this is good for now. we can use the slope and intercept from this model to estimate total_guests

log_mod <- lm(log(total_guests) ~ log(hotel_overnights), data = dom_acc_guests)

plot(log(dom_acc_guests$hotel_overnights), log(dom_acc_guests$total_guests))
abline(log_mod, col="red") 
# Log transforming the data improves the fit here
summary(log_mod)
# Significance of the intercept drops from the 0th sig level to the 0.05 sig level.
# But, R2 goes from 0.77 to 0.87

### ggplot version of the above plots
# Regular plot
ggplot(dom_acc_guests, aes(y = total_guests, x = hotel_overnights)) +
  geom_point(alpha = 0.5) +  # Add some transparency to points
  geom_abline(intercept = mod$coefficients['(Intercept)'], slope = mod$coefficients['hotel_overnights']
, color = "red", linetype = "dashed") +
  labs(title = "Domestic Accomodations: total guests and hotel overnights",
       x = "Hotel overnights",
       y = "Total guests") +
  theme_minimal()
...
# Log plot
ggplot(dom_acc_guests, aes(y = log(total_guests), x = log(hotel_overnights))) +
  geom_point(alpha = 0.5) +  
  geom_abline(intercept = log_mod$coefficients['(Intercept)'], 
              slope = log_mod$coefficients['log(hotel_overnights)'], 
              color = "red", linetype = "dashed") +
  labs(title = "Domestic Accomodations: log-transformed total guests and hotel overnights",
       x = "log(Hotel overnights)",
       y = "log(Total guests)") +
  theme_minimal()
```
...
sum(!is.finite(dom_acc_guests_agg$total_guests)) # 3454
sum(!is.finite(dom_acc_guests_agg$hotel_guests)) # 2506
...
mean(!is.na(dom_acc_guests_agg$total_guests)) # 0.2869529, aka 18% NA/total obs
mean(!is.na(dom_acc_guests_agg$hotel_guests)) # 0.482659, aka 30% NA/total obs
...
summary(dom_acc_guests_gf1) # now, `tourism_arrivals_ct` has 2419 NAs, versus 3454 NAs in `total_guests` from the raw clean accommodation data
...
length(unique(domestic_guests$rgn_id))
...
test_2022_int <- international_arrivals %>%
  filter(year %in% c(2020, 2021, 2022)) %>%
  # filter(!is.na(tourism_arrivals_ct)) %>%
  pivot_wider(names_from = year, 
              values_from = tourism_arrivals_ct,
              names_prefix = "int_tourism_") %>%
  mutate(tourism_ct_diff_2022 = (int_tourism_2022 - int_tourism_2020)/int_tourism_2020,
...
gf_2021_tourism_int <- mean(test_2022_int$tourism_ct_diff_2021, na.rm = TRUE) 
gf_2021_tourism_int
# v2024 and v2025: 0.1290837

gf_2022_tourism_int <- mean(test_2022_int$tourism_ct_diff_2022, na.rm = TRUE) 
gf_2022_tourism_int
# v2025: 1.667163 # 2020 tourism compared to 2022 tourism
...
gf_2020_tourism_int <- mean(test_2020_int$tourism_ct_diff, na.rm = TRUE)
gf_2020_tourism_int
...
unwto_all_gf_int <- unwto_gapfill_lm_2019_tourism_int %>%
  ...
  mutate(arrivals_method = ifelse(is.na(arrivals_method) & !is.na(tourism_arrivals_ct), "UNWTO - 2020:2022 gapfill method", arrivals_method)) %>%
  mutate(arrivals_gapfilled = ifelse(arrivals_method == "UNWTO - 2020:2022 gapfill method", "gapfilled", arrivals_gapfilled)) %>%
...
test_2021_dom <- domestic_guests %>%
  ...
  pivot_wider(names_from = year, 
              values_from = tourism_arrivals_ct,
              names_prefix = "int_tourism_") %>%
  mutate(tourism_arrivals_ct_diff_2022 = (int_tourism_2022 - int_tourism_2020)/int_tourism_2020,
         tourism_arrivals_ct_diff_2021 = (int_tourism_2021 - int_tourism_2020)/int_tourism_2020)

# global average increase for 2021 tourism_arrivals_ct
gf_2021_tourism_dom <- mean(test_2021_dom$tourism_arrivals_ct_diff_2021, na.rm = TRUE)
gf_2021_tourism_dom
# v2024: 0.2361595
# v2025: 0.3409155

gf_2022_tourism_dom <- mean(test_2021_dom$tourism_arrivals_ct_diff_2022, na.rm = TRUE)
gf_2022_tourism_dom
# v2025: 1.793485
...
# global average decrease for 2020 toursism
gf_2020_tourism_dom <- mean(test_2020_dom$tourism_arrivals_ct_diff, na.rm = TRUE) 
gf_2020_tourism_dom
# v2024: -0.2984019
# v2025: -0.6833755
...
summary(unwto_all_gf_dom) 
# Still have negative values here...
```
...
mean_domestic_guests <- arrivals_join %>% 
  group_by(rgn_id) %>% 
  complete(year = 2008:2022) %>% # ensures that there is a row for every year for each region, to ensure there are no gaps
  left_join(georegions, by = c("rgn_id","rgn_name")) %>% 
  ungroup() %>% 
  group_by(r2) %>% 
  mutate(
...
colnames(new_tr)
colnames(old_tr)
colSums(is.na(new_tr)) # 133 NAs
colSums(is.na(old_tr)) # 133 NAs

comb_tr <- new_tr %>% 
  rename(new_S_score = S_score) %>% 
  left_join(old_tr, by = join_by(rgn_id, year))

ggplot(comb_tr, aes(x = S_score, y = new_S_score)) +
  geom_point()
```
