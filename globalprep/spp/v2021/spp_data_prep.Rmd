---
title: "OHI 2021 - Species subgoal (Biodiversity goal and resilience)"
...
**Time range**: 1819 - 2021; some protected areas do not have an associated "status year" and are reported as year 0.
...
#2021
n_comp <- raster::raster(here("globalprep/spp/v2021/_output/n_spp_risk_raster_comp.tif"))
...
#2021
n_all <- raster::raster(here("globalprep/spp/v2021/_output/n_spp_risk_raster_all.tif"))
log_n_all <- log(n_all)
...
mean_risk_comp <- raster::raster(here("globalprep/spp/v2021/_output/mean_risk_raster_comp.tif"))

n_comp <- raster::raster(here("globalprep/spp/v2021/_output/n_spp_risk_raster_comp.tif"))

regions_ohi <- raster::raster(here("globalprep/spp/v2021/_spatial/eez_rast.tif"))
...
trend_comp <- raster::raster(here("globalprep/spp/v2021/_output/trend_raster_comp.tif"))

n_trend_comp <- raster::raster(here("globalprep/spp/v2021/_output/n_trend_raster_comp.tif"))
...
write.csv(status, here("globalprep/spp/v2021/output/sp_status_global.csv"), row.names=FALSE)  
...
```{r, eval = FALSE}
status <- read_csv(here("globalprep/spp/v2021/output/sp_status_global.csv"))

old_spp <- read.csv(here("globalprep/spp/v2020/output/sp_status_global.csv")) %>%
...
status <- read.csv(here("globalprep/spp/v2021/output/sp_status_global.csv")) %>%
...
write.csv(trend, here("globalprep/spp/v2021/output/sp_trend_global.csv"), row.names=FALSE)  
...
trend <- read.csv(here("globalprep/spp/v2021/output/sp_trend_global.csv"))  

old_spp <- read.csv(here("globalprep/spp/v2020/output/sp_trend_global.csv")) %>%
  rename(old_score = score) %>%
  left_join(trend) 
...
write.csv(status, here("globalprep/spp/v2021/output/sp_status_global_gf.csv"), row.names=FALSE)          

trend <- read.csv(here("globalprep/spp/v2021/output/sp_trend_global.csv")) %>%
  mutate(gapfilled = ifelse(rgn_id == 232, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 232, "mean of neighbors", NA)) %>%
  dplyr::select(rgn_id, gapfilled, method)
write.csv(trend, here("globalprep/spp/v2021/output/sp_trend_global_gf.csv"), row.names=FALSE)          
...
# saved in Mazu:spp/v2021
projectRaster(risk_x_n, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2021/int/risk_x_n_comp_mol.tif"),
              progress="text")
projectRaster(n_comp, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2021/int/n_comp_mol.tif"),
              progress="text")
...
risk_x_n_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2021/int/risk_x_n_comp_mol.tif"))
n_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2021/int/n_comp_mol.tif"))
...
projectRaster(trend_x_n, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2021/int/trend_x_n_comp_mol.tif"),
              progress="text")
projectRaster(n_trend_comp, rgns, method="ngb", over=TRUE, 
              filename=file.path(dir_M, "git-annex/globalprep/spp/v2021/int/n_trend_comp_mol.tif"),
              progress="text")
...
trend_3nm_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2021/int/trend_x_n_comp_mol.tif"))
n_trend_3nm_mol <- raster(file.path(dir_M, "git-annex/globalprep/spp/v2021/int/n_trend_comp_mol.tif"))
...
# get eez value:
eez_status <- read.csv(here("globalprep/spp/v2021/output/sp_status_global.csv")) %>%
  filter(rgn_id == 19) %>%
...
write.csv(resilience, here("globalprep/spp/v2021/output/sp_status_3nm.csv"), row.names=FALSE)  

```

### Resilience: save a record of gapfilling
```{r, eval = FALSE}
res <- read.csv(here("globalprep/spp/v2021/output/sp_status_3nm.csv")) %>%
  mutate(gapfilled = ifelse(rgn_id == 19, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 19, "eez scale data used", NA)) %>%
  dplyr::select(rgn_id, year, gapfilled, method)
write.csv(res, here("globalprep/spp/v2021/output/sp_status_3nm_gf.csv"), row.names=FALSE)          
...
status <- read.csv(here("globalprep/spp/v2021/output/sp_status_3nm.csv")) %>%
...
old_spp <- read.csv(here("globalprep/spp/v2020/output/sp_status_3nm.csv")) %>%
  filter(year == max(year)) %>%
  rename(old_score = score) %>%
  mutate(year = 2020) %>%
  left_join(status) %>%
