...
# The below file paths did not work in the 2025 assessment, something to do with using here(). Manually gave path above. 
...
# Check that max year is either the current or previous year
# currently 2025, checks out!
...
calc_areas <- function(stats_df) {
  area_df <- stats_df %>%
    ...
    mutate(n_cells_tot = sum(n_cells),
           a_tot_km2   = n_cells_tot / 4) %>% ## From the 500m * 500m calculation
...
library(dplyr)
# gap fill for eez
res_eez <- read_csv(file.path(dir_goal, 'output', "mpa_eez_resilience.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(res_eez, file.path(dir_goal, 'output', "mpa_eez_resilience_gf.csv"), row.names=FALSE)

# gap fill for 3nm
res_3nm <- read_csv(file.path(dir_goal, 'output', "mpa_3nm_resilience.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(res_3nm, file.path(dir_goal, 'output', "mpa_3nm_resilience_gf.csv"), row.names=FALSE)

# gap fill for inland 1km
inland <- read_csv(file.path(dir_goal, 'output', "lsp_prot_area_inland1km.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(inland, file.path(dir_goal, 'output', "lsp_prot_area_inland1km_gf.csv"), row.names=FALSE)

# gap fill for offshore 3nm
offshore <- read_csv(file.path(dir_goal, 'output', "lsp_prot_area_offshore3nm.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(offshore, file.path(dir_goal, 'output', "lsp_prot_area_offshore3nm_gf.csv"), row.names=FALSE)
...
Plot scores for 2024 vs 2025 assessment years

-   2024 AY data goes to 2024
-   2025 AY data goes to 2025 (downloaded June 2025) 


```{r plot_scores_vs_v2025, plotly = TRUE, eval = FALSE}
...
ggsave(file.path(dir_goal, 'figs/plot_v2024_v2025.png'),
...
write.csv(mjr_score_change, file.path(dir_goal, 'output/major_changes_2025.csv'))
```
