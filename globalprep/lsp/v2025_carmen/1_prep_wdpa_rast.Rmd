...
View 2_lsp_data_prep.Rmd for full description of Lasting Special Places data preparation.

# Methods

## Downloading Data

Accessing and downloading the data was difficult in 2023 due to a downloading bug, luckily there are multiple ways to download the data from the webpage. The below directions sound easy; but it is easy to be navigated to a page where the download functionality is broken.
...
4: There will be additional zip files within the zip file you download. Once unzipped, you will use the file containing polygons throughout the LSP dataprep.
...
Changed data source to 2025. All polygons are in one shapefile, so changes were made throughout the prep document to reflect this. 

Saving intermediates to `/home/shares/ohi/git-annex/globalprep/lsp/v2025/rast`
...
dir_data <- file.path(dir_M, 'git-annex', 'globalprep', '_raw_data',
                      'wdpa_mpa', 'd2025', 'WDPA_May2025_Public_shp')
# June data
# dir_data <- file.path(dir_M, 'git-annex', 'globalprep', '_raw_data',
...
# create path to polygon shapefile downloaded from source
shp_raw <- list.files(path = file.path(dir_data, 'WDPA_May2025_Public_shp_2'), pattern = "polygons.shp", full.names = TRUE)

# read in polygon shapefile
wdpa_poly_object <- st_read(shp_raw, stringsAsFactors = FALSE)
...
wdpa_poly_object$STATUS %>% unique()
...
if(nrow(x) == 0) {
  print("No non-mpa program in management plan column (good).")
} else {
  print(paste0("Found ", nrow(x), " non-mpa programs (bad)."))
}
```
...
wdpa_poly <- tidy_wdpa_data(wdpa_poly_object)

unique(wdpa_poly$mang_plan)
...
# created filepaths for the file, make sure their names aligned with your year's dates
shp_reorder <- file.path(dir_data, 'shps_ch', 'WDPA_May2025_shp_ordered') # fix to "shps"

# make new filepath if not already made
if (!dir.exists(shp_reorder)) {
  dir.create(shp_reorder, recursive = TRUE)
  message("Directory created: ", shp_reorder)
} else {
  message("Directory already exists: ", shp_reorder)
}

# write the shapefile to the raw_data Mazu folder, warning suppressed, takes about 2 minutes.
suppressWarnings(st_write(wdpa_poly,
                          dsn = shp_reorder,
...
2025 Note: We reverted back to a single shapefile. 

```{r transform_poly}

# file paths for shape files we just wrote to Mazu
shp_reorder <- file.path(dir_data, 'shps_ch', 'WDPA_May2025_shp_ordered') # change to "shps"

# read in those shape files as sf objects to ensure they were written correctly and to use below
wdpa_poly <- st_read(dsn = shp_reorder, 
                    stringsAsFactors = FALSE)

# transform CRS to Mollweide
wdpa_poly_xformed <- st_transform(wdpa_poly, 
                                      crs = '+proj=moll +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs')

# validate that the CRS has changed successfully
st_crs(wdpa_poly_xformed)

# convert wdpaid to character to save (too long to save as an integer)
wdpa_poly_xformed$wdpaid <- as.character(wdpa_poly_xformed$wdpaid)

# set file path for writing, be sure to change these to the correct titles for saving
shp_xformed <- file.path(dir_data, 'shps_ch', 'WDPA_May2025_shp_xformed') # replace month and year with the appropriate month and year

# write the transformed files to Mazu, this will likely take ~2 mins
st_write(wdpa_poly_xformed,
         dsn = shp_xformed,
         driver = "ESRI Shapefile",
         append = TRUE)
```
...
rast_wdpa_file <- file.path(dir_goal_anx,
                            'rast',
                            'wdpa_2025_moll_500m.tif') #update year
...
if(!file.exists(rast_wdpa_file)) {
  ...
  shp_xformed_file <- file.path(dir_data, 'shps_ch',
                                  'WDPA_May2025_shp_xformed', # added this line because we created nested folders to house each shapefile earlier
                                  'WDPA_May2025_shp_xformed.shp') 

  # create time stamp
  ptm <- proc.time()

  # read in data as SpatVectors with terra
  wdpa_poly <- vect(shp_xformed_file)
  ...
  rast_wdpa <- terra::rasterize(wdpa_poly, 
                                rast_base, 
                                field = 'status_yr', 
                                fun = 'min')
...
# you are encouraged to use this space to examine the raster created from the WDPA data 
...
Past raster seems to cover much more area than current. Potential effect of data download restructuring?
...
wdpa_poly_24_0 <- st_read(dsn = shp_reorder_24_0, 
                          #layer = basename(shp_reorder_24_0),
                          stringsAsFactors = FALSE)
wdpa_poly_24_1 <- st_read(dsn = shp_reorder_24_1, 
                          #layer = basename(shp_reorder_24_1),
                          stringsAsFactors = FALSE)
wdpa_poly_24_2 <- st_read(dsn = shp_reorder_24_2, 
...
# read in those shape files as sf objects to ensure they were written correctly and to use below
wdpa_poly_current <- st_read(dsn = shp_reorder, 
                    stringsAsFactors = FALSE)

# check colnames
colnames(wdpa_poly_all_previous) == colnames(wdpa_poly_current)

# check names
unique(wdpa_poly_current$orig_name)
unique(wdpa_poly_all_previous$orig_name)
# unique(wdpa_poly_all_previous$parent_iso)

unique(wdpa_poly_current$parent_iso)
unique(wdpa_poly_current$iso)

# test area to see if things look right
test_area_current <- wdpa_poly_current %>%
...
sum(st_area(test_area_current)) # 430,905,669 m2
sum(st_area(test_area_previous)) # 462,674,854 m2
...
- filtering to parent_iso == "GBR" and iso3 == "PCN" resulted in no data for the previous year's lsp data. 

2025 Note: 

- Same, so switched to iso3 == "IMN"
- Area decreased from 462,674,854 m2 in 2024 to 430,905,669 m2 in 2025
