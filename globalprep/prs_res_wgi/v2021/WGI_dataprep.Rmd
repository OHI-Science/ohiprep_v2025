---
title: 'OHI 2021 - World Governance Indicator'
...
[REFERENCE RMD FILE](http://ohi-science.org/ohiprep_v2021/globalprep/prs_res_wgi/v2021/WGI_dataprep.html)
...
Consider this improvement for future assessments: Explore a better way to gapfill... based on a linear model (~line 166)
...
April 29, 2021 (source data updated Sep 25 2020)

**Description**:  
The Worldwide Governance Indicators (WGI) project reports aggregate and individual governance indicators for 215 economies over the period 1996–2019, for six dimensions of governance:
...
date <- Sys.Date()
write.csv(d, here(sprintf('globalprep/prs_res_wgi/v2021/raw/worldbank_wgi_from_wdi_api_%s.csv', date)), row.names=FALSE)
...
d <- read.csv(here('globalprep/prs_res_wgi/v2021/raw/worldbank_wgi_from_wdi_api_2021-04-29.csv')) # change appended date so that it only changes if you do so actively
...
(NOTE: for the 2021 assessment all regions had at least 4 of the 6 indicators).
...
# summary(d_calcs) - checking to make sure NA values make sense
  ## v2021: NA_start max = 6, NA_post_gf_1 = 3

## v2021: score_wgi_sclae min = -2.44, ma = 1.95... perfect
...
write.csv(d_calcs %>%
          ...
          here('globalprep/prs_res_wgi/v2021/intermediate/wgi_combined_scores_by_country.csv'),
          row.names = FALSE)
...
population_weights <- data.frame(country = c("Virgin Islands (U.S.)", "Puerto Rico",
                                 ...
                                 population = c(106670, 3193690,
                                         1397715000, 7507400, 640450))
# updated population values on 29 April 2021 (source: World Bank website, 2019 values)
...
new2018 <- d_gf2 %>%
  filter(year==2018) %>% # update to most recent shared year for comparison
  select(rgn_id, score)

old2018 <- read.csv(here('globalprep/prs_res_wgi/v2020/output/wgi_res.csv')) %>% # change filepath to the previous assessment year's, rename variable for clarity
  filter(year == 2018) %>%
  select(rgn_id, old_score=resilience_score) %>%
  full_join(new2018)


score_compare_plot <- ggplot(old2018, aes(x = old_score, y = score, text = rgn_id)) +
  geom_point() +
...
write.csv(tmp_data_res, here("globalprep/prs_res_wgi/v2021/output/wgi_res.csv"), row.names=FALSE)

tmp_data_prs <- d_gf2 %>%
  mutate(score = 1 - score) %>%
  select(rgn_id, year, pressure_score = score)
write.csv(tmp_data_prs, here("globalprep/prs_res_wgi/v2021/output/wgi_prs.csv"), row.names=FALSE)
...
write.csv(tmp_gf, here("globalprep/prs_res_wgi/v2021/output/wgi_gf.csv"), row.names=FALSE)
  # change format to match data SOP, add method column


```

Checking on outliers in 2021 assessment:
```{r outliers, eval=FALSE}

## Do this after updating in global

worldbank_wgi_from_wdi_api <- read_csv("globalprep/prs_res_wgi/v2021/raw/worldbank_wgi_from_wdi_api_2021-04-29.csv")

worldbank_2016 <- worldbank_wgi_from_wdi_api %>% 
  filter(year == 2016) %>% 
  filter(country == "Australia" | country == "Marshall Islands")

# Compare to data from 2020:
worldbank_wgi_from_wdi_api_2020 <- read_csv("globalprep/prs_res_wgi/v2020/raw/worldbank_wgi_from_wdi_api_2020-06-23.csv")

worldbank_old <- worldbank_wgi_from_wdi_api_2020 %>% 
  filter(year == 2016) %>% 
  filter(country == "Australia" | country == "Marshall Islands")
