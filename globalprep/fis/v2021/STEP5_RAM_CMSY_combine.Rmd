...
old <- read_csv(file.path("../v2021/archive/fis_bbmsy_gf.csv"))
...
## check against old 
data_gf_old<- read.csv("archive/fis_bbmsy_gf.csv") 
...
old_pit <- old %>%
  filter(rgn_id == 146)
mean(old_pit$bbmsy, na.rm = TRUE) # 1.140603

new_pit <- new %>%
  filter(rgn_id == 146)
mean(new_pit$bbmsy, na.rm = TRUE) # 1.138512
