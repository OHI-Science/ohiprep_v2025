...
previous_current_unwto_dir <- here(raw_data_dir, "UNWTO", previous_data_dir)

# Tourism and recreation directory
current_tr_dir <- here("globalprep", "np", version_year)
previous_tr_dir <- here("globalprep", "np", previous_vyear)
```
...
unwto_clean <- unwto_arrivals_int %>% 
  select(country = `Basic data and indicators`, total_arrivals = `...6`, subdivision_1 = `...7`, subdivision_2 = `...8`, `1995`:`2022`) %>% # UPDATE MAX DATA YEAR WITH NEW DATA !!
  ...
  select(-name) %>% # get rid of the name column since it's reduncant
  ...
  # rename metrics so easier to work with and fix types
...
length(unique(unwto_clean$country)) # 223 

# ---- determine if there were any duplicates due to name_2_rgn ----

colSums(is.na(unwto_clean_names_int)) # v2025: 861 NAs in tourism_arrivals_ct
length(unwto_clean_names_int$tourism_arrivals_ct) # within 5012 observations for `tourism_arrivals_ct`
...
length(unique(international_arrivals_rgns$rgn_id)) # v2025: 156 regions!
...
length(unique(international_arrivals$rgn_id)) # v2025: 156 regions! great
...
unwto_clean_dom <- unwto_arrivals_dom_acc %>% 
  select(country = `Basic data and indicators`, source_arrivals = `...6`, subdivision_1 = `...7`, subdivision_2 = `...8`, `1995`:`2022`) %>% # UPDATE MAX DATA YEAR WITH NEW DATA !!
...
# total_guests     total_overnights  
#  Min.   :    2.6   Min.   :    23.1  
#  1st Qu.:  441.0   1st Qu.:  1761.0  
#  Median : 1532.0   Median :  6682.5  
#  Mean   : 6012.3   Mean   : 25975.7  
#  3rd Qu.: 5560.0   3rd Qu.: 19467.5  
#  Max.   :72370.0   Max.   :355059.0  
#  NA's   :4539      NA's   :4108      
#   hotel_guests      hotel_overnights  
#  Min.   :     0.3   Min.   :     0.1  
#  1st Qu.:   191.0   1st Qu.:   655.0  
#  Median :   785.0   Median :  2646.0  
#  Mean   :  4187.6   Mean   : 13713.0  
#  3rd Qu.:  3389.0   3rd Qu.: 10139.0  
#  Max.   :116266.0   Max.   :330080.0  
#  NA's   :3281       NA's   :3087 
...
abline(mod, col="red") # not a linear fit, would be better to do nonlinear, but this is good for now. we can use the slope and intercept from this model to estimate total_guests
...
unwto_dom_trips_clean <- unwto_arrivals_dom_trips %>% 
  select(country = `Basic data and indicators`, total_trips = `...6`, subdivision_1 = `...7`, subdivision_2 = `...8`, `1995`:`2022`) %>% # UPDATE MAX DATA YEAR WITH NEW DATA !!
  fill(country, .direction = "down") %>% # add country name to all data associated with that country
