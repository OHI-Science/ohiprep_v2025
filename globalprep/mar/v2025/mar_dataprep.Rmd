...
## Combine 2024 assessment SFW data
...
sw_sus_rgn_comb <- sw_sus_rgn_comb %>% 
  select(rgn_id, species, score_new, method, rec, water_body, score_old,
         ...
         rgn_name_old = rgn_name.y, 
         # vyear_new = vyear.x, vyear_old = vyear.y,
...
sw_sus_rgn_filt <- sw_sus_rgn_clean %>%
  select(rgn_id, species, method, water_body, score_new, score_old, escapes_new, escapes_old, 
         # vyear_old, vyear_new, 
         rgn_name = rgn_name_new, country = country_new, scientific_name = scientific_name_new)
view(sw_sus_rgn_filt)
...
length(unique(maric$species_code)) # 1350
...
sw_sp_c <- sw_sus_rgn %>%
  ...
  # select(rgn_id, species, score_old, score_new, escapes_old, escapes_new)
  select(rgn_id, species, Sust_sw_sp_c_old = score_old, esc_sw_sp_c_old = escapes_old, Sust_sw_sp_c_new = score_new, esc_sw_sp_c_new = escapes_new)
...
sw_sp_w <- gf_sw_sp_w %>%
  select(rgn_id, species, Sust_sw_sp_w_old = score_old, esc_sw_sp_w_old = escapes_old, Sust_sw_sp_w_new = score_new, esc_sw_sp_w_new = escapes_new)
...
sw_sp_g <- sw_sus_rgn %>% 
  ...
  select(species, Sust_sw_sp_g_old = score_old, esc_sw_sp_g_old = escapes_old, Sust_sw_sp_g_new = score_new, esc_sw_sp_g_new = escapes_new)
...
nrow(test3) # 2838
sum(is.na(test3$Sust_sw_sp_c_new)) # 2838
# All sust_sw_sp_c and esc_sw_sp_c scores are NA for this test...

# Checking for the entire data frame now
nrow(mar_sw_sus) # 60779
sum(is.na(mar_sw_sus$Sust_sw_sp_c_new)) # 49319
sum(is.na(mar_sw_sus$esc_sw_sp_c_new)) # 49319
...
```{r}
# Pivot longer so data is in tidy format
mar_sw_sus_long <- mar_sw_sus %>%
  pivot_longer(
    cols = c(
      Sust_sw_sp_c_old, Sust_sw_sp_c_new,
      esc_sw_sp_c_old,  esc_sw_sp_c_new,
      Sust_sw_sp_w_old, Sust_sw_sp_w_new,
      esc_sw_sp_w_old,  esc_sw_sp_w_new,
      Sust_sw_sp_g_old, Sust_sw_sp_g_new,
      esc_sw_sp_g_old,  esc_sw_sp_g_new
    ),
    names_to = c(".value", "version"),
    names_sep = "_(?=(old|new)$)"  # regex lookahead to split before 'old' or 'new'
  ) 

mar_sw_sus <- mar_sw_sus_long %>% 
  mutate(version = case_when(version == "old" ~ 2024,
                             version == "new" ~ 2025)) %>% 
  rename(vyear = version)
```
...
# mar_sw_sus <- mar_sw_sus %>% 
#   dplyr::mutate(Sust_new = ifelse(!is.na(Sust_sw_sp_c_new), Sust_sw_sp_c_new, Sust_sw_sp_w_new)) %>%
#   dplyr::mutate(Sust_new = ifelse(is.na(Sust_new), Sust_sw_sp_g_new, Sust_new)) %>%
#   dplyr::mutate(gapfill_sus_new = case_when(Sust_sw_sp_c_new == Sust_new ~ "none",
#                                             is.na(Sust_sw_sp_c_new) & Sust_sw_sp_w_new == Sust_new ~ "water_body",
#                                             is.na(Sust_sw_sp_c_new) & is.na(Sust_sw_sp_w_new) & Sust_sw_sp_g_new == Sust_new ~ "global_species")) %>%
#   dplyr::mutate(esc_score_new = ifelse(!is.na(esc_sw_sp_c_new), esc_sw_sp_c_new, esc_sw_sp_w_new)) %>%
#   dplyr::mutate(esc_score_new = ifelse(is.na(esc_score_new), esc_sw_sp_g_new, esc_score_new)) %>%
#   dplyr::mutate(gapfill_esc_new = case_when(esc_sw_sp_c_new == esc_score_new ~ "none",
#                                             is.na(esc_sw_sp_c_new) & esc_sw_sp_w_new == esc_score_new ~ "water_body",
#                                             is.na(esc_sw_sp_c_new) & is.na(esc_sw_sp_w_new) & esc_sw_sp_g_new == esc_score_new ~ "global_species")) %>%
# 
#   dplyr::mutate(Sust_old = ifelse(!is.na(Sust_sw_sp_c_old), Sust_sw_sp_c_old, Sust_sw_sp_w_old)) %>%
#   dplyr::mutate(Sust_old = ifelse(is.na(Sust_old), Sust_sw_sp_g_old, Sust_old)) %>%
#   dplyr::mutate(gapfill_sus_old = case_when(Sust_sw_sp_c_old == Sust_old ~ "none",
#                                             is.na(Sust_sw_sp_c_old) & Sust_sw_sp_w_old == Sust_old ~ "water_body",
#                                             is.na(Sust_sw_sp_c_old) & is.na(Sust_sw_sp_w_old) & Sust_sw_sp_g_old == Sust_old ~ "global_species")) %>%
#   dplyr::mutate(esc_score_old = ifelse(!is.na(esc_sw_sp_c_old), esc_sw_sp_c_old, esc_sw_sp_w_old)) %>%
#   dplyr::mutate(esc_score_old = ifelse(is.na(esc_score_old), esc_sw_sp_g_old, esc_score_old)) %>%
#   dplyr::mutate(gapfill_esc_old = case_when(esc_sw_sp_c_old == esc_score_old ~ "none",
#                                             is.na(esc_sw_sp_c_old) & esc_sw_sp_w_old == esc_score_old ~ "water_body",
#                                             is.na(esc_sw_sp_c_old) & is.na(esc_sw_sp_w_old) & esc_sw_sp_g_old == esc_score_old ~ "global_species")) %>%
#   dplyr::select(-c(Sust_sw_sp_c_new, Sust_sw_sp_w_new, Sust_sw_sp_g_new,
#                    esc_sw_sp_c_new, esc_sw_sp_w_new, esc_sw_sp_g_new,
#                    Sust_sw_sp_c_old, Sust_sw_sp_w_old, Sust_sw_sp_g_old,
#                    esc_sw_sp_c_old, esc_sw_sp_w_old, esc_sw_sp_g_old))
...
# 37376 NA values out of 76000 total obs: ~ 49% without scores (2025)
```
...
mar_harvest_tonnes <- mar_sw_sus_final %>%
  group_by(rgn_id, taxa_code, year, taxa_group, family) %>% 
  summarise(tonnes = first(tonnes)) %>% 
  ungroup() %>% 
...
mar_harvest_tonnes_gf <- mar_sw_sus_final %>%
  group_by(rgn_id, taxa_code, year, taxa_group, family) %>% 
  summarise(gapfill_fao = first(gapfill_fao)) %>% 
  ungroup() %>% 
