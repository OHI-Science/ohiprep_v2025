...
We checked the IUCN_CAT variable as a possible way to weight the score, but found that these are unranked categories. They represent styles or type of management. 
...
dir_data <- file.path(dir_M, 'git-annex', 'globalprep', '_raw_data',
                      'wdpa_mpa', 'd2025', 'WDPA_Jun2025_Public_shp')
...
shp_raw_0 <- file.path(dir_data, 'WDPA_Jun2025_Public_shp_0', 'WDPA_Jun2025_Public_shp-polygons')
shp_raw_1 <- file.path(dir_data, 'WDPA_Jun2025_Public_shp_1', 'WDPA_Jun2025_Public_shp-polygons')
shp_raw_2 <- file.path(dir_data, 'WDPA_Jun2025_Public_shp_2', 'WDPA_Jun2025_Public_shp-polygons')
...
print(paste0("Non-MPA Programs in x_0: ", nrow(x_0)))
print(paste0("Non-MPA Programs in x_1: ", nrow(x_1)))
print(paste0("Non-MPA Programs in x_2: ", nrow(x_2)))

# remove unneeded objects
rm(df_list, x_0, x_1, x_2)
```

**Issue:** There are Non-MPA Programs present in the WDPA polygons. For now, I've gone ahead and dropped these rows because each had <50 out of ~ 100K observations. These all seem to be in the U.S.

**After discussion**: These get filtered out on their own later on in the organize and clean code chunk.

```{r}
# Filter out non-mpa
wdpa_poly_0 <- wdpa_poly_0 %>%
  filter(!str_detect(tolower(MANG_PLAN), 'non-mpa program')) 

wdpa_poly_1 <- wdpa_poly_1 %>%
  filter(!str_detect(tolower(MANG_PLAN), 'non-mpa program'))

wdpa_poly_2 <- wdpa_poly_2 %>%
  filter(!str_detect(tolower(MANG_PLAN), 'non-mpa program'))
...
# Note 2025: Changed output directory from _raw_data to dir_goal_anx
shp_reorder_0 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_ordered_0')
shps_reorder_0 <- file.path('/home/shares/ohi/git-annex/globalprep/lsp/v2025/clean_shapes/WDPA_Jun2025_shp_ordered_0')
# replace month and year with the appropriate month and year
shp_reorder_1 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_ordered_1') # replace month and year with the appropriate month and year
shp_reorder_2 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_ordered_2') # replace month and year with the appropriate month and year
...
shp_reorder_0 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_ordered_0')
shp_reorder_1 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_ordered_1')
shp_reorder_2 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_ordered_2')
...
shp_xformed_0 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_xformed_0') # replace month and year with the appropriate month and year
shp_xformed_1 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_xformed_1') # replace month and year with the appropriate month and year
shp_xformed_2 <- file.path(dir_goal_anx, 'shps', 'WDPA_Jun2025_shp_xformed_2') # replace month and year with the appropriate month and year
...
# reminder, dir_goal_anx is git-annex/globalprep/lsp/v2025
rast_wdpa_file <- file.path(dir_goal_anx,
                            'rast',
                            'wdpa_2025_moll_500m.tif') 
...
if(!file.exists(rast_wdpa_file)) {
  ...
  # replace month and year with the appropriate month and year
  shp_xformed_file_0 <- file.path(dir_goal_anx, 'shps',
                                  'WDPA_Jun2025_shp_xformed_0', # added this line because we created nested folders to house each shapefile earlier
                                  'WDPA_Jun2025_shp_xformed_0.shp') 
  shp_xformed_file_1 <- file.path(dir_goal_anx, 'shps',
                                  'WDPA_Jun2025_shp_xformed_1',
                                  'WDPA_Jun2025_shp_xformed_1.shp') 
  shp_xformed_file_2 <- file.path(dir_goal_anx, 'shps',
                                  'WDPA_Jun2025_shp_xformed_2',
                                  'WDPA_Jun2025_shp_xformed_2.shp') 
...
check_past <- terra::rast(file.path("/home/shares/ohi/git-annex/globalprep/lsp/v2024/rast/wdpa_2024_moll_500m.tif"))

# visualize both rasters
plot(check_current, col = 'blue')
plot(check_past, col = 'blue')

# Check the difference between the two years
check_current_abs <- ifel(is.na(check_current), 0, 1)
check_past_abs <- ifel(is.na(check_past), 0, 1) 

compare <- check_past_abs - check_current_abs 

plot(compare, col = "blue")
```


2025: Some new large MPAs off the Northwestern coast of Australia. Various other smaller changes. On the whole, it looks like slightly more area this year compared to last year. 

## Data Checking

Compare shapefile from last year vs this year (2024 v 2025).
...
shp_reorder_24_0 <- file.path('/home/shares/ohi/git-annex/globalprep/_raw_data/wdpa_mpa/d2024/WDPA_May2024_Public_shp/shps/WDPA_May2024_shp_ordered_0')
shp_reorder_24_1 <- file.path('/home/shares/ohi/git-annex/globalprep/_raw_data/wdpa_mpa/d2024/WDPA_May2024_Public_shp/shps/WDPA_May2024_shp_ordered_1')
shp_reorder_24_2 <- file.path('/home/shares/ohi/git-annex/globalprep/_raw_data/wdpa_mpa/d2024/WDPA_May2024_Public_shp/shps/WDPA_May2024_shp_ordered_2')


# read in previous data
# These lines did not run during the 2025 assessment! See below code chunk
wdpa_poly_24_0 <- st_read(dsn = dirname(shp_reorder_24_0), 
                          layer = basename(shp_reorder_24_0),
                          stringsAsFactors = FALSE)
wdpa_poly_24_1 <- st_read(dsn = dirname(shp_reorder_24_1), 
                          layer = basename(shp_reorder_24_1),
                          stringsAsFactors = FALSE)
wdpa_poly_24_2 <- st_read(dsn = dirname(shp_reorder_24_2), 
                          layer = basename(shp_reorder_24_2),
                          stringsAsFactors = FALSE)
```

```{r}
# Had an error reading in the 2024 data. Decided to simplify it for troubleshooting
wdpa_poly_24_0 <- st_read("/home/shares/ohi/git-annex/globalprep/_raw_data/wdpa_mpa/d2024/WDPA_May2024_Public_shp/shps/WDPA_May2024_shp_ordered_0/WDPA_May2024_shp_ordered_0.shp")
wdpa_poly_24_1 <- st_read("/home/shares/ohi/git-annex/globalprep/_raw_data/wdpa_mpa/d2024/WDPA_May2024_Public_shp/shps/WDPA_May2024_shp_ordered_1/WDPA_May2024_shp_ordered_1.shp")
wdpa_poly_24_2 <- st_read("/home/shares/ohi/git-annex/globalprep/_raw_data/wdpa_mpa/d2024/WDPA_May2024_Public_shp/shps/WDPA_May2024_shp_ordered_2/WDPA_May2024_shp_ordered_2.shp")


# bind all into one
wdpa_poly_all_previous <- rbind(wdpa_poly_24_0, wdpa_poly_24_1, wdpa_poly_24_2)
...
# 2025 843725668865 m2
```

2024 Note: filtering to parent_iso == "GBR" and iso3 == "PCN" resulted in no data for the previous year's lsp data. 

2025: Test areas look good!
area: 843725668865 m^2 **Check that area of 3 individual shapefiles matches the total raster area**