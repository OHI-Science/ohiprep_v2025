...
dir_maddy <- file.path(dir_anx, goal, 'v2025', 'int_maddy')

if (!dir.exists(dir_maddy)) {
  dir.create(dir_maddy)
}

# list intermediate file paths
zonal_files <- c('zonal_3nm' =  file.path(dir_maddy, 'zonal_stats_3nm.csv'),
                 'zonal_1km' =  file.path(dir_maddy, 'zonal_stats_1km.csv'),
                 'zonal_eez' =  file.path(dir_maddy, 'zonal_stats_eez.csv'))


# load raster created in 1_prep_wdpa_rast.rmd
rast_wdpa <- terra::rast(file.path(dir_goal_anx, 'rast', 'wdpa_2025_moll_500m.tif'))
...
write_csv(prot_3nm, file.path(dir_maddy, 'area_protected_3nm.csv'))
write_csv(prot_1km, file.path(dir_maddy, 'area_protected_1km.csv'))
write_csv(prot_eez, file.path(dir_maddy, 'area_protected_eez.csv'))
```
...
prot_3nm <- read_csv(file.path(dir_maddy, 'area_protected_3nm.csv')) %>%
  rename(area = a_tot_km2,
         a_prot_3nm = a_prot_km2)
prot_1km <- read_csv(file.path(dir_maddy, 'area_protected_1km.csv')) %>%
  rename(area = a_tot_km2,
         a_prot_1km = a_prot_km2)
...
dir_maddy_2 <- file.path(dir_anx, goal, 'v2025', 'output_maddy')

if (!dir.exists(dir_maddy_2)) {
  dir.create(dir_maddy_2)
}
...
write_lsp_layer <- function(df, layers, layername) {
  df1 <- df[ , c('rgn_id', layers)] %>%
    filter(rgn_id <= 250) %>%
    distinct()
  write_csv(df1, file.path(dir_maddy_2, paste0(layername, '.csv')))
}
...
resil_3nm <- read_csv(file.path(dir_maddy, 'area_protected_3nm.csv')) %>%
  mutate(resilience.score = (a_prot_km2 / a_tot_km2) / area_ref,
         resilience.score = ifelse(resilience.score > 1, 1, resilience.score))

resil_eez <- read_csv(file.path(dir_maddy, 'area_protected_eez.csv')) %>%
  mutate(resilience.score = (a_prot_km2 / a_tot_km2) / area_ref,
         resilience.score = ifelse(resilience.score > 1, 1, resilience.score))
...
## Save resilience scores for 3 nm and EEZ data in output
  ...
  write_csv(tmp_3nm, file.path(dir_maddy_2, "mpa_3nm_resilience.csv"))

  tmp_eez <- resil_eez %>%
    dplyr::select(rgn_id, year, resilience.score)
  write_csv(tmp_eez, file.path(dir_maddy_2, "mpa_eez_resilience.csv"))
...
library(dplyr)
# gap fill for eez
res_eez <- read.csv(file.path(dir_maddy_2, "mpa_eez_resilience.csv")) %>%
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(res_eez, file.path(dir_maddy_2, "mpa_eez_resilience_gf.csv"), row.names=FALSE)

# gap fill for 3nm
res_3nm <- read.csv(file.path(dir_maddy_2, "mpa_3nm_resilience.csv")) %>%
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(res_3nm, file.path(dir_maddy_2, "mpa_3nm_resilience_gf.csv"), row.names=FALSE)

# gap fill for inland 1km
inland <- read.csv(file.path(dir_maddy_2, "lsp_prot_area_inland1km.csv")) %>%
  mutate(a_prot_1km = 0) %>% 
  rename(gapfilled = a_prot_1km)

write.csv(inland, file.path(dir_maddy_2, "lsp_prot_area_inland1km_gf.csv"), row.names=FALSE)

# gap fill for offshore 3nm
offshore <- read.csv(file.path(dir_maddy_2, "lsp_prot_area_offshore3nm.csv")) %>%
  mutate(a_prot_3nm = 0)%>% 
  rename(gapfilled = a_prot_3nm)

write.csv(offshore, file.path(dir_maddy_2, "lsp_prot_area_offshore3nm_gf.csv"), row.names=FALSE)
```
...
status_3nm_new <- read_csv(file.path(dir_maddy_2, 'lsp_prot_area_offshore3nm.csv')) %>%
  full_join(read_csv(file.path(dir_maddy_2, 'rgn_area_offshore3nm.csv')),
            by = 'rgn_id') %>%
...
# The scores for 2024 could not be pulled up, so 2023 was used
