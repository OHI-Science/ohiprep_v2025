...
mean_catch_FOFM <- mean_catch %>%
  ...
  data.frame() %>%
  # It seems like the catch here is NOT the mean catch? The file name it get's saved as is misleading...
  rename(catch = catch_non_human)

readr::write_csv(mean_catch_FOFM, here(current_np_dir, "int", "mean_catch_FOFM.csv"))


##### Updated data frame v2025 #####
np_tonnes_fofm_full <- mean_catch %>%
  mutate(stock_id_taxonkey = paste(stock_id, TaxonKey, sep="_")) %>%
  dplyr::select(rgn_id, stock_id_taxonkey, year, catch_non_human, mean_catch_non_human) %>%
  filter(year >= 2001) %>%  # filter to include only analysis years
  data.frame()

readr::write_csv(np_tonnes_fofm_full, here(current_np_dir, "int", "np_tonnes_fofm_full.csv"))
```
