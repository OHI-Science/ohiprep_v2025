  ...
  for (yr in years){
    for (mo in months){ 
      ...
      if (!file.exists(pts.shp)){
        ...
        writeRaster(r_type, 
                    file.path(dir_M, sprintf("git-annex/globalprep/_raw_data/NSIDC_SeaIce/v2021/tmp/%s_type.tif", p)),
                    overwrite=T) # r_type in memory, read r.typ from file for increased speed
        
        
        r.typ <- raster(file.path(dir_M, sprintf("git-annex/globalprep/_raw_data/NSIDC_SeaIce/v2021/tmp/%s_type.tif",p)))
