---
title: "OHI `r format(Sys.Date(), '%Y')`: Food Provision, Combining RAM and CMSY B/Bmsy data"
...
- New data (from steps 4a/4b) added in v2025
...
**2025 update**: This file was copied over from the 2024 assessment because steps 1-3 were not run this year.
...
## v2025 Gapfill the mean catch and CMSY data up to 2023

Use the data from 2019 and upfill to 2023. Not very satisfying, but the best workaround for now. This is necessary because the 2025 assessment RAM data goes up until 2023. 

```{r}
# Get the last known value in 2019 for each stock
upfill_catch <- mean_catch %>%
  filter(year == 2019, !is.na(mean_catch)) %>%
  select(stock_id, mean_catch, rgn_id, taxon_key) %>%
  crossing(year = 2020:2023) %>%
  mutate(gapfill_catch = "upfilling")

# Add a gapfilled column to the original data
mean_catch <- mean_catch %>%
  mutate(gapfill_catch = "none")

# Combine original with the upfilled
mean_catch_gf<- bind_rows(mean_catch, upfill_catch) %>%
  arrange(stock_id, year)

# Get the last known value in 2019 for each stock
upfill_cmsy <- cmsy %>%
  filter(year == 2019, !is.na(cmsy_bbmsy)) %>%
  select(stock_id, cmsy_bbmsy) %>%
  crossing(year = 2020:2023) %>%
  mutate(gapfill_cmsy = "upfilling")

# Add a gapfilled column to the original data
cmsy <- cmsy %>%
  mutate(gapfill_cmsy = "none")

# Combine original with the upfilled
cmsy_gf <- bind_rows(cmsy, upfill_cmsy) %>%
  arrange(stock_id, year)

# Reassign names for later on
mean_catch <- mean_catch_gf
cmsy <- cmsy_gf
```
...
```{r}
## SAUP v RAM BBMSY
length(unique(setdiff(ram$stock_id, mean_catch$stock_id))) # v2025: 76 stocks
length(unique(setdiff(mean_catch$stock_id, ram$stock_id))) # v2025: 8476 stocks
...
# 399 stocks with RAM-B/Bmsy data (although RAM is matched by fao and rgn ids) - v2025

## SAUP v CMSY
setdiff(cmsy$stock_id, mean_catch$stock_id) # v2025: 7 stocks in cmsy not in mean_catch
length(unique(setdiff(mean_catch$stock_id, cmsy$stock_id))) # v2025: 7853 in mean_catch not in cmsy
length(intersect(mean_catch$stock_id, cmsy$stock_id)) # v2025: 1022 stocks with CMSY-B/Bmsy data
...
# 1022 stocks with CMSY-B/Bmsy data; WOW! - v2025 (didn't change because none of these were updated this year)
...
```{r}
data <- mean_catch %>%
  # v2025: changed to a full join here because we want to keep RAM years up to 2023
  full_join(ram, by=c('rgn_id', 'stock_id', "year"))
...
# 648270 catch records (catch from specific fao and ohi regions) when joined with ram increases to 699525 because there are multiple stocks for some species and because we increased RAM data year to 2023 from 2019 - v2025


sum(!is.na(data$ram_bmsy))/nrow(data) # about 13.4% of catch records have RAM data - v2025
perc_ram <- sum(data$mean_catch[!is.na(data$ram_bmsy)], na.rm = TRUE)/sum(data$mean_catch, na.rm = TRUE) *100 # about 46.2% of tons of catch have RAM data - v2025
perc_ram
```
...
## Gapfill CMSY data from 2019 on to match RAM data
...
# 9867 regions with multiple RAM stocks (stockid_ram) for the same species - v2025
...
data <- data %>%
  group_by(rgn_id, taxon_key, stock_id, year, mean_catch, gapfill_catch) %>%   
...
#   # Changed from summarize to mutate to keep the catch gapfilling column
#   mutate(ram_bmsy = ifelse(all(!is.na(RAM_area_m2)), weighted.mean(ram_bmsy, RAM_area_m2, na.rm=TRUE), mean(ram_bmsy, na.rm = TRUE)),
#             gapfilled = ifelse(all(is.na(gapfilled)), NA, max(gapfilled, na.rm=TRUE)),
#             method = paste(method, collapse = ", ")) %>%
#   ungroup() %>% 
#   select(-RAM_area_m2, stocklong, stockid_ram)
# 
# nrow(data_m)

## check that averaging went ok - compare with mean catch values earlier (1350.19)
filter1 <- filter(data, rgn_id == 9, year == 2001, stock_id == "Thunnus_alalunga-71") # v2025: all good, still 1350.19
filter1

## check example of duplicate stock catch with ram_bmsy but no RAM_area_m2 value, ram_bmsy should not be NA
filter2 <- filter(data, rgn_id == 224, year == 2019, stock_id ==  "Heterocarpus_reedi-87") # v2025: all good
filter2
...
summary(data)
 ...
 #     rgn_id       taxon_key        
 # Min.   :  1.0   Length:648270     
 # 1st Qu.: 60.0   Class :character  
 ...
 #                                   
 #   stock_id              year     
 # Length:648270      Min.   :2001  
 # Class :character   1st Qu.:2006  
 ...
 #   mean_catch           ram_bmsy     
 # Min.   :      0.0   Min.   : 0.0    
 # 1st Qu.:      0.3   1st Qu.: 0.7    
 # Median :      9.6   Median : 1.1    
 # Mean   :   1800.8   Mean   : 1.2    
 # 3rd Qu.:    162.8   3rd Qu.: 1.6    
 # Max.   :2960647.0   Max.   :21.7    
 #                     NA's   :605823  
 #   gapfilled         method         
 # Min.   :0.0      Length:648270     
...
data_catch_gf <- data %>%
  arrange(stock_id, year, rgn_id) %>%
  group_by(stock_id, rgn_id) %>%
  mutate(mean_catch_gf = if_else(year > 2019, NA, mean_catch),
    taxon_key_gf = if_else(year > 2019, NA, taxon_key)) %>%
  fill(mean_catch_gf, taxon_key_gf, .direction = "up") %>%
  mutate(catch_gapfilled = if_else(is.na(mean_catch), "upfilling", "none"),
    taxon_gapfilled = if_else(is.na(taxon_key), "upfilling", "none")) %>%
  ungroup()
view(data_catch_gf)

data_catch_gf %>% 
  filter(!is.na(mean_catch_gf) & catch_gapfilled != "none") %>% 
  view()
...
data_catch_gf <- data %>%
  ...
  group_by(stock_id) %>%
  mutate(
    mean_catch_gf = if_else(year > 2019 & is.na(mean_catch), NA, mean_catch),
    taxon_key_gf = if_else(year > 2019 & is.na(taxon_key), NA, taxon_key)
  ) %>%
  fill(mean_catch_gf, taxon_key_gf, .direction = "up") %>%
  mutate(
    catch_gapfilled = if_else(is.na(mean_catch), "upfilling", "none"),
    taxon_gapfilled = if_else(is.na(taxon_key), "upfilling", "none")
  ) %>%
  ungroup()


class(data$mean_catch)
colSums(is.na(data_catch_gf))
dim(data_catch_gf)
view(data_catch_gf)

data_catch_gf %>% 
  filter(!is.na(mean_catch_gf)) %>% 
  view()

data %>%
  filter(year <= 2019) %>%
  filter(!is.na(mean_catch) | !is.na(taxon_key)) %>%
  count() # 653783

data %>%
  group_by(stock_id) %>%
  summarize(min_year = min(year), max_year = max(year)) %>%
  filter(max_year > 2019) # 8476 
nrow(data)

data %>%
  filter(stock_id == "Ablennes_hians-71") %>%
  arrange(year) %>%
  select(year, mean_catch, taxon_key)
...
data_catch_gf <- data %>%
  ...
  mutate(
    ...
    taxon_key_gf = taxon_key
  ) %>%
  fill(mean_catch_gf, taxon_key_gf, .direction = "up") %>%
  mutate(
    catch_gapfilled = if_else(is.na(mean_catch) & !is.na(mean_catch_gf), "upfilling", "none"),
    taxon_gapfilled = if_else(is.na(taxon_key) & !is.na(taxon_key_gf), "upfilling", "none")
  ) %>%
  ungroup()

# Get all combinations of stock_id, rgn_id, and full year range
full_grid <- data %>%
  distinct(stock_id, rgn_id) %>%
  crossing(year = 2001:2023)  # or use your actual year range

# Join with your data
data_full <- full_grid %>%
  left_join(data, by = c("stock_id", "rgn_id", "year"))
...
```{r save}
 data_gf <- data %>%
   ...
   dplyr::select(rgn_id, stock_id, taxon_key, year, bbmsy, bmsy_data_source, RAM_gapfilled = method, mean_catch, catch_gapfilled = gapfill_catch, cmsy_gapfilled = gapfill_cmsy) %>%
   filter(year >= 2001)
...
## check against old 
data_gf_old<- read_csv(here("globalprep","fis","v2024","output", "fis_bbmsy_gf.csv"))
...
old <- read_csv(here("globalprep","fis","v2024","output","fis_bbmsy_gf.csv"))

new <- read_csv(here("globalprep","fis","v2025","output","fis_bbmsy_gf.csv"))
...
new_pit <- new %>%
  filter(rgn_id == 146)
mean(new_pit$bbmsy, na.rm = TRUE) # 1.126548 

old_usa <- old %>%
  filter(rgn_id == 163)
mean(old_usa$bbmsy, na.rm = TRUE) # 1.150863

new_usa <- new %>%
  filter(rgn_id == 163)
mean(new_usa$bbmsy, na.rm = TRUE) # 1.195853 -- not the exact same, but close! v2025

# v2025: plot differences for better understanding
compare <- new %>%
  left_join(old, by = c("rgn_id", "stock_id", "taxon_key", "year")) %>%
  mutate(diff = bbmsy.x - bbmsy.y) %>%
  filter(year %in% c(2016:2023))

# v2025: points above the red line indicate an increase in B/Bmsy from 2024 to 2025, while points below indicate a decrease
comparison_plot <- ggplot(compare, aes(x = bbmsy.x, y = bbmsy.y)) +
  geom_point(alpha = 0.5) + 
  geom_abline(color = "darkred", linetype = "dashed") +  # reference line
  labs(
    title = "B/Bmsy Values: v2024 vs v2025",
    x = "v2024 B/Bmsy",
    y = "v2025 B/Bmsy"
  ) +
  theme_minimal() +
...
# v2025: check for year 2019 only
...
comparison_plot_2019 <- ggplot(compare_2019, aes(x = bbmsy.x, y = bbmsy.y)) +
  ...
  geom_abline(color = "darkred", linetype = "dashed") +  # reference line
  labs(
    title = "B/Bmsy Values in 2019: v2024 vs v2025",
    x = "v2024 B/Bmsy",
    y = "v2025 B/Bmsy"
  ) +
  theme_minimal() +
...
ggplot(fish_status_summary_2019, aes(x = fish_status, y = count, fill = fish_status)) +
  ...
  labs(title = "Comparison of Overfished and Sustainable Stocks in 2023",
...
over19_small <- overfished_2019[1:10, ]

# interactive plotly of b/bmsy for each stock
over_fis_plot <- over19_small %>%
  plot_ly(x = ~year, y = ~bbmsy, color = ~stock_id, 
          type = "scatter", mode = "lines") %>%
  layout(title = "B/Bmsy by Stock ID",
         xaxis = list(title = "Year"),
