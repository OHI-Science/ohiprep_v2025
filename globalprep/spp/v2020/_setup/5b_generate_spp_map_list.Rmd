...
if(!file.exists(bli_marine_shp)) {
  ...
  att_table <- foreign::read.dbf(bli_marine_dbf) %>%
    ...
    dplyr::rename(iucn_sid = sisid, binomial = sciname, year = date, tax_comm = tax_com) %>%
    dplyr::mutate(year = as.integer(as.character(year)))
  
#   [1] "iucn_sid"     "binomial"     "year"         "source"       "presence"     "origin"       "seasonal"    
#  [8] "data_sens"    "sens_comm"    "compiler"     "tax_comm"     "dist_com"     "reviewers"    "citation"    
# [15] "version"      "shape_length" "shape_area"
  
#    [1] "iucn_sid"   "binomial"   "year"       "source"     "presence"   "origin"     "seasonal"   "data_sens" 
#  [9] "sens_comm"  "compiler"   "tax_comm"   "dist_com"   "reviewers"  "citation"   "version"    "shape_leng"
# [17] "shape_area"
  

  foreign::write.dbf(att_table, bli_marine_dbf)
  #for some reason it is writing the column "shape_length" as "shape_leng".
}
...
## 2020: We do. Some of the files have been renamed to different things. Ex: Corals (2019) --> Reef_forming_corals (2020); Instead of downloading individual fish files, we just downloaded "marine fish"... etc.
