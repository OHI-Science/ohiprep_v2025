...
#v2024
- Mariculture and Fisheries data was updated since last scenario year
- Fixed trendline on graph

#v2023
- Updated Mariculture since last scenario year but not Fisheries
...
scen_year <- 2025 # change to latest year
prev_scen_year <- scen_year - 1

version_year  <- paste0("v", scen_year)
goal <- here::here("globalprep","fp")
...
```{r, eval = FALSE}
mar <- read.csv(here('globalprep/mar', version_year, "output", "MAR_FP_data.csv")) # see metadata in its prep
```

Fisheries data.
```{r, eval = FALSE}
fis <- read.csv(here('globalprep/fis', version_year, "output", "FP_fis_catch.csv")) %>%
...
```{r, eval = FALSE}
compare <- read.csv(here::here(goal, "v2024", "output/wildcaught_weight.csv")) %>%
...
```{r, eval = FALSE}
write.csv(tmp_weights, here::here(goal, version_year, 'output/wildcaught_weight.csv'), row.names=FALSE)
...
write.csv(tmp_gf, here::here(goal, version_year, 'output/wildcaught_weight_gf.csv'), row.names=FALSE)
```
