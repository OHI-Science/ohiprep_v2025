...
dir_wcmc <- file.path(file.path(dir_M, 'git-annex/globalprep/_raw_data/wcmc_coral/d2025')) #note: folder should be made for each year of new data
dir_aca <- file.path(file.path(dir_M, 'git-annex/globalprep/_raw_data/allen_coral_atlas/d2025')) #note: folder should be made for each year of new data
ohi_rasters() # call the region zones raster
```


## Prep Polygon Data for WCMC
```{r, eval = FALSE}
## read in polygon data
v4_coral_py <- sf::st_read(dsn = file.path(dir_wcmc, "14_001_WCMC008_CoralReefs2018_v4_1/01_Data"), layer = "WCMC008_CoralReef2018_Py_v4_1")

## take a look
head(v4_coral_py) 

# Make valid the raster
v4_coral_py <- v4_coral_py %>%
  st_make_valid() 
...
sum(st_area(v4_coral_py))*0.000001 # v2021: 152275.7 km2 using R function...
...
plot(fasterize_py, col = "blue")

writeRaster(fasterize_py, filename = file.path(dir_M, "/git-annex/globalprep/hab_coral/v2025/int/py_coral_rast.tif"), overwrite=TRUE) 
```
...
## save this
write.csv(zonal_sums_km2, file.path("globalprep/hab_coral/v2025/int/coral_area_zonal.csv"), row.names = FALSE)
```


## Check Data Points to see if WCMC data is still needed Prep
```{r, eval = FALSE}
## read in points data
v4_coral_pts <- sf::st_read(dsn = file.path(dir_wcmc, "14_001_WCMC008_CoralReefs2018_v4_1/01_Data"), layer = "WCMC008_CoralReef2018_Pt_v4_1")
...
st_write(coral_subset_points, file.path(dir_M, "git-annex/globalprep/hab_coral/v2025/int/coral_extent_regions_pts.shp"))

## plot to make sure it works 
ggplot() +
  geom_sf(data = rgns_eez$geometry,  col = sf.colors(239, categorical = TRUE)) +
  geom_sf(data = rgns_all$geometry, fill = sf.colors(229, categorical = TRUE)) +
...
coral_subset_pts <- st_read(file.path(dir_M, "git-annex/globalprep/hab_coral/v2025/int/coral_extent_regions_pts.shp"))

coral_pts_countries <- c(unique(coral_subset_pts$rgn_id))
...
mean(st_area(v4_coral_py)) # v2021 6.114509 km^2, v2025 8.680051 km^2

# Find the median value of the individual coral polygons
global_median_km2 <- as.numeric(st_area(v4_coral_py)) 
global_median_km2 <- (median(global_median_km2)) *0.000001  # v2021: 0.1371961, v2025: 0.1437127
...
coral_points_area_sum <- coral_points_area %>%
  mutate(extent_km2 = global_median_km2*count_points) %>%
  dplyr::select(rgn_id, sum_extent_km2_pts = extent_km2)

sum(coral_points_area_sum$sum_extent_km2_pts) # v2021: 126.9064 km2, v2025: 132.9343

## save this data
write.csv(coral_points_area_sum, file.path("globalprep/hab_coral/v2025/int/coral_points_area.csv"), row.names = FALSE)
```

**Combine points and polygon area estimates into one dataset**
```{r, eval = FALSE}
coral_points_area_sum <- read_csv(here("globalprep/hab_coral/v2025/int/coral_points_area.csv"))
coral_area_py_sum <- read_csv(here("globalprep/hab_coral/v2025/int/coral_py_area_zonal.csv"))
...
write.csv(coral_area_final, "globalprep/hab_coral/v2025/data/habitat_extent_coral_updated.csv", row.names = FALSE)
...
write.csv(coral_area_final_gf, "globalprep/hab_coral/v2025/data/extent_coral_gf.csv", row.names = FALSE)
```


# Datacheck
```{r, eval = FALSE}
coral_area_final <- read_csv(here("globalprep/hab_coral/v2025/data/habitat_extent_coral_updated.csv"))

habitat_extent_coral_old <- read_csv(here("globalprep/hab_coral/v2021/data/habitat_extent_coral_updated.csv"))


# Rename area columns for ease of comparison
habitat_extent_coral_old <- habitat_extent_coral_old %>%
  mutate(area_2021 = km2)

coral_area_final <- coral_area_final %>%
...
#132924.5 total area for the old extent d
