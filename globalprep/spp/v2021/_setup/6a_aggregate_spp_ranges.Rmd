...
dir_shp <- file.path(dir_M, 'git-annex/globalprep/_raw_data/iucn_spp/d2020-3')
...
## mclapply not working... just use a for loop... try with mclapply first though.. itll be quicker if it does work
...
``` {r examine non-comprehensively assessed, eval = FALSE, include = FALSE, echo = FALSE}
#even though the OHI Global Assessment does not use "non comprehesively assessed species", still run this section. It turns out that two of the spp in 2019's downloaded data that are actually comprehensively assessed, were noted as non comprehensive, Reptiles and Damselfish. 
x <- spp_maps %>%
  filter(!comp_assessed)
table(x$max_depth)

## area file is created in 6b setup file at the beginning...
area <- read_csv(file.path(dir_data, 'iucn_spp_range_area_2020-3.csv')) %>%
  dplyr::select(iucn_sid, range_km2)
