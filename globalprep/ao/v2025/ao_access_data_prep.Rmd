...
# This was used in v2025 to determine if there actually were countries missing from the data when pulled in 2025, or if there were mismatches in names 
...
## As above, this was the new method of comparing country names using countrycode() function in v2025 to determine if values were actually missing
...
## Add regions not encapsulated in the clean_data 
...
```{r}
# Create a dataframe to concatenate with our clean_data
...
# Merge this dataframe with the clean_data 
clean_data_complete <- rbind(clean_data, merge_with_clean_data)
```


```{r}
# Join the region_info (manually loaded from UN with m49 codes) and the clean_data (loaded via API with geo_area codes)
regional_data <- clean_data_complete %>%
...
# In the v2025 version, there were 28 regions still unaccounted for, we will now repeat this process for those regions (with the exception of uninhabited regions) 
```
...
## In 2025, all scores were filled with this method
```
...
## v2025 this filled up all of our scores!
```
...
version_year_new <- paste0("v", 2025 )
version_year_old <- paste0("v", 2024 )
...
# Replace NA scores with 0 to get new scores to show up on graph
