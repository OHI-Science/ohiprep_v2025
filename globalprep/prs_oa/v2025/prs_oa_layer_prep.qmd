---
title: 'OHI 2025: Ocean Acidification Data Prep'
...
The [data source product](https://data.marine.copernicus.eu/product/MULTIOBS_GLO_BIO_CARBON_SURFACE_REP_015_008/description) corresponds to a REP (reprocessed) L4 (optimally interpolated) time series of monthly global reconstructed surface ocean pCO2 (partial pressure of carbon dioxide), air-sea fluxes of CO2, pH, total alkalinity, dissolved inorganic carbon, saturation state with respect to calcite and aragonite, and associated uncertainties on a 0.25° x 0.25° regular grid.
...
### Notes from 2025 assessment
There were no major changes from the 2024 methodology, aside from one new data year (2023). As was the case in v2024, Copernicus Marine Service's Global Ocean Surface Carbon product is still an excellent resource.
...
v2025: Changed to MULTIOBS_GLO_BIO_CARBON_SURFACE_MYNRT_015_008
...
cms_dir <- here::here(raw_data_dir, "CMS", data_dir_version_year)

# prs_oa dir
oa_dir <- here::here(dir_M, "git-annex", "globalprep", "prs_oa", version_year)

# output data dir for intermediate data products
int_dir <- here::here(oa_dir, "int")
# dir.create(int_dir) # to create the path on Mazu if it has not already been done

# final output dir
output_dir <- here::here("globalprep","prs_oa", version_year, "output")
...
# v2025: copernicusmarine version 2.1.2

# Attempting to fix API upoad:
...
## New Data Download (Copernicus Marine Service, 2010 - 2022; Downloaded July 1st, 2025)
...
-   v2025: The link from previous years has been depricated. Link to data layer is now: <https://data.marine.copernicus.eu/product/MULTIOBS_GLO_BIO_CARBON_SURFACE_MYNRT_015_008/download>
...
# v2025: username <- menda; password <- OceanHealth2025

import os
import copernicusmarine

# set the credentials made on the CMS website
# copernicusmarine.login(username='menda', password='OceanHealth2025')
# v2024: INFO - 2024-07-30T16:04:43Z - Credentials file stored in /home/lecuona/.copernicusmarine/.copernicusmarine-credentials.
# v2025: INFO - 2025-07-01T18:48:15Z - Credentials file stored in /home/enda/.copernicusmarine/.copernicusmarine-credentials.
...
# v2025: Issues with API download after update. Previous function depricated
...
# v2025: 161 layers; data through 2023 has been added
...
## Run through the list and create individual rasters
...
for (i in 1:num_layers) {
  layer <- oa_multi_raster[[i]] # the layer in the seq from 1:161 (v2025)
...
names(individual_rasters) # v2024: "2010-01-01" - "2023-12-01"
...
```{r, eval= FALSE}
annual_avg_rescaled_dir <- here::here(oa_dir, "int","annual_avg_rescaled") 
...
oaRescale_updated <- function(file){
  ...
  writeRaster(oa_pressure, 
              filename = sprintf("%s/oa_rescaled_%s.tif",annual_avg_rescaled_dir, yr),
              overwrite = TRUE)
...
plot(oa_rescaled) # v2024: this looks great!
                  # v2025: Looks great! 14 rasters that show aragonite level rescaled
```
...
plot(log(zones)) # examine the .tif and ensure it looks correct. Using log() highlights zones
# example: convert anything greater than 500 to 1 to check data
...
reprojecting_func <- function(file){ # file = oa_rescaled_annual_files[1]
  ...
  r_moll_resample <- terra::resample(x = r_moll, y = zones, method = "bilinear", filename = sprintf("%s/oa_rescaled_moll_%s.tif",annual_avg_rescaled_moll_dir, yr), overwrite = TRUE) 
  }

tic()
lapply(oa_rescaled_annual_files, reprojecting_func)
toc() # v2024: 1047.173 sec elapsed
      # v2025: 1084.791 sec elapsed
```
...
plot(oa_reprojected) # v2025: this looks great!

ext(oa_reprojected) == ext(zones) # v2025: TRUE
res(oa_reprojected) == res(zones) # v2025: TRUE
crs(oa_reprojected) == crs(zones) # v2025: TRUE
...
regions_stats <- terra::zonal(oa_reprojected, zones, fun = "mean", na.rm = TRUE, progress = "text") %>%
  data.frame() %>%
  setNames(c("rgn_id", "scores_2010","scores_2011","scores_2012","scores_2013","scores_2014","scores_2015","scores_2016","scores_2017","scores_2018","scores_2019","scores_2020","scores_2021","scores_2022","scores_2023" ))
toc() # v2024: 361.876 sec elapsed
    # v2025: 373.009 sec elapsed
...
setdiff(regions_stats$rgn_id, rgn_data$rgn_id) # v2025: good! only the high seas regions that are not within the 250 OHI regions: 260 261 262 263 264 266 267 269 270 272 273 274 275 276 277
setdiff(rgn_data$rgn_id, regions_stats$rgn_id) # v2025: integer(0)


# Note: v2025 three countries had no values associated with their regional IDs. They were filled in based on regional patterns, which in this case, happened to be places of low oa pressure (Slovenia, Singapore, Jordan, & Bosnia and Herzegovina)
...
yearly_data <- regions_stats_join %>%
  group_by(year) %>% # group by all years, in this case it should be 2010 - 2023
...
# v2025: Create 4-year rolling average for scores 

- To reduce some of the variation we see, and to get a better idea of annual trends, we will be implementing a 4-year rolling average to the oa scores using the rollaply() function from the 'zoo' package.

- This will be used as a check in the future for region trends

```{r, eval= FALSE}
# Convert the pressure scores to integers
regions_stats_join$pressure_score <- as.numeric(regions_stats_join$pressure_score)

# Using region_stats_join, apply a 4-year rolling average 
four_year_average <- regions_stats_join %>%
  group_by(rgn_name) %>%
  mutate(mean_scores = rollmean(pressure_score, 4, NA, "right"))
```

```{r, eval= FALSE}
# Plot the 4-year rolling average scores
pressure_oa_plot <- four_year_average %>%
  plot_ly(x = ~year, y = ~mean_scores, color = ~rgn_name, 
          type = "scatter", mode = "lines") %>%
  layout(title = "Ocean Acidification 4-Year Averaged Pressure Scores by Region",
         xaxis = list(title = "Year"),
         yaxis = list(title = "Pressure Score"))
pressure_oa_plot
```
...
Climate Change Indicators: Ocean Acidity (2024) EPA Climate Change Indicators.
Available at: <https://www.epa.gov/climate-indicators/climate-change-indicators-ocean-acidity> (Accessed: 06 August 2024).

Barker, S.
& Ridgwell, A.
(2012) Ocean Acidification.
