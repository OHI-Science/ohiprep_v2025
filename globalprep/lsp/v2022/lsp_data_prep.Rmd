...
v2022
Using updated June 2022 data
Switched `raster` functions over to their `terra` equivalents.

v2021
Using updated February 2021 data 
...
rast_wdpa <- terra::rast(file.path(dir_goal_anx, 'rast', 'wdpa_2022_moll_500m.tif'))
...
* 2021 AY data goes through Jan (downloaded on Feb 2) 2021
...
status_3nm_old <- read_csv(file.path(dir_goal, '../v2021/output', 'lsp_prot_area_offshore3nm.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output', 'rgn_area_offshore3nm.csv')),
            by = 'rgn_id') %>%
...
status_1km_old <- read_csv(file.path(dir_goal, '../v2021/output', 'lsp_prot_area_inland1km.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output', 'rgn_area_inland1km.csv')),
            by = 'rgn_id') %>%
...
lsp_status_plot <- ggplot(lsp_new_old, 
  ...
  labs(x = 'LSP status v2021 (data through March 2021)',
       y = 'LSP status v2022 (data through Jan 2022)',
       title = 'Comparing LSP status: 2022 vs 2021') +
  facet_wrap( ~ rgn)

lsp_status_plot #got rid of ggplotly

ggsave(file.path(dir_goal, 'Figs/plot_v2021_v2022.png'), 
...
write.csv(x, "output/major_changes_2022.csv" ) #added this so I could come back to it without rerunning everything
```
...
lsp_new_old <- status_3nm_new %>%
  full_join(status_1km_new, by = c('rgn_id', 'year')) %>%
  mutate(status_new = (status_3nm_new + status_1km_new) / 2)
