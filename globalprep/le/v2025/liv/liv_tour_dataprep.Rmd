...
un_jobs_data <- un_jobs_data_raw %>% 
  select(country = GeoAreaName, year = TimePeriod, tourism_jobs = Value) %>% 
         # GeoAreaCode, Source, FootNote, Nature) %>% 
...
tour_jobs_labor <- full_join(tour_jobs_rgn_agg, labor_force , by = c("rgn_id", "rgn_name", "year"))  %>% 
...
#              year  tourism_jobs_ag   g 
#                 0              1883 
# labor_force_total              unit 
#               390                 0 
#    tour_jobs_prop 
#              1909 
...
line_plot <- plotly::plot_ly(tour_jobs_prop_df, x = ~year, y = ~tour_jobs_prop, color = ~rgn_name, type = "scatter", mode = "lines") %>% 
...
wage_data_clean <- ilo_wage_data %>% 
  ...
  filter(year %in% c(2009:max(tour_jobs_prop_df$year))) %>%
...
table(wage_data_clean$classif2_label)
