...
gofc <- ram_bmsy_new %>% 
  filter(str_detect(coalesce(stocklong, ""), regex("\\bCalifornia\\b", ignore_case = TRUE)))
write_csv(gofc, here("globalprep/fis/v2025/int/GofC_project.csv"))

pacific <- ram_bmsy_new %>% 
  filter(str_detect(coalesce(stocklong, ""), regex("\\bCalifornia\\b", ignore_case = TRUE)))
