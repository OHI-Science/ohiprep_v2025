...
dir_goal <- file.path('/home/shares/ohi/git-annex/globalprep', goal, scenario)
...
zonal_files <- c('zonal_3nm' =  file.path(dir_goal, 'int_haylee', 'zonal_stats_3nm.csv'),
                 'zonal_1km' =  file.path(dir_goal, 'int_haylee', 'zonal_stats_1km.csv'),
                 'zonal_eez' =  file.path(dir_goal, 'int_haylee', 'zonal_stats_eez.csv'))

# load raster created in 1_prep_wdpa_rast.rmd
rast_wdpa <- terra::rast(file.path(dir_goal_anx, 'rast', 'haylee_wdpa_2025_moll_500m.tif'))
...
all(paste(dest_dir, basename(files), sep="/") %in% files_dest)
...
# 2192.728 ~ 35 minutes
write_csv(x, '/home/shares/ohi/git-annex/globalprep/lsp/v2025/int_haylee/zonal_stats_3nm.csv')

y <- lsp_crosstab(rgn_rast_list[2], rast_values = rast_wdpa) #35 minutes to run #1km
cat('writeRaster elapsed: ', (proc.time() - ptm)[3])
# 4283.705 ~ 1hr 10 minutes
write_csv(y, '/home/shares/ohi/git-annex/globalprep/lsp/v2025/int_haylee/zonal_stats_1km.csv')

z <- lsp_crosstab(rgn_rast_list[3], rast_values = rast_wdpa) #~51 min minutes to run #eez
cat('writeRaster elapsed: ', (proc.time() - ptm)[3])
# 7391.148 ~ 2 hours
write_csv(z, '/home/shares/ohi/git-annex/globalprep/lsp/v2025/int_haylee/zonal_stats_eez.csv')
...
# Wrong file paths, something to do with using here()
...
write_csv(prot_3nm, file.path(dir_goal, 'int_haylee', 'area_protected_3nm.csv'))
write_csv(prot_1km, file.path(dir_goal, 'int_haylee', 'area_protected_1km.csv'))
write_csv(prot_eez, file.path(dir_goal, 'int_haylee', 'area_protected_eez.csv'))
```
...
prot_3nm <- read_csv(file.path(dir_goal, 'int_haylee', 'area_protected_3nm.csv')) %>%
  rename(area = a_tot_km2,
         a_prot_3nm = a_prot_km2)
prot_1km <- read_csv(file.path(dir_goal, 'int_haylee', 'area_protected_1km.csv')) %>%
  rename(area = a_tot_km2,
...
write_lsp_layer <- function(df, layers, layername) {
  df1 <- df[ , c('rgn_id', layers)] %>%
    filter(rgn_id <= 250) %>%
    distinct()
  write_csv(df1, file.path(dir_goal, 'output_haylee', paste0(layername, '.csv')))
}
...
resil_3nm <- read_csv(file.path(dir_goal, 'int_haylee', 'area_protected_3nm.csv')) %>%
  mutate(resilience.score = (a_prot_km2 / a_tot_km2) / area_ref,
         resilience.score = ifelse(resilience.score > 1, 1, resilience.score))

resil_eez <- read_csv(file.path(dir_goal, 'int_haylee', 'area_protected_eez.csv')) %>%
  mutate(resilience.score = (a_prot_km2 / a_tot_km2) / area_ref,
         resilience.score = ifelse(resilience.score > 1, 1, resilience.score))
...
## Save resilience scores for 3 nm and EEZ data in output
  ...
  write_csv(tmp_3nm, file.path(dir_goal, 'output_haylee', "mpa_3nm_resilience.csv"))

  tmp_eez <- resil_eez %>%
    dplyr::select(rgn_id, year, resilience.score)
  write_csv(tmp_eez, file.path(dir_goal, 'output_haylee', "mpa_eez_resilience.csv"))
...
library(dplyr)
# gap fill for eez
res_eez <- read_csv(file.path(dir_goal, 'output_haylee', "mpa_eez_resilience.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(res_eez, file.path(dir_goal, 'output_haylee', "mpa_eez_resilience_gf.csv"), row.names=FALSE)

# gap fill for 3nm
res_3nm <- read_csv(file.path(dir_goal, 'output_haylee', "mpa_3nm_resilience.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(res_3nm, file.path(dir_goal, 'output_haylee', "mpa_3nm_resilience_gf.csv"), row.names=FALSE)

# gap fill for inland 1km
inland <- read_csv(file.path(dir_goal, 'output_haylee', "lsp_prot_area_inland1km.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(inland, file.path(dir_goal, 'output_haylee', "lsp_prot_area_inland1km_gf.csv"), row.names=FALSE)

# gap fill for offshore 3nm
offshore <- read_csv(file.path(dir_goal, 'output_haylee', "lsp_prot_area_offshore3nm.csv")) %>% 
  mutate(resilience.score = 0) %>% 
  rename(gapfilled = resilience.score)

write.csv(offshore, file.path(dir_goal, 'output_haylee', "lsp_prot_area_offshore3nm_gf.csv"), row.names=FALSE)
...
**Issue:** Cannot find files from v2024 calculation
...
status_3nm_new <- read_csv(file.path(dir_goal, 'output_haylee', 'lsp_prot_area_offshore3nm.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output_haylee', 'rgn_area_offshore3nm.csv')),
            by = 'rgn_id') %>%
...
status_3nm_old <- read_csv(file.path(dir_goal, '../v2024/output', 'lsp_prot_area_offshore3nm.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output_haylee', 'rgn_area_offshore3nm.csv')),
            by = 'rgn_id') %>%
...
status_1km_new <- read_csv(file.path(dir_goal, 'output_haylee', 'lsp_prot_area_inland1km.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output_haylee', 'rgn_area_inland1km.csv')),
            by = 'rgn_id') %>%
...
status_1km_old <- read_csv(file.path(dir_goal, '../v2024/output', 'lsp_prot_area_inland1km.csv')) %>% 
  full_join(read_csv(file.path(dir_goal, 'output_haylee', 'rgn_area_inland1km.csv')),
            by = 'rgn_id') %>%
...
lsp_status_plot <- ggplot(lsp_new_old, 
  ...
  labs(x = 'LSP status v2024 (data through Jun 2023)',
       y = 'LSP status v2025 (data through)',
       title = 'Comparing LSP status: 2025 vs 2024') +
  facet_wrap( ~ rgn)
...
write.csv(mjr_score_change, "output_haylee/major_changes_2025.csv")
```
