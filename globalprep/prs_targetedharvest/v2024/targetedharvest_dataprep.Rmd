...
This analysis converts FAO capture production data into the OHI 2024 targeted harvest pressure data.
...
© FAO 2024. Global Capture Production. In: Fisheries and Aquaculture. Rome. [Cited Wednesday, August 14th 2024]. 
<https://www.fao.org/fishery/en/collection/capture?lang=en>

**Downloaded**: July 10, 2024
...
fao_clean <- fis_fao_raw %>% 
  ...
  dplyr::select(-c("Unit Name", "ISSCAAP group Name En", "Family Scientific name", "Inland/Marine areas Name En")) %>% #  added removing Unit also
...
#not sure if this is necessary still but I ran it and kept it in just in case-v2024
fao_clean <- fao_clean %>%
  dplyr::mutate(
...
unique(fao_clean$species) # 3398 species
unique(target_spp$species) # 72 species
```
...
Data is rescaled by dividing by the 95th quantile of values across all regions from 2011 to 2022 (most recent year of FAO data).
...
# Add year; for v2024, min year is 1950, and max year is 2022
...
# pull just 2021 data from target_harvest df, since its the most recent year in common
common_year <- 2021
...
old <- readr::read_csv(here("globalprep", "prs_targetedharvest", paste0('v', version_year - 1), "output", "fao_targeted.csv")) %>%
  dplyr::filter(year == common_year) %>%
  dplyr::select(-year) %>%
  dplyr::select(rgn_id, pressure_score_2021 = pressure_score) %>%
  dplyr::left_join(new, by = c("rgn_id")) %>%
  dplyr::rename(pressure_score_2022 = pressure_score) %>%
  left_join(rgns_eez)
  

# Compare pressure_score between last year and this year's assessments
compare_plot <- ggplot(data = old, aes(x = pressure_score_2021, y = pressure_score_2022, label = rgn_name)) +
  geom_point() +
  geom_abline(color = "red")
