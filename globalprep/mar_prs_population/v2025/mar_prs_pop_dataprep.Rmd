...
**Reference**: [SEDAC](https://search.earthdata.nasa.gov/search/granules?p=C3540929692-ESDIS&pg[0][v]=f&pg[0][gsk]=-start_date&q=GPWv4&tl=1277985600!5!!), search for GPWv4
...
Publisher: ESDIS
Release Date: 2018-12-31T00:00:00.000Z
Release Place: Palisades, NY
Linkage: https://doi.org/10.7927/H4PN93PB
[URL](https://cmr.earthdata.nasa.gov/search/concepts/C3540909447-ESDIS.html)
...
Release Date: 2018-12-31T00:00:00.000Z
Release Place: Palisades, NY
Linkage: https://doi.org/10.7927/H4F47M65
[URL](https://cmr.earthdata.nasa.gov/search/concepts/C3540929692-ESDIS.html)
...
raw1 <- terra::rast(raw[1]) %>% 
  terra::writeRaster(., filename = file.path(dir_M, sprintf("git-annex/globalprep/mar_prs_population/%s/int/human_count_%s_mol.tif", scenario_yr, 2000)), overwrite = TRUE)

raw2 <- terra::rast(raw[2]) %>% 
  terra::writeRaster(., filename = file.path(dir_M, sprintf("git-annex/globalprep/mar_prs_population/%s/int/human_count_%s_mol.tif", scenario_yr, 2005)), overwrite = TRUE)

raw3 <- terra::rast(raw[3]) %>% 
  terra::writeRaster(., filename = file.path(dir_M, sprintf("git-annex/globalprep/mar_prs_population/%s/int/human_count_%s_mol.tif", scenario_yr, 2010)), overwrite = TRUE)

raw4 <- terra::rast(raw[4]) %>% 
  terra::writeRaster(., filename = file.path(dir_M, sprintf("git-annex/globalprep/mar_prs_population/%s/int/human_count_%s_mol.tif", scenario_yr, 2015)), overwrite = TRUE)

raw5 <- terra::rast(raw[5]) %>% 
  terra::writeRaster(., filename = file.path(dir_M, sprintf("git-annex/globalprep/mar_prs_population/%s/int/human_count_%s_mol.tif", scenario_yr, 2020)), overwrite = TRUE)
...
# files <- list.files(file.path(dir_M, "git-annex/globalprep/mar_prs_population/", scenario_yr, "/int"), pattern = ".tif$", full = TRUE)
...
dir_M # Checking dir_M ("/home/shares/ohi")
...
count_2018 <- terra::rast(file.path(dir_M, "git-annex/globalprep/mar_prs_population/", scenario_yr, "/int/human_count_2018_mol.tif"))
plot(count_2018)
...
compare_yr <- 2020 # doing below functions for comparison between raster and UN population data for 2020
...
compare_pop <- data.frame(pop_counts) %>%
  dplyr::rename(sum = pop_2020) %>% #gpw_v4_population_count_adjusted_to_2015_unwpp_country_totals_rev11_2020_30_sec
