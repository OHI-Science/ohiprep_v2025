...
1. Read in intermediate data B (USD/tonnes)
2. Read in the intermediate files with absolute tonnes of production (raw data)
...
# Commodity value and raw production data
...
colSums(is.na(harvest_tonnes_usd))
nrow(harvest_tonnes_usd)

#### Quantity data (raw) ####
# Ornamentals
orn <- read_csv(here(current_np_dir, "int", "np_relative_sust_tonnes_ornamentals_full.csv")) 
colnames(orn)
colSums(is.na(orn))
nrow(orn)
summary(orn)
...
# Seaweed
seaweed <- read_csv(here(current_np_dir, "int", "np_relative_sust_tonnes_seaweeds_full.csv")) 
colnames(seaweed)
colSums(is.na(seaweed))
nrow(seaweed)
summary(seaweed)

# Fish oil fish meal: Not sure if this is the right file...
fofm <- read_csv(here(current_np_dir, "int", "raw_mean_catch_FOFM.csv")) 
colnames(fofm)
colSums(is.na(fofm))
nrow(fofm)
summary(fofm)
...
# Calculate total USD for each product

FOFM: USD/tonne * tonnes of prod for SAUP

FOFM: only data from 2019 because the 4 year range goes from 2023-2019. Gapfill up to more recent years with the 2019 value?
...
harvest_tonnes_usd_geo_rgn <- harvest_tonnes_usd %>%
  ...
  arrange(rgn_id) %>% 
  # Sanity check (this didn't change anything)
  distinct(rgn_name, rgn_id, product, year)

view(harvest_tonnes_usd_geo_rgn)
nrow(harvest_tonnes_usd_geo_rgn) # 3300
...
geo_rgn_values_df <- harvest_tonnes_usd_geo_rgn %>%
  ...
  dplyr::mutate(georgn_usd_per_tonne = usd/tonnes) %>%
    mutate(georgn_usd_per_tonne = ifelse(georgn_usd_per_tonne == "Inf", NA, georgn_usd_per_tonne)) %>% ## assign Inf as NA
  # v2025: removing this line for now because I don't think we want 5 year averages yet. 
  # summarise(georgn_values = mean(values, na.rm = TRUE)) %>%
  ungroup() %>% ## now we have a 5 year average value for geo region
  distinct(georgn_id, year, product, georgn_usd_per_tonne) %>% 
  select(georgn_id, year, product, georgn_usd_per_tonne)

view(geo_rgn_values_df)
nrow(geo_rgn_values_df) # 1728
...
global_values_df <- harvest_tonnes_usd_geo_rgn %>%
  ...
  mutate(global_usd_per_tonne = ifelse(global_usd_per_tonne == "Inf", NA, global_usd_per_tonne)) %>% ## assign Inf as NA
  # summarise(global_values = mean(values, na.rm = TRUE)) ## now we have a 5 year average global value
  ungroup() %>% 
  distinct(product, year, global_usd_per_tonne) %>% 
  select(product, year, global_usd_per_tonne)

view(global_values_df)
nrow(global_values_df) # 1244
...
colnames(harvest_tonnes_usd_geo_rgn)
colnames(geo_rgn_values_df)
colnames(global_values_df)
colnames(harvest_tonnes_usd_values_gf)

nrow(harvest_tonnes_usd_geo_rgn) # 3300
nrow(geo_rgn_values_df) # 1728
nrow(global_values_df) # 1244
nrow(harvest_tonnes_usd_values_gf) # 1557636
...
view(harvest_tonnes_usd_values_gf)
nrow(harvest_tonnes_usd_values_gf) # 18607
colnames(harvest_tonnes_usd_values_gf)
...
weighting_usd_values_raw <- harvest_tonnes_usd_values_gf %>%
  ...
  # distinct(rgn_id, product, year, usd_per_tonne)

nrow(weighting_usd_values_raw) # 130837 was (1557636)
view(weighting_usd_values_raw)
...
## Individual commodities data prep
...
sw_tonnes <- sw_tonnes_raw %>%
  ...
  # ungroup() %>%
  # group_by(rgn_id, product) %>%
  # summarise(mean_tonnes = mean(tonnes)) %>% ## calculate 5 year average
  # ungroup()
...
fofm_tonnes <- read_csv(here(current_np_dir, "int", "raw_mean_catch_FOFM.csv")) %>%
  ...
  # group_by(rgn_id, product) %>%
  # calculate 5 year mean of catch -- 
  # summarise(mean_sum_tonnes = mean(sum_tonnes)) %>% 
  # ungroup() %>%
  # mutate(mean_sum_tonnes = ifelse(is.na(mean_sum_tonnes), 0, mean_sum_tonnes)) 
view(fofm_tonnes)



    # tonnes = ifelse(tonnes == 0, ))
...
colSums(is.na(usd_tonnes)) # 2641
nrow(usd_tonnes) # 10860
# ~ 24% NAs
...
view(np_value)
colnames(np_value)
unique(np_value$year)

harvest_tonnes_usd_geo_rgn <- np_value %>%
  # full_join(region_product, by = join_by("rgn_id", "product", "year")) %>%
  add_georegion_id() %>%
  arrange(rgn_id) # %>% 
  # Sanity check (this didn't change anything)
...
#   dplyr::group_by(georgn_id, year, product) %>%
#   # calculate values ($/tonnes)
#   dplyr::mutate(georgn_usd_per_tonne = usd/tonnes_commod) %>%
#     mutate(georgn_usd_per_tonne = ifelse(georgn_usd_per_tonne == "Inf", NA, georgn_usd_per_tonne))  %>% ## assign Inf as NA
#   distinct(georgn_id, year, product, georgn_usd_per_tonne) %>%
#   select(georgn_id, year, product, georgn_usd_per_tonne)
...
geo_rgn_values_df <- harvest_tonnes_usd_geo_rgn %>%
  ...
  mutate(georgn_usd_per_tonne = usd / tonnes_commod) %>%
...
view(geo_rgn_values_df)
nrow(geo_rgn_values_df) # 345

# calculate global values (for each product across all regions for each year)
# global_values_df <- harvest_tonnes_usd_geo_rgn %>%
#   dplyr::group_by(product, year) %>%
#   mutate(global_usd_per_tonne = usd/tonnes_commod) %>%
#   mutate(global_usd_per_tonne = ifelse(global_usd_per_tonne == "Inf", NA, global_usd_per_tonne)) %>% ## assign Inf as NA
#   # summarise(global_values = mean(values, na.rm = TRUE)) ## now we have a 5 year average global value
#   ungroup() %>% 
#   distinct(product, year, global_usd_per_tonne) %>% 
...
harvest_tonnes_usd_values_gf <- harvest_tonnes_usd_geo_rgn %>%
  ...
  # This isn't joining correctly because there are multiple georgn_ids per rgn_id, hence the many-to-many relationship. I thought I could still do this part here, but I think it may have to wait until after you join with the commodities data. 
...
np_tonnes <- read_csv(here(current_np_dir, "int", "np_harvest_usd_tonnes.csv")) %>% 
...
g_new <- ggplot(check, aes(x = weight, y = new_weight, color = product,
  ...
  scale_color_manual(values = c("fish_oil" = "#edae49",
                                "seaweeds" = "#386641",
                                "ornamentals" = "#83c5be")) +
  labs(title = paste(scen_year, "vs", prev_scen_year, "Natural Product Weights by Product"),
       x = paste(prev_scen_year, "Weights"),
       y = paste(scen_year, "Weights"),
       color = "Product") +
  theme_minimal()
