...
maric_join <- left_join(maric, identifier, by = join_by(rgn_id, species, environment))

# It looks like all the identifiers applied correctly 
length(unique(maric_join$species_code)) # 1501
...
# 20023 NA values out of 40133 total obs: ~ 49.9% without scores (2025 + 2024 combined)
```
...
compare_gen_vietnam <- full_join(old_gen_vietnam, new_gen_vietnam, by = c("rgn_id", "year"))

# View(compare_gen_vietnam)

compare_gen_vietnam_plot <- ggplot() + 
  geom_point(data = compare_gen_vietnam, aes(x = prs_score_old, y = pressure_score, text = paste("Region ID:", rgn_id))) +
...
compare_gen_denmark <- full_join(old_gen_denmark, new_gen_denmark, by = c("rgn_id","year"))

# View(compare_gen_denmark)

compare_gen_denmark_plot <- ggplot() + 
  geom_point(data = compare_gen_denmark, aes(x = prs_score_old, y = pressure_score, text = paste("Region ID:", rgn_id))) +
  labs(x = paste("Previous Pressure", previous_data_year), 
       y = paste("Current Pressure", previous_data_year),
