for (p in poles){ 
  ...
  range.years = (last_year-10):(last_year) # last_year defined in data prep set-up code chunk
  ...
  z.h.T = sih.avg3yr %>% zonal(r.rgn, sum) %>% merge(z.h.R) # has years in range.years (last_year-10 to last_year)
  ...
  for (j in (last_year-6):last_year){
    # j = 2012 # testing
    trend.years = (j - 4):j
  ...
  for (j in (last_year-6):last_year){ # j = 2015
    trend.years = (j - 4):j
    early_year <- min(trend.years) 
