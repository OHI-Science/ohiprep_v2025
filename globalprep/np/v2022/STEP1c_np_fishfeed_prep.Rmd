...
This script takes the SAUP 2022 catch data, a list of fish oil/fish meal (FOFM) species, and B/Bmsy estimates to calculate a score for FOFM species per OHI region. The FIS data prep will need to be completed prior to this data prep.  
...
forage <- read_csv(file.path(here(), "globalprep/np/v2022/raw/msleckman.61.1-CatchMSY_Nis_FAOAreas.csv"))
sort(unique(forage$Species)) #238 forage fish groups listed

saup_taxonkey <- read.csv(file.path(here(), "globalprep/fis/v2022/int/taxon_key_v2022.csv"))
...
write.csv(foragefish_list, here("globalprep/np/v2022/int/master_taxa_list_SAUP.csv"), row.names=FALSE)
...
foragefish_list <- read.csv(here("globalprep/np/v2022/int/master_taxa_list_SAUP.csv"))
...
write.csv(forage_fish_no_saup, file.path(here(), "globalprep/np/v2022/int/forage_fish_not_in_SAUP.csv"), row.names = FALSE)
...
write.csv(catch_fishfeed, file.path(here(), "globalprep/np/v2022/int/saup_catch_forage_fish.csv"), row.names = FALSE)
...
```{r}
catch_fishfeed <- read_csv(file.path(here(),"globalprep/np/v2022/int/saup_catch_forage_fish.csv"))
...
write.csv(mean_catch_FOFM,  file.path(here(), "globalprep/np/v2022/int/mean_catch_FOFM.csv"), row.names = FALSE)
```
...
fis_bbmsy_gf <- read.csv(file.path(here(), "globalprep/fis/v2022/output/fis_bbmsy_gf.csv"))

catch_FOFM <- read_csv(file.path(here(), "globalprep/np/v2022/int/mean_catch_FOFM.csv")) 
...
## The upper limit can be deleted, since we already cap pedscores at 1 anyways. We still want to keep the lower buffer.
 ...
 penaltyTable <- data.frame(TaxonPenaltyCode = 1:6,
  ...
  write.csv(gap_fill_data, here('globalprep/np/v2022/output/NP_bbmsy_summary_gf.csv'), row.names = FALSE)
...
write.csv(score_data, here('globalprep/np/v2022/output/np_fofm_scores.csv'), row.names = FALSE)


old <- read_csv(here('globalprep/np/v2021/output/np_fofm_scores.csv')) 
summary(old)

new <- read_csv(here('globalprep/np/v2022/output/np_fofm_scores.csv'))
