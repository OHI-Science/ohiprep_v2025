...
1. An average non-industrial catch dataset used to weight B/Bmsy (biomass/biomass of maximum sustainable yield) values in the fisheries model. For this dataset, the catch is assigned to FAO and OHI regions.
...
The SAUP data was not updated in v2023, v2024, or v2025, so this script uses the 2019 SAUP data for catch. Raw and intermediate data stored on Mazu is pulled from v2022.
...
catch <- read_csv(here(fis_path, "stock_catch_by_rgn_taxa.csv")) # data goes up to 2019
...
output_df <- catch %>% 
    ...
    dplyr::ungroup()
# Note: We are writing the output to the server, so if folder does not exist, it must be created
write_csv(output_df, file = here(dir_M,"git-annex","globalprep","ao","v2025","int","ao_stock_catch_by_rgn_taxa.csv")) # Update year
...
df <- read_csv(here::here(dir_M,'git-annex', 'globalprep', 'ao','v2025','int','ao_stock_catch_by_rgn_taxa.csv')) %>%
  left_join(rgns_eez)
...
```{r mean catch, eval= FALSE}

file <- here::here(dir_M,'git-annex', 'globalprep', 'ao','v2025','int','ao_stock_catch_by_rgn_taxa.csv')
