...
* Since SEDAC was terminated (see below), we opted to use Oak Ridge National Library LandScan Global data for a couple of reasons:

1. Data is still being updated
2. There are data from year 2000 onward
...
pop <- read.csv(file.path(dir_server, 
                ...
                # Added NA to blank cells
                na.strings = c(""))
```
## Crop rasters to common extent

The data from LandScan vary *slightly* in extent. So, extents were compiled into a df and then the largest commonn extent was determined. Rasters were cropped to this extent rather than filled with NA values.

```{r Crop Rasters, eval=FALSE}
# Initiate an empty list
rasters <- list()

# Add each raw raster name to the list to iterate over
for (i in 1:24) {
...
# files <- list.files(file.path(dir_M, "git-annex/globalprep/mar_prs_population/", scenario_yr, "/int"), pattern = "human_count_\\d{4}_mol.tif$", full = TRUE)
# 
...
# added for 2025 analysis:
pop_clean <- pop %>%
  filter(Type == "Country/Area")

pop_gather <- dplyr::select(pop_clean, c(matches("Region"), "Year", matches("Total.*July"))) %>%
...
**2025 NOTE:**
The format of the UN population data changed, so I had to make a decision to take the total population from the "Total Population January" column. These numbers are similar to the past data (i.e. v2024/output/UN_population.csv) but not the same. 
...
compare_yr <- 2023 # doing below functions for comparison between raster and UN population data for 2020
...
compare_pop <- data.frame(pop_counts) %>%
  dplyr::rename(sum = paste0("pop_", compare_yr)) %>% #gpw_v4_population_count_adjusted_to_2015_unwpp_country_totals_rev11_2020_30_sec
...
prev_scen_yr <- paste0("v", as.numeric(substr(scenario_yr, 2, 5)) -1) # last number was 3? not 1?
